...
async function setupPatient(){
 ...
 await page.getByText('Paciente QA').first().click();
 await page.waitForTimeout(1000);
}
await setupPatient();
for (const tab of ['Tarefas','Parametros','Exames','Medicacoes','Boletins','Resumo diário']) {
  ...
  const snapshot = await page.evaluate((tabName) => ({
    ...
    inputs: Array.from(document.querySelectorAll('input, textarea, select')).map((el) => ({ tag: el.tagName, placeholder: el.getAttribute('placeholder'), value: el.value })).slice(0, 40),
  }), tab);
  console.log('TABSNAP', JSON.stringify(snapshot));
}
await browser.close();
