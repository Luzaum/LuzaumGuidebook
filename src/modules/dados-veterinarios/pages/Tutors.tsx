...
export const Tutors = () => {
  ...
  const draftKey = `dv:tutors:new-draft:${selectedClinicId}`;
  ...
  useEffect(() => {
    try {
      ...
      const parsed = JSON.parse(raw) as Partial<{
        ...
        draftPatients: NewPatientDraft[];
      }>;
      if (parsed.newTutor) setNewTutor((prev) => ({ ...prev, ...parsed.newTutor }));
      if (Array.isArray(parsed.draftPatients) && parsed.draftPatients.length) {
        setDraftPatients(parsed.draftPatients);
      }
    } catch (error) {
      console.warn('[Tutors] Failed to hydrate draft:', error);
    }
  }, [draftKey]);

  useEffect(() => {
    try {
      window.localStorage.setItem(draftKey, JSON.stringify({ newTutor, draftPatients }));
    } catch (error) {
      console.warn('[Tutors] Failed to persist draft:', error);
    }
  }, [draftKey, newTutor, draftPatients]);
  ...
  const resolveCep = async (cepDigits: string): Promise<CepLookupResponse> => {
    ...
    return {
      ...
      complement: brasilApiData.service || '',
    };
  };
  ...
  const handleCepImport = async () => {
    ...
    setCepError('');
    try {
      const data = await resolveCep(cepDigits);
      setNewTutor((prev) => ({
        ...prev,
      ...
      setCepError('Não foi possível importar o endereço completo pelo CEP.');
    } finally {
      setCepLoading(false);
    }
  };
  ...
  return (
    <div className="dv-aurora-bg dv-section-enter space-y-8 rounded-[28px] p-3 md:p-5 min-h-full flex flex-col">
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-gray-400">ID: {tutor.id.padStart(5, '0')}</p>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Novo Tutor + Pacientes" className="max-w-7xl">
