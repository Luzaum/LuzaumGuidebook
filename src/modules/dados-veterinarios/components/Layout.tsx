...
export const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="dv-app flex h-screen w-full bg-gray-50 dark:bg-neutral-900 transition-colors duration-300 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 h-full overflow-hidden relative">
        <div className="h-11 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-md border-b border-gray-200 dark:border-neutral-800 flex items-center justify-between px-4 md:px-6">
          <Link to="/hub" className="inline-flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:text-primary transition-colors">
            <ArrowLeft size={16} />
            Voltar para o Vetius Hub
          </Link>
          <Link to={dvPath('account')} className="inline-flex items-center gap-2 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white/80 dark:bg-neutral-900/70 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-200 hover:border-primary/40 hover:text-primary transition-colors">
            <Shield size={14} />
            Conta: {currentUser.name}
          </Link>
        </div>
        <header className="h-16 bg-white/70 dark:bg-neutral-900/70 backdrop-blur-md border-b border-gray-200 dark:border-neutral-800 flex items-center justify-between px-4 md:px-6 sticky top-0 z-20 transition-colors duration-300 gap-4">
          ...
          <div className="flex-1 flex justify-end items-center gap-3">
            <Link to={dvPath('account')} className="hidden sm:inline text-sm text-gray-700 dark:text-gray-200 hover:text-primary transition-colors font-medium">
              {currentUser.name}
            </Link>
