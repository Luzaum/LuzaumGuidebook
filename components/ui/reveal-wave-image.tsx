...
function ImagePlane({
    ...
    const scale = useMemo<[number, number, number]>(() => {
        ...
        if (aspectRatio > 1) {
            ...
            return [aspectRatio * zoom, 1 * zoom, 1];
        } else {
            // Image is taller than wide
            return [1 * zoom, (1 / aspectRatio) * zoom, 1];
        }
    }, [aspectRatio, zoom]);
    useFrame((state) => {
        if (meshRef.current) {
            ...
            if (hasEnteredRef.current) {
                // Ajusta coordenada do mouse para compensar o deslocamento da mesh (imageShiftX)
                // UV vai de 0 a 1. Mesh tem largura scale[0] * 2.
                // Shift de X unidades no mundo equivale a X / (2 * scale[0]) unidades em UV.
                const uvShiftX = imageShiftX / (2 * scale[0]);

                material.uniforms.uMouse.value.set(
                    ((pointer.x + 1) / 2) - uvShiftX,
                    (pointer.y + 1) / 2,
                );
            }
        }
    ...
    return (
        <mesh ref={meshRef} scale={scale} position={[imageShiftX, 0, 0]}>
