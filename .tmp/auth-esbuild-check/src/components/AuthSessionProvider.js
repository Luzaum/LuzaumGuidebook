...
function authorizedSession(session) {
  return session?.user && hasAppAccess(session.user) ? session : null;
}
...
function AuthSessionProvider({ children }) {
  ...
  const refreshSession = useCallback(async () => {
    ...
    try {
      ...
      const nextSession = authorizedSession(data.session || null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (data.session && !nextSession) {
          await supabaseSignOut().catch(() => void 0);
        }
      } catch {
        if (!alive) return;
        setSession(null);
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange((_event, rawSession) => {
      if (!alive) return;
      const nextSession = authorizedSession(rawSession);
  ...
  const value = useMemo(
    () => ({
      ...
      profile,
      isAuthenticated: Boolean(session?.user && hasAppAccess(session.user)),
      refreshSession,
      logout
    }),
