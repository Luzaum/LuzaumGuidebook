              ...
              {filteredReports.map((report) => {
                ...
                return (
                  <article key={report.id} className="grid gap-4 rounded-2xl border border-border bg-card p-4 transition-colors duration-200 hover:border-primary/25 lg:grid-cols-[minmax(180px,0.7fr)_minmax(260px,1.2fr)_repeat(3,minmax(110px,0.45fr))_auto] lg:items-center">
                    ...
                    <div className="min-w-0"><p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Prescrição</p><p className="mt-1 truncate text-sm font-medium text-foreground">{prescription}{report.formula.contributions.length > 2 ? ` +${report.formula.contributions.length - 2}` : ''}</p><p className="mt-1 truncate text-xs text-muted-foreground">{getHumanRequirementLabel(report.diet.requirementProfileId)}</p></div>
