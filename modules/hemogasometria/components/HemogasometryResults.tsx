...
const ResultCard = ({ title, content, emoji, dataKey, onOpenModal, delay }: any) => (
  <div className={`bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl p-5 rounded-2xl shadow-xl border border-white/20 dark:border-slate-800 flex items-start gap-4 animate-in fade-in slide-in-from-bottom-4 print:shadow-none print:border-slate-200 print:bg-white print:animate-none print:p-4`} style={{ animationDelay: `${delay}ms` }}>
...
const ElectrolyteCard = ({ electrolyteStatus, onOpenModal, delay }: { electrolyteStatus: any[], onOpenModal: any, delay: number }) => {
  ...
  return (
    <div className={`bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl p-6 rounded-2xl shadow-xl border border-white/20 dark:border-slate-800 flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 print:shadow-none print:border-slate-200 print:bg-white print:animate-none print:p-4`} style={{ animationDelay: `${delay}ms` }}>
      <div className="flex items-center gap-3">
        <div className="text-3xl bg-amber-500/10 text-amber-500 p-3 rounded-xl border border-amber-500/20 print:bg-slate-50 print:border-slate-200">⚡</div>
        <h3 className="font-bold text-xl text-slate-800 dark:text-white print:text-slate-700">Eletrólitos e Proteínas</h3>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
        {electrolyteStatus.map((e: any) => {
          const isNormal = e.status === 'Normal';
          return (
            <div key={e.name} className={`p-4 rounded-xl border flex items-center justify-between transition-all hover:scale-[1.02] ${isNormal ? 'bg-green-500/5 border-green-500/20 print:border-green-200' : 'bg-amber-500/5 border-amber-500/20 print:border-amber-200'} print:animate-none`}>
              <div>
                <div className="text-xs font-bold text-slate-500 underline decoration-slate-200 uppercase tracking-widest">{e.name}</div>
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="font-black text-2xl text-slate-900 dark:text-white print:text-slate-800">{e.value}</span>
              ...
              <div className="flex flex-col items-end gap-2">
                <span className={`font-black text-[10px] uppercase tracking-widest px-2.5 py-1 rounded-full ${isNormal ? 'bg-green-500/20 text-green-500 print:bg-green-50 print:text-green-600' : 'bg-amber-500/20 text-amber-500 print:bg-amber-50 print:text-amber-600'}`}>{e.status}</span>
                <button type="button" onClick={() => onOpenModal(e.status.toLowerCase())} className="text-xl text-slate-400 hover:text-blue-500 transition-colors print:hidden">❓</button>
              </div>
            </div>
          );
        })}
