...
export const HemogasometrySidebar: React.FC<Props> = ({ result, inputs, onOpenModal }) => {
  if (!result) return (
    <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-white/20 dark:bg-slate-900/20 backdrop-blur-xl border-l border-white/10 dark:border-slate-800 rounded-3xl">
      <div className="size-20 bg-blue-500/10 rounded-full flex items-center justify-center mb-6 animate-pulse">
        <span className="material-symbols-outlined text-4xl text-blue-500">biometry_setup</span>
      </div>
      <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">Aguardando Dados</h3>
      <p className="text-slate-500 dark:text-slate-400">Complete o formulário à esquerda para gerar a impressão clínica inteligente.</p>
    </div>
  );
  ...
  return (
    <div className="h-full flex flex-col gap-6 p-6 bg-white/20 dark:bg-slate-900/20 backdrop-blur-3xl border-l border-white/10 dark:border-slate-800 rounded-l-3xl shadow-2xl overflow-y-auto custom-scrollbar">
      ...
      <div className="mt-auto pt-6 border-t border-white/10 dark:border-slate-800">
        <div className="flex flex-col gap-3">
          <button className="w-full py-4 bg-white/5 dark:bg-slate-800/50 hover:bg-white/10 border border-white/10 dark:border-slate-700 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all">
