...
export const HemogasometryAnalyzer: React.FC<Props> = ({ inputs, setInputs, onSubmit, onReset }) => {
  ...
  const handleLoadDefaults = (species: Species) => {
    const ref = HEMO_REF[species];
    const type = inputs.declaredSampleType as 'arterial' | 'venous';
    setInputs({
      species,
      declaredSampleType: type,
      fio2: 21,
      ph: 7.40,
      ...
      glucose: inputs.glucose || 100,
      lactate: inputs.lactate || 1.5,
      be: 0
    });
  };

  const handleSpeciesChange = (species: Species) => {
    handleLoadDefaults(species);
  };
