...
const CATEGORY_LABELS: Record<string, { icon: string; theme: string }> = {
  ...
  Endocrinologia: { icon: '⚖️', theme: 'endocrinologia' },
  ...
  Infectologia: { icon: '☣️', theme: 'infectologia' },
...
export function ReceitasTab({ templates, favorites, onToggleFavorite, onSelectTemplate, onDeleteTemplates }: ReceitasTabProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2.5 rounded-xl border border-border/80 bg-card/60 p-3 shadow-xs backdrop-blur-md lg:flex-row">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-border/80 bg-background py-1.5 pl-8 pr-3 text-xs text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
          />
        </div>

        <div className="flex h-9 items-center rounded-lg border border-border/80 bg-muted/50 p-0.5">
          {([
            ...
            <button
              ...
              className={`h-7 rounded-md px-2.5 text-xs font-medium transition-colors ${selectedSpecies === option.value ? 'bg-background font-semibold text-foreground shadow-xs' : 'text-muted-foreground hover:text-foreground'}`}
            >
              {option.label}
            </button>
          ))}
        ...
        <button
          ...
          className={`inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border px-3 text-xs font-medium transition-colors ${showOnlyFavorites ? 'border-amber-300 bg-amber-50 text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200' : 'border-border/80 bg-background text-muted-foreground hover:bg-muted'}`}
        >
          <Star className={`h-3.5 w-3.5 ${showOnlyFavorites ? 'fill-current' : ''}`} /> Favoritos ({favoriteRecipeCount})
        </button>
      </div>

      <div className="flex flex-col gap-4 xl:flex-row xl:items-start">
        <aside className="w-full shrink-0 rounded-xl border border-border/80 bg-background/50 p-3 backdrop-blur-xs xl:w-56">
          <div className="mb-2.5 flex items-center justify-between">
            <h2 className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Especialidades</h2>
          </div>
          <nav className="flex flex-row flex-wrap gap-1.5 xl:flex-col xl:gap-1" aria-label="Filtrar receitas por especialidade">
            <button
              type="button"
              onClick={() => setSelectedCategory('all')}
              className={cn('flex items-center gap-2 rounded-lg border border-border/60 px-2.5 py-1.5 text-left text-xs font-semibold transition-all duration-200 xl:w-full', selectedCategory === 'all' ? 'border-primary bg-primary/[0.06] text-primary shadow-xs' : 'bg-card/50 text-foreground/80 hover:border-border-hover hover:bg-card')}
            >
              <LayoutGrid className="h-3.5 w-3.5 shrink-0" />
              <span className="flex-1">Todas</span>
              <span className="rounded-full bg-muted px-1.5 py-0.5 text-[9px] font-bold text-muted-foreground">{recipesMatchingFilters.length}</span>
            </button>
        ...
        <main className="min-w-0 flex-1">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-border/60 pb-2">
            <h2 className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-muted-foreground"><Stethoscope className="h-3.5 w-3.5" />Modelos de receita</h2>
            <div className="flex flex-wrap items-center justify-end gap-2">
              <span className="rounded-full border border-border bg-card px-2.5 py-0.5 text-[11px] font-bold text-foreground">{filteredRecipes.length} {filteredRecipes.length === 1 ? 'receita' : 'receitas'}</span>
              <button
                type="button"
                onClick={() => editing ? closeEditing() : setEditing(true)}
                className="inline-flex h-8 cursor-pointer items-center justify-center gap-1.5 rounded-lg border border-border bg-background px-2.5 text-xs font-semibold text-foreground transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                title={personalRecipeCount ? 'Selecionar modelos próprios para excluir' : 'Ver quais modelos podem ser excluídos'}
              >
                {editing ? <X className="h-3.5 w-3.5" /> : <ListChecks className="h-3.5 w-3.5" />}
                {editing ? 'Sair da edição' : 'Editar modelos'}
              </button>
            </div>
          </div>
          {editing ? (
            <section className="sticky top-2 z-20 mb-3 flex flex-col gap-2 rounded-xl border border-sky-300 bg-sky-50/95 p-2.5 shadow-md backdrop-blur dark:border-sky-900 dark:bg-sky-950/95 sm:flex-row sm:items-center sm:justify-between" aria-label="Ações dos modelos selecionados">
              <div>
                <p className="text-xs font-bold text-sky-950 dark:text-sky-100">{selectedIds.length} {selectedIds.length === 1 ? 'modelo selecionado' : 'modelos selecionados'}</p>
                <p className="text-[11px] text-sky-800 dark:text-sky-300">Somente modelos próprios podem ser excluídos. Modelos globais ficam protegidos.</p>
              </div>
              <div className="grid grid-cols-2 gap-2 sm:flex">
                <button type="button" onClick={toggleAllVisible} disabled={!visiblePersonalIds.length} className="h-8 rounded-lg border border-sky-300 bg-background px-2.5 text-xs font-semibold disabled:opacity-50">
                  {visiblePersonalIds.length > 0 && visiblePersonalIds.every((id) => selectedIds.includes(id)) ? 'Desmarcar visíveis' : 'Selecionar visíveis'}
                </button>
                <button type="button" onClick={() => setConfirmDeleteOpen(true)} disabled={!selectedIds.length} className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg bg-rose-600 px-2.5 text-xs font-bold text-white transition-colors hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-45">
          ...
          {filteredRecipes.length ? (
            ...
            <div className="rounded-xl border border-border bg-card py-12 text-center text-xs font-medium text-muted-foreground">Nenhuma receita encontrada com os filtros atuais.</div>
          )}
        </main>
      </div>
...
function RecipeCard({
  ...
  return (
    <article className={cn('group relative flex h-full flex-col rounded-xl border bg-card p-3.5 transition-all duration-200', selected ? 'border-sky-500 ring-2 ring-sky-500/25' : 'border-border/80', !editing && theme.borderHover, !editing && theme.glow)} style={{ background: `linear-gradient(135deg, var(--card) 0%, ${theme.glowBg || 'var(--card)'} 100%)` }}>
      <div className="mb-2.5 flex items-start justify-between gap-2.5">
        ...
        {editing ? template.is_global ? (
          <span className="inline-flex h-8 shrink-0 items-center gap-1 rounded-md border border-border/60 bg-muted/50 px-2 text-[9px] font-semibold text-muted-foreground" title="Modelos globais são protegidos"><LockKeyhole className="h-3 w-3" />Protegido</span>
        ) : (
          <button type="button" onClick={() => onToggleSelected(template.id)} aria-pressed={selected} className={cn('flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-lg border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-600', selected ? 'border-sky-600 bg-sky-600 text-white' : 'border-border bg-background text-muted-foreground hover:border-sky-500 hover:text-sky-700')} aria-label={selected ? `Desmarcar ${template.title}` : `Selecionar ${template.title}`}>
            {selected ? <Check className="h-4 w-4" /> : <span className="h-3.5 w-3.5 rounded border border-current" />}
          </button>
        ) : (
          <button type="button" onClick={() => onToggleFavorite(template.id)} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-border/60 bg-background/50 text-muted-foreground transition-colors hover:bg-background hover:text-amber-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" title={isFavorite ? 'Remover dos favoritos' : 'Favoritar'} aria-label={isFavorite ? `Remover ${template.title} dos favoritos` : `Favoritar ${template.title}`}>
      ...
      {medications.length ? (
        <div className="mb-2.5">
          <p className="mb-1 flex items-center gap-1 text-[10px] font-semibold text-muted-foreground"><Pill className="h-3 w-3" />Medicamentos do modelo</p>
          <div className="flex flex-wrap gap-1">
            {medications.slice(0, 4).map((medication) => medication.slug ? (
              <Link key={medication.name} to={`/consulta-vet/medicamentos/${medication.slug}`} className="relative z-10 rounded border border-border/60 bg-muted/35 px-1.5 py-0.5 text-[10px] font-medium text-primary transition-colors hover:bg-primary/10 hover:underline">{medication.name}</Link>
            ) : (
              <span key={medication.name} className="rounded border border-border/50 bg-muted/30 px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">{medication.name}</span>
            ))}
            {medications.length > 4 ? <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">+{medications.length - 4}</span> : null}
          </div>
        </div>
      ) : <p className="mb-2.5 text-xs leading-relaxed text-muted-foreground">Abra o modelo para revisar e incluir os medicamentos necessários.</p>}

      <div className={cn('mt-auto flex items-center justify-between border-t pt-2.5', theme.line)}>
        <span className="text-[10px] font-medium text-muted-foreground/75">{editing ? template.is_global ? 'Modelo global protegido' : selected ? 'Selecionado para exclusão' : 'Modelo próprio' : 'Revise antes de emitir'}</span>
        {editing ? !template.is_global ? <button type="button" onClick={() => onToggleSelected(template.id)} className="relative z-10 h-7 rounded px-1.5 text-[11px] font-bold uppercase tracking-wider text-sky-700 transition-colors hover:bg-sky-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-600 dark:text-sky-300 dark:hover:bg-sky-950">{selected ? 'Desmarcar' : 'Selecionar'}</button> : null : <button type="button" onClick={() => onOpen(template)} className="relative z-10 h-7 rounded px-1.5 text-[11px] font-bold uppercase tracking-wider text-primary transition-colors hover:bg-primary/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">Usar modelo</button>}
      </div>
    </article>
  );
}
