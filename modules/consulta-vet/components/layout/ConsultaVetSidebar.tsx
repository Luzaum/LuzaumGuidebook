      ...
      >
        <div className="flex items-center justify-between p-6">
          <div>
            ...
            <p className="mt-1 text-xs font-medium uppercase tracking-wider text-muted-foreground">Consulta VET</p>
          </div>

          <button onClick={onClose} className="flex h-11 w-11 items-center justify-center rounded-md p-2 text-muted-foreground hover:bg-muted md:hidden">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-4">
          {links.map((link) => {
            const isActive = location.pathname === link.to || (link.to !== '/consulta-vet' && location.pathname.startsWith(link.to));
