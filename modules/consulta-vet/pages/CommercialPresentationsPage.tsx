...
const SUBCLASS_LABELS: Record<CommercialMedicationSubclass, string> = {
  ...
  gi_antiflatulent: 'Antiflatulentos / antigases',
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            {relatedMedications.map((medication) => (
              <Link
                ...
                className="inline-flex items-center gap-1.5 rounded-full border border-amber-500/30 bg-amber-500/[0.08] px-2.5 py-1 text-xs font-semibold text-amber-900 transition-colors hover:bg-amber-500/[0.14] dark:text-amber-200"
              >
                Ver monografia: {medication.title}
              </Link>
            ))}
          </div>
        </div>
