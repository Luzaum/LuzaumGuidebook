    ...
    if (!tutor && !patient) {
        return (
            <div className="rounded-xl border border-dashed border-slate-300 p-6 text-center">
                <p className="text-sm text-slate-400">Nenhum tutor ou paciente selecionado.</p>
                <Link
...
=======
            ...
            const saved = await savePrescription(payload)
            updateState((prev) => ({ ...prev, supabaseId: saved.id }))
            if (!silent) pushToast('✅ Receita salva no sistema!')
            return saved.id
        } catch (err) {
...
>>>>>>> Stashed changes
    ...
    const topbar = (
        <div className={`sticky top-0 z-30 border-b px-4 py-3 print:hidden backdrop-blur-md ${isDark ? 'border-[#274b20] bg-[#0a1507]/90' : 'border-slate-200 bg-white/90'}`}>
...
>>>>>>> Stashed changes
    ...
    if (isReviewMode) {
        return (
            <ReceituarioChrome section="nova" title="Revisar Receita" forcedTheme={rxTheme} onThemeChange={setRxTheme}>
                <div className={`min-h-screen transition-colors duration-200 ${isDark ? 'bg-[#0a0f0a] text-slate-100' : 'bg-[#f4f4f4] text-slate-900'}`}>
                    {topbar}
