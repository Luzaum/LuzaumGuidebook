...
function SectionCard({
  ...
  const titleColor =
    accent === 'primary' ? 'var(--primary)' : accent === 'chart5' ? 'var(--chart-5)' : 'var(--foreground)'
  ...
  return (
    ...
    >
      <h3 className="border-b pb-2 text-base font-bold leading-tight" style={{ borderColor: 'var(--border)', color: titleColor }}>
        {title}
      </h3>
...
export function AntibioticV2Panel({ sheet }: AntibioticV2PanelProps) {
  ...
  return (
    ...
    >
      <header className="rounded-[var(--radius)] border p-4" style={{ borderColor: 'var(--border)', background: 'var(--card)' }}>
        ...
        <p className="mt-2 text-xs leading-relaxed" style={{ color: 'var(--muted-foreground)' }}>
          <span className="font-medium text-[var(--foreground)]">{sheet.classLabel}</span>
          {' · '}
          {sheet.subclassLabel}
        </p>
        {concordance && (
      ...
      <SectionCard title="Uso neste módulo (v2)" accent="foreground">
        <ul className="ml-4 list-disc space-y-2 text-sm leading-relaxed" style={{ color: 'var(--muted-foreground)' }}>
          {sheet.usesInApp.map((u, i) => (
            <li key={i} className="pl-1 marker:text-[var(--primary)]">
              {u}
            </li>
          ))}
        </ul>
      ...
      >
        <h3 className="border-b pb-2 text-base font-bold" style={{ borderColor: 'var(--border)', color: 'var(--foreground)' }}>
          Stewardship
        </h3>
        <ul className="ml-4 mt-3 list-disc space-y-2 text-sm leading-relaxed" style={{ color: 'var(--muted-foreground)' }}>
          {sheet.stewardshipNotes.map((c, i) => (
            <li key={i} className="pl-1 marker:text-[var(--accent)]">
              {c}
            </li>
          ))}
        </ul>
      ...
      {sheet.synonyms.length > 0 && (
        ...
        >
          <span className="font-semibold text-[var(--foreground)]">Sinônimos controlados: </span>
          {sheet.synonyms.join(' · ')}
        </div>
      )}
      ...
      <details className="rounded-[var(--radius)] border text-xs" style={{ borderColor: 'var(--border)' }}>
        <summary className="cursor-pointer px-3 py-2 font-semibold" style={{ color: 'var(--muted-foreground)' }}>
          Referências técnicas (equipe)
        </summary>
        <ul className="space-y-1 border-t px-3 py-2 leading-relaxed" style={{ borderColor: 'var(--border)', color: 'var(--muted-foreground)' }}>
