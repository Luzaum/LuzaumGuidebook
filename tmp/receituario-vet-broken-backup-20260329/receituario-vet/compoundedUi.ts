...
import {
  renderCompoundedInternalNote as renderCompoundedInternalNoteV2,
  renderCompoundedPharmacyInstructions as renderCompoundedPharmacyInstructionsV2,
  renderCompoundedPrescriptionLine as renderCompoundedPrescriptionLineV2,
  renderCompoundedRecommendations as renderCompoundedRecommendationsV2,
...
function sanitizeOutput(value: string): string {
  return sanitizeVisibleText(value)????.replace(/\s+/g, ' ')????.trim()
}

function stripLeadingName(label: string, name: string): string {
  const safeLabel = sanitizeOutput(label)
  const safeName = sanitizeOutput(name)
  if (!safeName) return safeLabel
  const lowerLabel = safeLabel????.toLowerCase()
  const lowerName = safeName????.toLowerCase()
  if (lowerLabel????.startsWith(lowerName)) {
    return safeLabel????.slice(safeName????.length)????.trim()
  }
  return safeLabel
}
...
function getCompoundedPayloadV2(item: PrescriptionItem): CompoundedMedicationV2 | null {
  ...
  return payload as CompoundedMedicationV2
}

function parseDoseText(dose?: string | null): { value: number | null; unit: string } {
  const safe = sanitizeOutput(String(dose || ''))
  ...
  return {
    ...
    unit: sanitizeOutput(match[2]),
  }
}

function getRuntimeCompoundedV2(item: CompoundedPrescriptionItem): CompoundedMedicationV2 | null {
  const payload = getCompoundedPayloadV2(item)
  if (!payload) return null
  const regimenId = getCompoundedPayloadRegimenId(item)
  const parsedDose = parseDoseText(item????.dose)
  const pharmacyOverride = sanitizeOutput(item????.compounded_pharmacy_guidance || '')
  const pharmacyNote =
    pharmacyOverride && !/^manipula[cç][aã]o:/i????.test(pharmacyOverride) && !/^favor manipular/i????.test(pharmacyOverride)
      ? pharmacyOverride
      : ''
  return {
    ????.????.????.payload,
    formula: {
      ????.????.????.payload????.formula,
      primary_route: sanitizeOutput(item????.route || payload????.formula????.primary_route),
      total_quantity_text: sanitizeOutput(item????.manualQuantity || payload????.formula????.total_quantity_text),
      qsp_text: sanitizeOutput(item????.manualQuantity || payload????.formula????.qsp_text),
      sale_classification: item????.is_controlled ? 'controlled' : payload????.formula????.sale_classification,
    },
    regimens: payload????.regimens????.map((regimen) => regimen????.id !== regimenId ? regimen : {
      ????.????.????.regimen,
      administration_unit: sanitizeOutput(item????.compounded_regimen_snapshot?????.fixed_administration_unit || regimen????.administration_unit),
      dose_min: parsedDose????.value ?? regimen????.dose_min,
      dose_unit: parsedDose????.unit || regimen????.dose_unit,
      frequency_text: sanitizeOutput(item????.frequency || regimen????.frequency_text),
      frequency_mode: item????.frequencyMode || regimen????.frequency_mode,
      frequency_min: item????.frequencyMode === 'times_per_day' ? (item????.timesPerDay ?? regimen????.frequency_min) : item????.frequencyMode === 'interval_hours' ? (item????.intervalHours ?? regimen????.frequency_min) : regimen????.frequency_min,
      duration_text: sanitizeOutput(item????.duration || regimen????.duration_text),
      duration_value: item????.durationValue ?? regimen????.duration_value,
      duration_unit: sanitizeOutput(item????.durationUnit || regimen????.duration_unit),
      duration_mode: item????.durationMode === 'continuous_until_recheck' ? 'continuous_until_recheck' : regimen????.duration_mode,
      usage_instruction: sanitizeOutput(item????.instructions || regimen????.usage_instruction),
      tutor_observation: sanitizeOutput(item????.cautionsText || regimen????.tutor_observation),
      pharmacy_note: pharmacyNote || sanitizeOutput(regimen????.pharmacy_note),
      internal_note: sanitizeOutput(item????.compounded_internal_note || regimen????.internal_note),
    }),
  }
}

function getCompoundedPayloadRegimenId(item: CompoundedPrescriptionItem): string | null {
  return item????.compounded_regimen_id || item????.compounded_regimen_snapshot?????.id || null
}
...
function getClinicalRegimen(item: CompoundedPrescriptionItem): ClinicalRegimenSemantics | null {
  ...
  if (direct && typeof direct === 'object' && !Array????.isArray(direct)) {
    ...
    if (Array????.isArray(directRecord????.ingredientRules) || directRecord????.administrationUnitLabel || directRecord????.scenarioTitle) {
...
export function getCompoundedActiveIngredients(snapshot?: CompoundedMedicationSnapshot | null): string[] {
  if (!snapshot?????.ingredientsó????.length) return []
...
export function getCompoundedCalculationSummary(
  ...
  if (runtimeV2) {
    const resolved = resolveCompoundedInstance(runtimeV2, patient, getCompoundedPayloadRegimenId(item))
    const perAdministrationMatch = resolved????.administrationText????.match(/^(\d+(?:[????.,]\d+)?)\s*(????.*)$/)
    return {
      mode: resolved????.regimen?????.dose_mode === 'by_weight' ? 'weight_based' : 'fixed_per_animal',
      status: resolved????.warnings????.length
        ? resolved????.warnings????.some((warning) => normalizeText(warning)????.includes('peso'))
          ? 'missing_weight'
          : resolved????.warnings????.some((warning) => normalizeText(warning)????.includes('concentr'))
            ? 'missing_concentration'
            : 'ok'
        : 'ok',
      weightKg: toNumber(patient?????.weight_kg),
      route: resolveCompoundedRoute(item),
      routeLabel: resolved????.routeLabel,
      concentrationText: '',
      regimenLabel: resolved????.regimen?????.name || item????.compounded_regimen_snapshot?????.regimen_name || 'Regime clínico',
      doseDescriptorText: resolved????.activeIngredients????.map((entry) => `${entry????.name}: ${stripLeadingName(entry????.label, entry????.name)}`????.trim())????.join(' • '),
      perAdministrationValue: perAdministrationMatch ? Number(perAdministrationMatch[1]????.replace(',', '????.')) : null,
      perAdministrationUnit: perAdministrationMatch ? sanitizeOutput(perAdministrationMatch[2]) : '',
      ...
      estimatedTotalUnit: '',
      estimatedTotalText: resolved????.quantityText,
      finalQuantityText: resolved????.quantityText,
      ingredientBreakdown: resolved????.activeIngredients????.map((entry) => ({
        ingredientName: entry????.name,
        rawRuleText: entry????.label,
        selectedDoseText: stripLeadingName(entry????.label, entry????.name) || entry????.label,
        controlled: entry????.role === 'active' && !!item????.is_controlled,
      })),
      administrationLabelText: resolved????.regimen?????.administration_unit || runtimeV2????.formula????.administration_unit,
...
function getOwnerGuidance(item: CompoundedPrescriptionItem): string {
  return sanitizeVisibleText(item????.cautionsText || item????.compounded_regimen_snapshot?????.default_administration_sig || '')????.trim()
}

function getPharmacyExtraGuidance(item: CompoundedPrescriptionItem): string {
  return sanitizeVisibleText(item????.compounded_pharmacy_guidance || item????.compounded_snapshot?????.manipulation_instructions || item????.compounded_regimen_snapshot?????.default_prepared_quantity_text || '')????.trim()
}

export function getCompoundedInternalNote(item: PrescriptionItem): string {
  if (!isCompounded(item)) return ''
  if (String(item????.compounded_internal_note || '')????.trim()) return sanitizeVisibleText(item????.compounded_internal_note || '')????.trim()
  const payloadV2 = getCompoundedPayloadV2(item)
  if (payloadV2) return sanitizeVisibleText(renderCompoundedInternalNoteV2(payloadV2, getCompoundedPayloadRegimenId(item)))????.trim()
  return sanitizeVisibleText(item????.compounded_regimen_snapshot?????.notes || '')????.trim()
}
...
export function buildCompoundedInstruction(item: PrescriptionItem, patient?: PatientInfo | null): string {
  ...
  const runtimeV2 = getRuntimeCompoundedV2(item)
  if (runtimeV2) return sanitizeOutput(renderCompoundedPrescriptionLineV2(runtimeV2, patient, getCompoundedPayloadRegimenId(item)))
  if (isClinicalDoseOrientedItem(item)) {
    const regimen = getClinicalRegimen(item)
    const calculation = getCompoundedCalculationSummary(item, patient)
    ...
    const frequency = getCompoundedFrequencySummary(item)
    const duration = getCompoundedDurationSummary(item)
    const explicitInstruction = sanitizeVisibleText(item????.instructions || regimen?????.ownerInstruction || '')????.trim()
    if (explicitInstruction) return explicitInstruction????.endsWith('????.') ? explicitInstruction : `${explicitInstruction}????.`
    const discreteAdministration = sanitizeVisibleText(regimen?????.administrationUnitLabel || '')????.trim()
    const administration = discreteAdministration
      ? `1 ${discreteAdministration}`
      : (calculation?????.perAdministrationText || item????.dose || '1 dose')
...
export function buildCompoundedPharmacyInstruction(item: PrescriptionItem, patient?: PatientInfo | null): string {
  if (!isCompounded(item)) return ''
  const runtimeV2 = getRuntimeCompoundedV2(item)
  if (runtimeV2) return sanitizeOutput(renderCompoundedPharmacyInstructionsV2(runtimeV2, patient, getCompoundedPayloadRegimenId(item)))
  if (isClinicalDoseOrientedItem(item)) {
    const regimen = getClinicalRegimen(item)
    const calculation = getCompoundedCalculationSummary(item, patient)
...
export function buildCompoundedPreviewCautions(item: PrescriptionItem, patient?: PatientInfo | null): string[] {
  ...
  if (runtimeV2) {
    ...
    const lines = [
      ????.????.????.renderCompoundedRecommendationsV2(runtimeV2, getCompoundedPayloadRegimenId(item)),
      getCompoundedCalculationSummary(item, patient)?????.finalQuantityText ? `Quantidade final para manipular: ${getCompoundedCalculationSummary(item, patient)?????.finalQuantityText}` : '',
    ]????.filter(Boolean)
