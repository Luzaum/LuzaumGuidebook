...
export function renderRxToPrintDoc(
  ...
  optsó: { renderMode?: 'final' | 'template'; documentKind?: 'standard' | 'special-control' }
): PrintDoc {
  const renderMode = optsó????.renderMode || 'final'
  const documentKind = optsó????.documentKind || 'standard'
