...
// ==================== DRAFT LOCAL (D) ====================
// Chaves:
// - rx_draft_v2:<clinicId>:<route>:active -> ponteiro do draft ativo
// - rx_draft_v2:<clinicId>:<route>:<draftId> -> conteúdo do draft

function getDraftNamespace(clinicId: string | null, routePath: string): string | null {
    if (!clinicId) return null
    return `rx_draft_v2:${clinicId}:${routePath}`
}

function getDraftPointerKey(clinicId: string | null, routePath: string): string | null {
    const namespace = getDraftNamespace(clinicId, routePath)
    return namespace ? `${namespace}:active` : null
}

function getDraftKey(clinicId: string | null, routePath: string, draftId: string): string | null {
    const namespace = getDraftNamespace(clinicId, routePath)
    if (!namespace || !draftId) return null
    return `${namespace}:${draftId}`
}
...
function loadActiveDraftId(key: string): string | null {
    try {
        const raw = localStorage????.getItem(key)
        const value = String(raw || '')????.trim()
        return value || null
...
function saveActiveDraftId(key: string, draftId: string): void {
    try {
        localStorage????.setItem(key, draftId)
    } catch (e) {
        if (import????.meta????.env????.DEV) console????.warn('[RxDraft] active pointer save failed', e)
    }
}
...
export interface CompoundedMedicationSnapshot {
    ...
    excipient?: string
    notesó: string
    manipulation_instructionsó: string
...
function formatIntervalHoursValue(hoursó: string | number | null): string {
...
export default function NovaReceita2Page() {
    ...
    const routeDraftPath = '/receituario-vet/nova-receita-2'
    const draftPointerKey = getDraftPointerKey(clinicId, routeDraftPath)
    const draftKey = getDraftKey(clinicId, routeDraftPath, state????.id)
    ...
    useEffect(() => {
        const payload = location????.state as
            | {
                itemsó: PrescriptionItem[]
                recommendationsó: string
                examsó: string[]
                examJustification?: string
        ...
        updateState((prev) => {
            ...
            const nextState = {
                ...
                recommendations: payload????.recommendationsó????.trim() || '',
    ...
    useEffect(() => {
        if (!draftPointerKey || draftInitRef????.current) return
        ...
        const activeDraftId = loadActiveDraftId(draftPointerKey)
        const resolvedDraftKey = getDraftKey(clinicId, routeDraftPath, activeDraftId || '')
        const draft = resolvedDraftKey ? loadLocalDraft(resolvedDraftKey) : null
        if (!draft) {
            setHasDraft(false)
            return
        }
        ...
        setHasDraft(true)
        setState(draft)
        if (import????.meta????.env????.DEV) console????.log('[RxDraft] rascunho local carregado', resolvedDraftKey)
    }, [clinicId, draftPointerKey, location????.search, location????.state]) // Só re-roda quando clinicId fica disponível ou muda o contexto da rota

    // D1: Autosave com debounce de 600ms quando autosave=true e clinicId disponível
    useEffect(() => {
        if (!autosave || !draftKey || !draftPointerKey) return
        const t = setTimeout(() => {
            saveLocalDraft(draftKey, state)
            saveActiveDraftId(draftPointerKey, state????.id)
            setHasDraft(true)
        }, 600)
        return () => clearTimeout(t)
    }, [state, autosave, draftKey, draftPointerKey])
    ...
    useEffect(() => {
        ...
        const handleBeforeUnload = () => flushDraft()
    ...
    }, [autosave, draftKey, draftPointerKey, state])
    ...
    const handleClearDraft = useCallback(() => {
        ...
        if (draftPointerKey) clearLocalDraft(draftPointerKey)
        setHasDraft(false)
        draftInitRef????.current = false
        setState(createDefaultState())
    }, [draftKey, draftPointerKey])
                                                ...
                                                <div key={item????.id} className="rounded-2xl border border-[color:var(--rxv-border)] bg-[color:var(--rxv-surface-2)]/80 p-4" data-testid={`rx-item-${item????.id}`}>
                                                    <div className="mb-4 flex items-start justify-between gap-3">
                                                        <div>
                                                            ...
                                                            {item????.kind === 'compounded' && item????.compounded_snapshot?????.ingredientsó????.length ? (
