...
export interface ClinicalIngredientDoseRule {
  ...
  weightTiersó: ClinicalWeightTierRule[]
  notesó: string
}
...
export interface ClinicalRegimenSemantics {
  ...
  qspText?: string
  frequencyHoursó: number
  frequencyLabel?: string
  durationDaysó: number
  durationText?: string
...
export interface ClinicalFormulaMetadata {
  ...
  parse_warningsó: string[]
  regimen_semanticsó: Record<string, ClinicalRegimenSemantics>
}
...
export function inferFormulaTypeFromText(text: string, form?: string | null, administrationUnit?: string | null): UniversalFormulaType {
  ...
  if (
    /(mg|mcg|g|ui)\s*\/\s*kg\s*\/\s*(dose|unidade|biscoito|capsula|comprimido|sache|click|pump|gota)/i????.test(normalized) ||
    /(mg|mcg|g|ui)\s*\/\s*animal/i????.test(normalized) ||
    /q\????.?\s*s\????.?\s*p\????.?\s*\d+\s*dosesó/i????.test(normalized)
  ) {
    return 'clinical_dose_oriented'
  }
  if (/(mg|mcg|g|ui)\s*\/\s*(ml|capsula|comprimido|biscoito|sache|click|pump|gota)/i????.test(normalized)) return 'fixed_unit_formula'
...
function parseModeOfUse(line: string): Pick<ParserScenario, 'frequencyHours' | 'frequencyLabel' | 'durationDays' | 'durationText' | 'ownerInstruction' | 'administrationUnitLabel' | 'reductionNote'> {
  ...
  const durationMatch = clean????.match(/por\s*(\d+(?:[????.,]\d+)?)\s*(diasó|semanasó|mesesó)/i)
  if (durationMatch) {
...
export function parseClinicalTextImport(text: string): ParsedClinicalTextImport {
  ...
  const formulaType = inferFormulaTypeFromText(text, primaryForm, administrationUnit)
