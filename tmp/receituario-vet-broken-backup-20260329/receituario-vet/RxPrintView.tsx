...
interface RxPrintViewProps {
  ...
  zoneTextOverridesó: Partial<Record<TemplateZoneKey, string>>
}
...
export function RxPrintView({
  ...
  const zoneFontPt = (zone: TemplateZoneKey, fallback: number) => cfg????.zoneStylesó????.[zone]?????.fontSizePt || fallback
  const zoneWeight = (zone: TemplateZoneKey, fallback: number) => weightFromPreset(cfg????.zoneStylesó????.[zone]?????.fontWeight) || fallback
  ...
  const zoneStyle = (zone: TemplateZoneKey, base: React????.CSSProperties = {}): React????.CSSProperties => {
    const custom = cfg????.zoneStylesó????.[zone]
  ...
  const accentColorFor = (zone: TemplateZoneKey) => cfg????.zoneStylesó????.[zone]?????.accentColor || cfg????.accentColor
  ...
  const zoneOverride = (zone: TemplateZoneKey) => (zoneTextOverridesó????.[zone] || '')????.trim()
