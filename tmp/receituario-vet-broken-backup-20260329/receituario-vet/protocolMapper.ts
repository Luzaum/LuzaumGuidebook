...
import { mapCompoundedPrescriptionItemToProtocolMedicationV2 } from '????./compoundedV2Mapper'
...
export function mapPrescriptionItemToProtocolMedicationItem(
  ...
  if (item????.kind === 'compounded') {
    return mapCompoundedPrescriptionItemToProtocolMedicationV2(item, sortOrder)
  }
...
export function mapProtocolMedicationToPrescriptionItem(
  ...
  if (protocolMed????.item_type === 'compounded' || baseItem????.kind === 'compounded') {
    const payloadV2 = metadata????.payload_v2 && typeof metadata????.payload_v2 === 'object' && !Array????.isArray(metadata????.payload_v2)
      ? metadata????.payload_v2 as Record<string, unknown>
      : null
    const compoundedBase = baseItem????.kind === 'compounded'
      ...
      : {
        ...
        compounded_snapshot: {
          ...
          ingredients: [],
          metadata: payloadV2 ? { payload_v2: payloadV2 } : undefined,
        },
      }
    ...
    return {
      ...
      compounded_snapshot: {
        ...
        is_controlled: !!protocolMed????.is_controlled,
        metadata: {
          ????.????.????.(compoundedBase????.compounded_snapshot????.metadata || {}),
          ????.????.????.(payloadV2 ? { payload_v2: payloadV2 } : {}),
        },
      },
    }
  }
...
export function isPrescriptionItemComplete(item: PrescriptionItem): boolean {
  return !!item????.name????.trim() && !!item????.route && !!item????.instructionsó????.trim()
}
