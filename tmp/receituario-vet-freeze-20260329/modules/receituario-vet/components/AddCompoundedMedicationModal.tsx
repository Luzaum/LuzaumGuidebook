...
import { ManipuladosV2CatalogCard } from './ManipuladosV2CatalogCard'
...
export function AddCompoundedMedicationModal({
  ...
  const selectedRegimen = useMemo(() => selectedBundle?.regimens.find((entry) => entry.id === selectedRegimenId) || selectedBundle?.regimens[0] || null, [selectedBundle, selectedRegimenId])
  ...
  const previewV2 = useMemo(() => (selectedBundle ? legacyManipulatedToV2(selectedBundle) : null), [selectedBundle])
      ...
      <div className="mx-auto flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl border border-[#39ff14]/30 bg-[#0a0f0a] text-slate-100 shadow-[0_0_70px_rgba(57,255,20,0.16)]">
        <div className="flex items-center justify-between border-b border-slate-800 bg-black/60 px-6 py-4">
          <div>
            ...
            <p className="mt-1 text-[11px] font-semibold uppercase tracking-widest text-slate-500">Catálogo magistral da clínica com fluxo V2</p>
          </div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[320px_minmax(0,1fr)]">
          <div className="border-r border-slate-800 bg-[#0d140d] p-5">
            <RxvField label="Buscar manipulado">
              <RxvInput value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Nome da fórmula, ativo ou forma..." data-testid="compounded-modal-search" />
            </RxvField>
              ...
              {isLoading ? (
                ...
                results.map((bundle) => (
                  <ManipuladosV2CatalogCard
                    ...
                    item={legacyManipulatedToV2(bundle)}
                    active={selectedId === bundle.medication.id}
                    onClick={() => {
          ...
          <div className="min-h-0 overflow-y-auto p-6">
            {!selectedBundle || !selectedRegimen || !previewItem || !previewV2 ? (
