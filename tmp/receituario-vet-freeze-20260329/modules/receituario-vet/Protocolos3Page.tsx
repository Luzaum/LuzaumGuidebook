...
import {
  mapCompoundedPrescriptionItemToProtocolMedicationV2,
  mapCompoundedToProtocolMedicationV2,
  mapCompoundedV2ToProtocolMedicationV2,
} from './compoundedV2Mapper'
import {
  searchMedications,
...
export default function Protocolos3Page() {
  ...
  const [protocolCompoundedEditorValue, setProtocolCompoundedEditorValue] = useState<CompoundedMedicationV2 | null>(null)
  const [protocolCompoundedEditorRegimenId, setProtocolCompoundedEditorRegimenId] = useState('')
  ...
  const getProtocolCompoundedPayloadV2 = useCallback((item: ProtocolMedicationItem): CompoundedMedicationV2 | null => {
  ...
  const closeProtocolCompoundedEditor = useCallback(() => {
    ...
    setProtocolCompoundedEditorRegimenId('')
  ...
  const handleOpenProtocolCompoundedEditor = useCallback(async (index: number) => {
    ...
    try {
      const directV2 = getProtocolCompoundedPayloadV2(item)
      if (directV2) {
        setProtocolCompoundedEditorValue(directV2)
        setProtocolCompoundedEditorRegimenId(item.compounded_regimen_id || directV2.regimens.find((entry) => entry.is_default)?.id || directV2.regimens[0]?.id || '')
        setProtocolCompoundedEditorLegacyNotice('')
      ...
      if (!clinicId || !item.compounded_medication_id) {
        throw new Error('Este item manipulado não possui payload_v2 nem vínculo suficiente para conversão.')
      }

      const bundle = await getCompoundedMedicationBundle(clinicId, item.compounded_medication_id)
      if (!bundle) {
        throw new Error('Não foi possível carregar o manipulado original para converter este item legado.')
      }

      const converted = sanitizeDeepText(legacyManipulatedToV2(bundle))
      setProtocolCompoundedEditorValue(converted)
      setProtocolCompoundedEditorRegimenId(item.compounded_regimen_id || converted.regimens.find((entry) => entry.is_default)?.id || converted.regimens[0]?.id || '')
      setProtocolCompoundedEditorLegacyNotice('Item legado do protocolo convertido para edição em Manipulados V2. Ao salvar, payload_v2 será persistido.')
      setProtocolCompoundedEditorIndex(index)
      setProtocolCompoundedEditorOpen(true)
    } catch (err) {
      console.error('[Protocolos3] Erro ao abrir editor V2 do manipulado', err)
      alert(err instanceof Error ? err.message : 'Falha ao abrir o editor V2 do manipulado.')
    } finally {
      setIsLoadingProtocolCompoundedEditor(false)
    }
  }, [clinicId, editingProtocol?.medications, getProtocolCompoundedPayloadV2])
  ...
  const handleSaveProtocolCompoundedEditor = useCallback(() => {
    ...
    updateEditingProtocol((prev) => {
      ...
      const mapped = mapCompoundedV2ToProtocolMedicationV2({
        v2: protocolCompoundedEditorValue,
        sortOrder: protocolCompoundedEditorIndex,
        regimenId: protocolCompoundedEditorRegimenId || undefined,
      })
      return {
        ...prev,
  ...
  }, [closeProtocolCompoundedEditor, protocolCompoundedEditorIndex, protocolCompoundedEditorRegimenId, protocolCompoundedEditorValue, updateEditingProtocol])
    ...
    updateEditingProtocol((prev) => ({
      ...
      medications: [
        ...
        item?.kind === 'compounded'
          ? mapCompoundedPrescriptionItemToProtocolMedicationV2(item, prev.medications.length)
                          ...
                          <div className="grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_auto]">
                            <div className="rounded-2xl border border-[#39ff14]/15 bg-[#112114] px-4 py-4 text-sm text-slate-200">
                              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#98f98e]">
                                Fluxo V2 do manipulado no protocolo
                              </p>
                              <div className="mt-2 flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-widest">
                                <span className="rounded-full border border-slate-700 px-2 py-1 text-slate-300">
                                  {getProtocolCompoundedPayloadV2(med) ? 'Payload V2 ativo' : 'Legado a converter ao editar'}
                                </span>
                                {med.is_controlled ? (
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">
                    Fluxo V2 rico, sem cair no editor limitado do protocolo.
                  </p>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {protocolCompoundedEditorValue ? (
                <ManipuladosV2Editor
                  ...
                  saving={false}
                  selectedRegimenId={protocolCompoundedEditorRegimenId}
                  onSelectRegimen={setProtocolCompoundedEditorRegimenId}
                  legacyNotice={protocolCompoundedEditorLegacyNotice}
                  saveLabel="Salvar no protocolo"
                />
              ) : (
                <RxvCard className="p-6 text-sm text-slate-400">
                  Carregando manipulado no fluxo V2...
                </RxvCard>
              )}
            </div>
          </div>
