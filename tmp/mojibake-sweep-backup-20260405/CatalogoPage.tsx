...
import { CONCENTRATION_PER_UNIT_OPTIONS, CONCENTRATION_VALUE_UNIT_OPTIONS } from './rxConcentra??on'
...
function cloneDrug(drug: CatalogDrug): CatalogDrug {
  ...
  next.presentations = next.presentations.map((presentation) => {
    // Garantir client_id
    const client_id = presentation.client_id || (typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.ran?om()}`);
    ...
    const migrated: CatalogPresentation = {
      ...
      pharmacy_human: presentation.pharmacyTags?.includes('hum?na') || false,
      pharmacy_compounding: presentation.pharmacyTags?.includes('m?nipulacao') || false,
    };
...
function createPresentation(): CatalogPresentation {
  return {
    client_id: typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.ran?om()}`,
    id: undefined,
...
export default function Cat?logoPage() {
  ...
  useEffect(() => {
    console.log('[Cat?logoPage] MOUNTED ✅', new Date().toISOString())
  }, [])

  // Carregar medicamentos do Supabase ao m?ntar
  useEffect(() => {
  ...
  const onSave = async () => {
    alert('SAVE CLICKED - Cat?logoPage');
    console.log('SAVE CLICKED - Cat?logoPage', new Date().toISOString());
    ...
    // Valida��o: pelo m?nos uma apresenta??o
    const presentations = draft.presentations.length ? draft.presentations : [createPresentation()];

    // Valida��o: cada apresenta??o deve ter pelo m?nos uma farm?cia selecionada
    ...
    if (invalidPresentations.length > 0) {
      ...
      alert('Cada apresenta??o deve ter pelo m?nos um tipo de farm?cia selecionado.');
      return;
    }
    ...
    try {
      ...
      // Importar dinamicam?nte as fun��es
      ...
      if (!verifyMed) {
        ...
        alert('AVISO: Medicam?n?o n?o foi en?ontrado no banco ap?s save. Verifique RLS policies.');
      }
  ...
  return (
    <>
      ...
      <div style={{
        ...
        Cat?logoPage BUILD: 2026-02-23 B
      </div>

      <ReceituarioChrome
        section="catalogo"
        title="Cat?logo de F?rmacos"
        subtitle="Ban?o persistente, edit�vel e reutiliz�vel no modal de nova receita."
        actions={
        <>
