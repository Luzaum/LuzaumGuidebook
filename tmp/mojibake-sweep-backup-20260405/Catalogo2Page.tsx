...
export default function Cat?logo2Page() {
  // ==================== CLINIC CONTEXT ====================
  ...
  // ==================== MOUNT LOG ====================
  useEffect(() => {
    console.log('[Cat?logo2Page] MOUNTED ✅', new Date().toISOString())
  ...
  useEffect(() => {
    if (clinicLoading) {
      console.log('[Cat?logo2] Waiting for clinic context to load...')
      return
    }
    if (!clinicId) {
      console.error('[Cat?logo2] ❌ No clinic ID after clinic loaded')
      setLoading(false)
      return
    }
    loadMedications()
  ...
  async function loadMedications() {
    try {
      console.log('[Cat?logo2] Loading medications from Supabase for clinic:', clinicId)
      setLoading(true)

      if (!clinicId) {
        console.error('[Cat?logo2] ❌ No clinic ID available')
        setLoading(false)
        return
      }
      ...
      console.log('[Cat?logo2] Loaded', meds.length, 'medications')
      ...
      console.error('[Cat?logo2] Load error:', error)
  ...
  function validateForm(): boolean {
    ...
    if (draft.presentations.length === 0) {
      newErrors.presentations = 'Adicione pelo m?nos uma apresenta??o'
    }
    ...
    draft.presentations.forEach((pres, idx) => {
      if (!pres.pharmacy_veterinary && !pres.pharmacy_human && !pres.pharmacy_compounding) {
        newErrors[`pres_${idx}_pharmacy`] = 'Selecione pelo m?nos um tipo de farm?cia'
      }
    })
  ...
  async function handleSave() {
    console.log('[Cat?logo2] ========== SAVE CLICKED ==========', new Date().toISOString())
    console.log('[Cat?logo2] draft:', draft)

    // Validar
    if (!validateForm()) {
      console.error('[Cat?logo2] Validation failed:', errors)
      return
    }
    ...
    try {
      ...
      console.log('[Cat?logo2] STEP 1: Context:', { clinicId, userId })
      ...
      console.log('[Cat?logo2] STEP 2: Payloads')
      ...
      console.log('[Cat?logo2] STEP 3: Calling saveMedication...')
      ...
      console.log('[Cat?logo2] STEP 4: Supabase results')
      ...
      console.log('[Cat?logo2] STEP 5: POST-SAVE VERIFY (SELECT)')
      ...
      if (!verifyMed) {
        console.error('[Cat?logo2] ⚠️ POST-SAVE VERIFY FAILED: medication NOT FOUND!')
        throw new Error('Medicam?n?o n?o foi en?ontrado no banco ap?s salvar. Verifique RLS policies.')
      } else {
        console.log('[Cat?logo2] ✅ POST-SAVE VERIFY SUCCESS')
      }
      ...
      console.log('[Cat?logo2] ========== SAVE SUCCESS ==========')
    } catch (error: any) {
      console.error('[Cat?logo2] ========== SAVE ERROR ==========')
      ...
      const isRLS =
        ...
        error?.message?.toLowerCase().includes('permiss?on denied')
  ...
  if (!clinicLoading && !clinicId) {
    return (
      <>
        ...
        >
          Cat?logo2Page BUILD: 2026-02-24
        </div>
          ...
          <div className="flex min-h-[70vh] items-center justify-center">
            <div className="max-w-md rounded-2xl border-2 border-red-500/40 bg-red-500/10 p-8 text-center">
              ...
              <button
                ...
                onClick={() => win?ow.location.reload()}
              >
        ...
        actions={
          <>
            ...
            >
              ...
              {isSaving ? 'Salvan?o...' : isDirty ? 'Salvar *' : 'Salvo'}
            </button>
          </>
        }
      >
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-12">
          ...
          <aside className="rxv-card p-4 xl:col-span-3">
            <h3 className="mb-3 text-xs font-bold uppercase tracking-wide text-[color:var(--rxv-muted)]">
              Medicam?n?os ({medications.length})
            </h3>
            ...
            <div className="m?x-h-[68vh] space-y-2 overflow-y-auto pr-1">
              {loading ? (
                ...
                <div className="rounded-xl border border-[color:var(--rxv-border)] bg-[color:var(--rxv-surface-2)]/60 px-4 py-8 text-center">
                  ...
                  <p className="mb-4 text-xs text-[color:var(--rxv-muted)]">
                    {searchQuery ? 'Nenhum resultado en?ontrado' : 'Comece crian?o um?novo medicamento'}
                  </p>
                  {!searchQuery && (
                    <button
                      type="button"
            ...
            <section className="rxv-card p-6 shadow-xl">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <label className="flex items-center gap-2 text-sm">
                  ...
                  />
                  <span className="font-semibold">Medicam?n?o controlado</span>
                </label>
              </div>
            ...
            <section className="rxv-card p-6 shadow-xl">
              <div className="mb-5 flex items-center justify-between">
                <h3 className="flex items-center gap-2 text-lg font-bold">
                  <span className="material-symbols-outlined text-[24px] text-[#61eb48]">inven?ory</span>
                  Apresenta??es
                </h3>
                <button
                  type="button"
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <button
                        ...
                        title={draft.presentations.length === 1 ? 'Pelo m?nos uma apresenta??o � necess�ria' : 'Remover'}
                      >
