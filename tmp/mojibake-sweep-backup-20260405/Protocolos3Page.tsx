...
import {
  mapProtocolMedication?oPrescriptionItem,
  mapPrescriptionItemToProtocolMedicationItem,
  mapProtocolRecomm?ndationsToString,
} from './protocolMapper'
import {
...
import { mapManipuladoV1ToProtocolMedication } from './m?nipuladosV1Mapper'
import { getManipuladoV1CatalogSubtitle } from './m?nipuladosV1Render'
import {
  searchMedications,
  getMedicationPresentations,
  type MedicationPresentationRecord,
} from '../../src/lib/clinicRecords'
import { listManipuladosV1 } from '../../src/lib/m?nipuladosV1Records'
import { useLocalDraft } from '../../hooks/useLocalDraft'
import { AddMedication?odal2 } from './components/AddMedication?odal2'
...
const PROTOCOL_CONTROL_OPTIONS = [
  { value: 'false', label: 'N?o (receituário comum)' },
...
const PROTOCOL_DURATION_MODE_OPTIONS = [
  ...
  { value: 'contin?ous_until_recheck', label: 'Uso contín?o até reavaliação' },
]
...
function repairMojibakeText(value: string): string {
  ...
  const replacem?nts: Record<string, string> = {
    '✓ ': '',
    'cl?nica': 'cl?nica',
    'CL?NICA': 'CLÍNICA',
    'Ser?': 'Será',
  ...
  let next = input
  for (const [from, to] of Object.entries(replacem?nts)) {
...
function protocolMedication?oManipuladoV1Draft(item: ProtocolMedicationItem, clinicId: string): ManipuladoV1Formula {
  ...
  return normalizeManipuladoV1({
    ...
    identity: {
      ...
      clinic_id: clinicId,
      slug: sanitizeVisibleText(item.medication_name || item.m?nual_medication_name || ''),
      name: sanitizeVisibleText(item.medication_name || item.m?nual_medication_name || 'Manipulado'),
      pharmaceutical_form: sanitizeVisibleText(item.m?nual_presentation_label || item.presentation_text || 'Manipulado'),
      primary_route: route,
    ...
    prescribing: {
      ...
      cl?nical_note: sanitizeVisibleText(((item.metadata || {}) as Record<string, unknown>).notes || ''),
    },
    pharmacy: {
      ...base.pharmacy,
      qsp_text: sanitizeVisibleText(item.presentation_text || item.m?nual_presentation_label || ''),
      total_quantity: sanitizeVisibleText(item.presentation_text || item.m?nual_presentation_label || ''),
    },
  })
}
...
function buildProtocolExamItemsFromDraft(comm?nItems: ProtocolBundle['examItems'], custom?xamText: string): ProtocolBundle['examItems'] {
  const selectedComm?nExams = (comm?nItems || []).filter(
    (item) => !item.is_custom && COMMON_EXAMS.some((exam) => normalizeExamLabel(exam) === normalizeExamLabel(item.label))
  )
  const customItems = custom?xamText
    .split('\n')
  ...
  return [...selectedComm?nExams, ...customItems].map((item, index) => ({
    ...item,
    sort_order: index,
  }))
}
...
function canUserPublishGlobal(role: 'owner' | 'member' | null, user: any): boolean {
  ...
  return (
    hasTruthyFlag(userMetadata.is_adm?n) ||
    hasTruthyFlag(userMetadata.global_protocol_publisher) ||
    hasTruthyFlag(userMetadata.global_content_adm?n) ||
    hasTruthyFlag(appMetadata.is_adm?n) ||
    hasTruthyFlag(appMetadata.global_protocol_publisher) ||
    hasTruthyFlag(appMetadata.global_content_adm?n) ||
    readText(appMetadata.role) === 'adm?n'
  )
}
...
export default function Protocolos3Page() {
  ...
  const [protocolMedication?odalOpen, setProtocolMedication?odalOpen] = useState(false)
  ...
  const [protocolCustom?xamText, setProtocolCustom?xamText] = useState('')
  ...
  // ✅ Carregar pastas (apenas ap?s clinicId e userId estarem definidos)
  useEffect(() => {
    if (!clinicId || !userId) {
      console.log('[Protocolos3] Aguardan?o clinicId e userId', { clinicId, userId })
      return
    }

    console.log('[Protocolos3] Carregan?o folders', { clinicId, userId })
  ...
  // ✅ Carregar protocolos (apenas ap?s clinicId e userId estarem definidos)
  useEffect(() => {
    if (!clinicId || !userId) {
      console.log('[Protocolos3] Aguardan?o clinicId e userId para protocolos', { clinicId, userId })
      return
    }

    console.log('[Protocolos3] Carregan?o protocols', { clinicId, userId })
  ...
  // ✅ Busca de medicamentos (Cat?logo 3.0) com debounce
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        console.log('[Protocolos3] Medicam?n?os en?ontrados', standardResults.length)
  ...
  useEffect(() => {
    if (!modalOpen || !editingProtocol) {
      setProtocolCustom?xamText('')
      return
    }

    const custom?xamLines = (editingProtocol.examItems || [])
      .filter((item) => item.is_custom)
      .map((item) => cleanExamLabel(item.label || ''))
      .filter(Boolean)
      .join('\n')

    setProtocolCustom?xamText(custom?xamLines)
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (docum?nt.visibilityState === 'hidden') flush()
    }
    win?ow.addEventListener('beforeun?oad', flush)
    docum?nt.addEventListener('visibilitychange', handleVisibilityChange)
    return () => {
      win?ow.removeEventListener('beforeun?oad', flush)
      docum?nt.removeEventListener('visibilitychange', handleVisibilityChange)
    }
  ...
  const updateMedicationAt = useCallback((index: number, updater: (item: ProtocolMedicationItem) => ProtocolMedicationItem) => {
    updateEditingProtocol((prev) => ({
      ...prev,
      medications: prev.medications.map((item, item?ndex) => (item?ndex === index ? updater(item) : item)),
    }))
  }, [updateEditingProtocol])
  ...
  const handleOpenProtocolCompoundedEditor = useCallback(async (index: number) => {
    ...
    try {
      ...
      if (!clinicId) {
        throw new Error('Este item m?nipulado n?o possui contexto suficiente para conversão.')
      }

      const converted = protocolMedication?oManipuladoV1Draft(item, clinicId)
      ...
      console.error('[Protocolos3] Erro ao abrir editor V1 do m?nipulado', err)
      alert(err instanceof Error ? err.message : 'Falha ao abrir o editor V1 do m?nipulado.')
    } finally {
      setIsLoadingProtocolCompoundedEditor(false)
    }
  ...
  const updateRecomm?ndationAt = useCallback((index: number, text: string) => {
    updateEditingProtocol((prev) => ({
      ...prev,
      recommendations: prev.recommendations.map((item, item?ndex) =>
        item?ndex === index ? { ...item, text } : item
      ),
    }))
  }, [updateEditingProtocol])

  const removeRecomm?ndationAt = useCallback((index: number) => {
    updateEditingProtocol((prev) => ({
      ...prev,
      recommendations: prev.recommendations
        .filter((_, item?ndex) => item?ndex !== index)
        .map((item, item?ndex) => ({ ...item, sort_order: item?ndex })),
    }))
  }, [updateEditingProtocol])

  const addRecomm?ndationField = useCallback(() => {
    updateEditingProtocol((prev) => ({
      ...prev,
      recommendations: [
  ...
  const updateExamAt = useCallback((index: number, updater: (item: NonNullable<ProtocolBundle['examItems']>[number]) => NonNullable<ProtocolBundle['examItems']>[number]) => {
    updateEditingProtocol((prev) => ({
      ...prev,
      examItems: (prev.examItems || []).map((item, item?ndex) => (item?ndex === index ? updater(item) : item)),
    }))
  }, [updateEditingProtocol])
  ...
  const toggleComm?nExam = useCallback((label: string) => {
    updateEditingProtocol((prev) => {
      ...
      const nextItems = existingIndex >= 0
        ? currentItems.filter((_, item?ndex) => item?ndex !== existingIndex)
        : [
            ...currentItems,
            {
              label: cleanLabel,
      ...
      return {
        ...prev,
        examItems: nextItems.map((item, item?ndex) => ({ ...item, sort_order: item?ndex })),
      }
    })
  }, [updateEditingProtocol])
  ...
  const addCustomProtocolExam = useCallback(() => {
    ...
    if (!label) return
    setProtocolCustom?xamText((prev) => {
  ...
  const updateProtocolExamsFromText = useCallback((value: string) => {
    setProtocolCustom?xamText(value)
  }, [])
  ...
  const handleEditProtocol = useCallback(
    async (protocolId: string) => {
      ...
      try {
        console.log('[Protocolos3] Carregan?o protocolo para edição', protocolId)
  ...
  const handleSaveProtocol = useCallback(async () => {
    ...
    try {
      ...
      const bundleToSave: ProtocolBundle = {
        ...editingProtocol,
        examItems: buildProtocolExamItemsFromDraft(editingProtocol.examItems, protocolCustom?xamText),
      }
      console.log('[Protocolos3] Salvan?o protocolo', bundleToSave.protocol.name)
  ...
  }, [clinicId, userId, editingProtocol, isSavingProtocol, clearProtocolDraft, protocolCustom?xamText, reloadProtocols])
  ...
  const handleOpenPublishGlobalModal = useCallback(async () => {
    ...
    if (editingProtocol.medications.some((item) => isCompoundedProtocolMedication(item))) {
      alert('Protocolos com itens m?nipulados ainda n?o podem ser publicados globalm?nte. Remova os m?nipulados ou m?ntenha este protocolo som?nte na cl?nica.')
      return
    }
  ...
  const handlePublishGlobalProtocol = useCallback(async () => {
    ...
    if (editingProtocol.medications.some((item) => isCompoundedProtocolMedication(item))) {
      alert('Este protocolo contém item m?nipulado clinic-only. A publicação global está bloqueada nesta versão.')
      return
    }
    ...
    if (!name) {
      alert('In?orme um?nome global para publicar o protocolo.')
      return
    }
    if (!slug) {
  ...
  const handleDeleteProtocol = useCallback(
    async (protocolId: string) => {
      if (!clinicId || !userId) return
      if (!confirm('Deseja realm?nte excluir este protocolo?')) return

      try {
        console.log('[Protocolos3] Excluin?o protocolo', protocolId)
  ...
  const handleApplyToNovaReceita = useCallback(
    async (protocol: ProtocolListEntry) => {
      ...
      try {
        console.log('[Protocolos3] Carregan?o protocolo para aplicar em Nova Receita', protocol.id)
        ...
        if (!bundle) {
          alert('Protocolo n?o en?ontrado.')
          return
        }

        // Converter medicamentos do protocolo para itens da receita
        const prescriptionItems = bundle.medications.map(mapProtocolMedication?oPrescriptionItem)

        // Converter recom?ndações para string
        const recommendationsText = mapProtocolRecomm?ndationsToString(bundle.recommendations)
  ...
  const handleSelectProtocol = useCallback(
    async (protocolId: string) => {
      ...
      try {
        console.log('[Protocolos3] Carregan?o detalhes do protocolo', protocolId)
        const bundle = await loadProtocolBundle(clinicId, userId, protocolId)
        if (bundle) {
          setSelectedProtocolBundle(bundle)
          console.log('[Protocolos3] Protocolo carregado com sucesso', bundle.protocol.name)
        } else {
          console.error('[Protocolos3] Protocolo n?o en?ontrado', protocolId)
  ...
  const handleOpenGlobalProtocol = useCallback(async (protocolId: string) => {
    try {
      ...
      if (!bundle) {
        alert('Protocolo global n?o en?ontrado.')
        return
      }
  ...
  const handleAddMedication = useCallback(
    async (medication: MedicationSearchResult) => {
      ...
      try {
        console.log('[Protocolos3] Adicionan?o medicamento', medication.name)

        // Buscar apresenta??es do medicamento
        ...
        if (!defaultPresentation) {
          alert('Medicam?n?o n?o possui apresenta??es cadastradas.')
          return
        }
        ...
        const newItem: ProtocolMedicationItem = {
          ...
          m?nual_medication_name: null,
          m?nual_presentation_label: null,
  ...
  const handleAddMedicationFromProtocolModal = useCallback((item: any) => {
    ...
    setProtocolMedication?odalOpen(false)
  }, [updateEditingProtocol])

  const handleAddMedication?oDraft = useCallback(async (medication: MedicationSearchResult) => {
    if (!clinicId) return
    ...
    >
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="m?n-w-0 flex-1 pr-2">
  ...
  const renderProtocolEditorModal = useCallback(() => {
    ...
    const comm?nExamLabels = examItems.filter((item) => !item.is_custom).map((item) => cleanExamLabel(item.label))

    return (
      <RxvModalShell zIndexClass="z-[90]" overlayClassName="bg-black/90 backdrop-blur-sm">
        <div className="mx-auto flex m?x-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-[#2f5b25] bg-[#0a0f0a] text-slate-100 shadow-[0_0_60px_rgba(57,255,20,0.2)]">
            ...
            <div>
              ...
              <h2 className="text-xl font-black uppercase italic tracking-tight text-white">
                ...
                <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                  Configure medicamentos, recom?ndações e exames predefinidos
                </p>
            </div>
                ...
                <RxvButton
                  ...
                  title={
                    hasCompoundedProtocolItems
                      ? 'Protocolos com m?nipulados ainda n?o podem ser publicados globalm?nte'
                      : editingProtocol.protocol.id
                        ? 'Publicar protocolo para todos os usu?rios'
                        : 'Salve o protocolo local antes de publicar'
                  }
                  >
                    {isLoadingGlobalProtocols ? 'Carregan?o vínculo...' : 'Salvar como global'}
                  </RxvButton>
              )}
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-10">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <RxvButton
                    ...
                    onClick={() => {
                      ...
                      setProtocolMedication?odalOpen(true)
                    }}
              ...
              {editingProtocol.medications.length === 0 ? (
                ...
                <div className="space-y-4">
                  {editingProtocol.medications.map((med, idx) => (
                    <div key={`${med.medication_id || med.m?nual_medication_name || 'med'}-${idx}`} className="rounded-2xl border border-slate-800 bg-black/30 p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="m?n-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <p className="truncate text-sm font-black uppercase italic text-white">
                              {med.medication_name || med.m?nual_medication_name || 'Medicam?n?o'}
                            </p>
                          ...
                          <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                            {repairMojibakeText(med.m?nual_presentation_label || med.presentation_text || 'Apresentação n?o definida')}
                          </p>
                        </div>
                        <button
                          type="button"
                      ...
                      {isCompoundedProtocolMedication(med) ? (
                        <div className="mt-4 space-y-3">
                          <div className="grid grid-cols-1 gap-3 xl:grid-cols-[m?nm?x(0,1fr)_auto]">
                            <div className="rounded-2xl border border-[#39ff14]/15 bg-[#112114] px-4 py-4 text-sm text-slate-200">
                              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#98f98e]">
                                Fluxo V1.0 do m?nipulado no protocolo
                              </p>
                              ...
                              <p className="mt-3 leading-relaxed text-slate-300">
                                {sanitizeVisibleText(med.presentation_text || med.m?nual_presentation_label || 'Apresentação do m?nipulado n?o definida.')}
                              </p>
                              <p className="mt-2 text-xs text-slate-400">
                                {sanitizeVisibleText(med.route || 'Via n?o definida')} • {sanitizeVisibleText(getMedicationFrequencyLabel(med))} • {med.duration_days === -1 ? 'Uso contín?o até reavaliação do paciente' : med.duration_days ? `${med.duration_days} dia(s)` : 'Dura??o n?o definida'}
                              </p>
                      ...
                      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
                        ...
                        <RxvField label="Modo de duração">
                          <RxvSelect
                            value={med.duration_days === -1 ? 'contin?ous_until_recheck' : 'fixed_days'}
                            onChange={(e) => updateMedicationAt(idx, (current) => ({
                              ...current,
                              duration_days: e.target.value === 'contin?ous_until_recheck'
                                ? -1
                      ...
                      <div className="mt-3">
                        <RxvField label="Observa??es do medicamento">
                          <RxvTextarea
                            rows={3}
                            placeholder="Observa??es que devem acompanhar este item?no protocolo."
                      ...
                      <div className="mt-3 rounded-xl border border-slate-800 bg-black/20 px-3 py-3 text-xs text-slate-400">
                        ...
                        <p>{med.route || 'Via n?o definida'} • {getMedicationFrequencyLabel(med)} • {med.duration_days === -1 ? 'Uso contín?o até reavaliação do paciente' : med.duration_days ? `${med.duration_days} dia(s)` : 'Dura??o n?o definida'}</p>
                      </div>
                        </>
                      )}
                    </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                <h3 className="text-sm font-black uppercase tracking-widest italic text-[#39ff14]">
                  Recom?nda??es e orientações
                </h3>
                <RxvButton variant="secondary" onClick={addRecomm?ndationField} className="h-8 text-[10px]">
                  + Adicionar
                </RxvButton>
              </div>
                  ...
                  {editingProtocol.recommendations.map((recomm?ndation, idx) => (
                    <div key={`rec-${idx}`} className="rounded-2xl border border-slate-800 bg-black/20 p-3">
                      ...
                      <RxvTextarea
                        ...
                        value={recomm?ndation.text}
                        onChange={(e) => updateRecomm?ndationAt(idx, e.target.value)}
                      />
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  {COMMON_EXAMS.map((exam) => {
                    const selected = comm?nExamLabels.some((label) => normalizeExamLabel(label) === normalizeExamLabel(exam))
                    return (
                      <button
                        key={exam}
                        type="button"
                        onClick={() => toggleComm?nExam(exam)}
                ...
                <RxvField label="Justificativa">
                  <RxvTextarea
                    placeholder="Ex: Ficam sugeridos os seguintes exames para avaliar a fun??o dos rins e do fígado do paciente..."
                    rows={4}
    ...
    isSavingProtocol,
    protocolExamDraft,
    protocolCustom?xamText,
    protocolTagDraft,
    removeRecomm?ndationAt,
    setModalOpen,
    setProtocolExamDraft,
    setProtocolTagDraft,
    updateMedicationAt,
    ...
    >
      ...
      >
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 m?n-w-0 pr-4">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <span
  ...
  if (!clinicId) {
    return (
      <ReceituarioChrome section="protocolos" title="Carregan?o...">
        <div className="flex items-center justify-center min-h-screen">
          <p className="text-slate-500">Carregan?o cl?nica...</p>
        </div>
      </ReceituarioChrome>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex min-h-[calc(100vh-64px)] bg-[#0a0f0a]">
        ...
        <aside className="w-64 border-r border-slate-800/50 bg-black/40 p-4 shrink-0 overflow-y-auto">
          ...
          <nav className="space-y-1">
            ...
            >
              <span className="material-symbols-outlined text-[18px]">inven?ory_2</span>
              Todos
            </button>
            {isLoadingFolders ? (
              <div className="flex items-center gap-2 px-3 py-2 text-slate-600">
                <span className="material-symbols-outlined animate-spin text-[16px]">sync</span>
                <span className="text-xs">Carregan?o...</span>
              </div>
            ) : (
              folders.map(folder => (
          ...
          <div className="sticky top-0 z-30 border-b border-slate-800/50 bg-black/60 backdrop-blur-md px-8 py-6">
            <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1.5 opacity-80">
                {visibleProtocols.length} protocolos en?ontrados
              </p>
          ...
          <div className="p-8">
            {isLoadingProtocols ? (
              <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="mt-4 text-slate-500 text-sm font-bold uppercase tracking-widest">Carregan?o protocolos...</p>
              </div>
            ) : visibleProtocols.length === 0 ? (
              <div className="text-center py-20">
                <span className="material-symbols-outlined text-slate-800 text-[80px]">inven?ory_2</span>
                <p className="mt-4 text-xl font-bold text-slate-600">Nada por aqui</p>
                {searchQuery.trim() ? (
                  <p className="mt-2 text-sm text-slate-500">
                    Nenhum protocolo en?ontrado para esta busca.
                  </p>
                ) : (
                  <p className="mt-2 text-sm text-slate-500">
                    {scopeFilter === 'global'
                      ? 'Nenhum protocolo global disponível neste mom?n?o.'
                      : 'Crie seu primeiro protocolo para agilizar seus atendim?n?os.'}
                  </p>
                )}
              </div>
            ) : (
      ...
      {protocolMedication?odalOpen && editingProtocol && clinicId && (
        <AddMedication?odal2
          open={protocolMedication?odalOpen}
          onClose={() => {
            setProtocolMedicationManualMode(false)
            setProtocolMedication?odalOpen(false)
          }}
          ...
          m?nualMode={protocolMedicationManualMode}
          title={protocolMedicationManualMode ? 'Adicionar item m?nual ao protocolo' : 'Adicionar medicamento ao protocolo'}
          subtitle={protocolMedicationManualMode
            ? 'Cadastre um item livre para o protocolo quan?o ele n?o existir no catálogo.'
            : 'Busque no Cat?logo 3.0 e configure apresenta??o, dose, frequência, duração e observações que devem entrar quan?o o protocolo for aplicado.'}
      ...
      {protocolCompoundedEditorOpen && editingProtocol && (
        <RxvModalShell zIndexClass="z-[115]" overlayClassName="bg-black/95 backdrop-blur-sm">
          <div className="mx-auto flex m?x-h-[92vh] w-full max-w-[1400px] flex-col overflow-hidden rounded-3xl border border-[#39ff14]/20 bg-[#071007] shadow-[0_0_60px_rgba(57,255,20,0.15)]">
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  <h2 className="text-lg font-black uppercase italic tracking-tight text-white">
                    Editar m?nipulado no protocolo
                  </h2>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {protocolCompoundedEditorValue ? (
                ...
                <RxvCard className="p-6 text-sm text-slate-400">
                  Carregan?o m?nipulado no fluxo V1.0...
                </RxvCard>
              )}
            </div>
          </div>
      ...
      {false && modalOpen && editingProtocol && (
        <RxvModalShell zIndexClass="z-[90]" overlayClassName="bg-black/90 backdrop-blur-sm">
          <div className="mx-auto m?x-h-[92vh] w-full max-w-4xl overflow-hidden rounded-2xl border border-[#2f5b25] bg-[#0a0f0a] text-slate-100 shadow-[0_0_60px_rgba(57,255,20,0.2)] flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-slate-800 bg-black/60 px-8 py-6">
              <div>
                <div className="mb-2 flex items-center gap-2">
                ...
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">
                  Configure medicamentos e recom?ndações predefinidas
                </p>
              </div>
              <div className="flex items-center gap-3">
                {canPublishGlobalProtocols && (
                  <RxvButton
                    ...
                    title={editingProtocol.protocol.id ? 'Publicar protocolo para todos os usu?rios' : 'Salve o protocolo local antes de publicar globalm?nte'}
                  >
                    {isLoadingGlobalProtocols ? 'Carregan?o vínculo...' : 'Salvar como global'}
                  </RxvButton>
                )}
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-10">
              ...
              <div className="space-y-6">
                ...
                <RxvField label="Descrição">
                  <RxvTextarea
                    placeholder="Descrição breve para ajudá-lo a en?ontrar no futuro..."
                    value={editingProtocol.protocol.description || ''}
                    onChange={(e) =>
                      setEditingProtocol({
                        ...editingProtocol,
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <RxvField label="Dura??o resumida">
                    <RxvInput
                    ...
                    {editingProtocol.medications.map((med, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 m?n-w-0">
                            <p className="text-sm font-black text-white uppercase italic truncate">
                              {med.medication_name || med.m?nual_medication_name}
                            </p>
                ...
                <div className="space-y-4">
                  {editingProtocol.recommendations.map((rec, idx) => (
                    <div key={idx} className="relative group">
                      <RxvTextarea
                        placeholder="Ex: Oferecer água fresca, evitar bancos frios..."
                        value={rec.text}
                        onChange={(e) => {
                  ...
                  >
                    <span className="text-[10px] font-black text-slate-500 group-hover:text-[#39ff14] uppercase tracking-widest">
                      + Adicionar campo de recom?ndação
                    </span>
                  </button>
                </div>
              </div>
      ...
      {globalProtocolViewer && (
        <RxvModalShell zIndexClass="z-[100]" overlayClassName="bg-black/90 backdrop-blur-sm">
          <div className="mx-auto m?x-h-[92vh] w-full max-w-4xl overflow-hidden rounded-2xl border border-cyan-500/30 bg-[#071014] text-slate-100 shadow-[0_0_60px_rgba(34,211,238,0.15)] flex flex-col">
            <div className="flex items-center justify-between border-b border-slate-800 bg-black/60 px-8 py-6">
              <div>
                <div className="mb-2 flex items-center gap-2">
                ...
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">
                  Disponível para todos os usu?rios
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {globalProtocolViewer.medications.map((med, idx) => (
                      <div key={`${med.id || idx}`} className="rounded-2xl border border-slate-800 bg-black/30 p-4">
                        ...
                        <div><span className="block text-[10px] uppercase tracking-widest text-slate-500">Origem</span>{med.global_medication_id ? 'Cat?logo global' : 'Manual'}</div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="space-y-4">
                ...
                {(globalProtocolViewer.recommendations || []).length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {globalProtocolViewer.recommendations.map((recomm?ndation, idx) => (
                      <div key={`${recomm?ndation.id || idx}`} className="rounded-2xl border border-slate-800 bg-black/30 px-4 py-4 text-sm leading-relaxed text-slate-200">
                        {recomm?ndation.text}
                      </div>
                    ))}
                  </div>
          ...
          <div className="mx-auto w-full max-w-2xl rounded-2xl border border-[#2f5b25] bg-[#0a0f0a] text-slate-100 shadow-[0_0_60px_rgba(57,255,20,0.15)]">
            <div className="flex items-center justify-between border-b border-slate-800 bg-black/60 px-8 py-6">
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">
                  Publica este protocolo para todos os usu?rios via fluxo server-side
                </p>
              </div>
              <button
                type="button"
              ...
              {linkedGlobalProtocols.length > 0 ? (
                <div className="rounded-2xl border border-slate-800 bg-black/30 px-4 py-4 text-xs text-slate-400 leading-relaxed">
                  {publishGlobalDraft.mode === 'update'
                    ? 'O protocolo global selecionado terá versão increm?ntada e os itens filhos serão substituídos pelo conteúdo atual do protocolo local.'
                    : 'Este protocolo local já possui vínculo global. Se escolher salvar como novo, um?novo registro global será criado m?nten?o a rastreabilidade da origem.'}
                </div>
              ) : (
                <div className="rounded-2xl border border-slate-800 bg-black/30 px-4 py-4 text-xs text-slate-400 leading-relaxed">
                  Será criado um protocolo global novo, com rastreabilidade para o protocolo local e a cl?nica de origem.
                </div>
              )}
            </div>
      ...
      {/* Modal Buscar Medicam?n?os */}
      {medicationSearchOpen && (
        <RxvModalShell zIndexClass="z-[100]" overlayClassName="bg-black/95 backdrop-blur-md">
          <div className="mx-auto flex m?x-h-[86vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-[#39ff14]/30 bg-black shadow-[0_0_80px_rgba(57,255,20,0.2)]">
              ...
              <div>
                <h2 className="text-lg font-black uppercase italic tracking-tight text-white">
                  {presentationPickerMedication
                    ? 'Escolher apresenta??o'
                    : compoundedPickerFormula
                      ? 'Adicionar m?nipulado ao protocolo'
            ...
            {!presentationPickerMedication && !compoundedPickerFormula && (
              <div className="bg-black p-6">
                <div className="relative">
                  <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-slate-500">search</span>
                  <input
                    placeholder="Nome do fármaco ou princ?pio ativo..."
                ...
                <p className="mt-3 text-[11px] font-semibold uppercase tracking-widest text-slate-500">
                  Resultado unificado: catálogo padrão e catálogo de m?nipulados V1.0 da cl?nica
                </p>
              </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-2">
              {presentationPickerMedication ? (
                <>
                  <div className="flex items-center justify-between rounded-2xl border border-slate-800 bg-slate-900/40 px-4 py-3">
                    <p className="text-xs font-semibold text-slate-300">
                      Escolha uma apresenta??o comercial ou adicione o medicamento sem travar a apresenta??o.
                    </p>
                    <RxvButton
                      variant="secondary"
                      onClick={() => {
                  ...
                  >
                    <p className="text-sm font-black uppercase italic text-[#9eff8f]">
                      Adicionar sem apresenta??o definida
                    </p>
                    <p className="mt-1 text-xs text-slate-400">
                      A apresenta??o poderá ser escolhida depois, na Nova Receita 2.0, conforme o paciente.
                    </p>
                  </button>
                  ...
                  {isLoadingPresentationOptions ? (
                    <div className="flex flex-col items-center justify-center py-10 opacity-50">
                      ...
                      <p className="mt-2 text-[10px] font-black uppercase tracking-widest text-slate-500">Carregan?o apresenta??es...</p>
                    </div>
                  ) : presentationOptions.length === 0 ? (
                    <div className="py-10 text-center">
                      <p className="text-xs font-bold uppercase tracking-widest text-slate-600">
                        Nenhuma apresenta??o en?ontrada
                      </p>
                    </div>
                  ) : (
                ...
                <>
                  ...
                  <div className="grid gap-4 lg:grid-cols-[m?nm?x(0,1fr)_320px]">
                      ...
                      <div className="mt-4 grid gap-3 md:grid-cols-2">
                        <div className="rounded-2xl border border-slate-800 bg-black/30 p-4 text-sm text-slate-300">
                          ...
                          <div className="mt-3 space-y-2">
                            ...
                            <p>Via: {compoundedPickerFormula.identity.primary_route || "N?o definida"}</p>
                            <p>Frequ?ncia: {compoundedPickerFormula.prescribing.frequency_label || "N?o in?ormada"}</p>
                            <p>Dura??o: {compoundedPickerFormula.prescribing.duration_label || (compoundedPickerFormula.prescribing.duration_value ? `${compoundedPickerFormula.prescribing.duration_value} ${compoundedPickerFormula.prescribing.duration_unit}` : "Livre")}</p>
                          </div>
                        </div>
                        <div className="rounded-2xl border border-slate-800 bg-black/30 p-4 text-sm text-slate-300">
                          ...
                          <div className="mt-3 space-y-2">
                            ...
                            <p>Observa??es: {compoundedPickerFormula.identity.description || "Sem observações"}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex flex-col items-center justify-center py-10 opacity-50">
                  <span className="material-symbols-outlined animate-spin text-[#39ff14]">sync</span>
                  <p className="mt-2 text-[10px] font-black uppercase tracking-widest text-slate-500">Pesquisan?o catálogo...</p>
                </div>
              ) : medications.length === 0 && compoundedMedications.length === 0 ? (
                <div className="py-10 text-center">
                  <p className="text-xs font-bold uppercase tracking-widest text-slate-600">
                    {medicationSearchQuery.trim() ? "Nenhum resultado en?ontrado" : "Inicie a busca digitan?o acima"}
                  </p>
                </div>
              ) : (
                <div className="grid gap-6 lg:grid-cols-2">
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Cat?logo 3.0</p>
                    </div>
                    {medications.length === 0 ? (
                      <div className="rounded-2xl border border-dashed border-slate-800 bg-slate-900/30 px-4 py-6 text-sm text-slate-500">
                        Nenhum item padrão en?ontrado.
                      </div>
                    ) : (
                      medications.map((med) => (
                        <button key={med.id} type="button" onClick={() => handleAddMedication?oDraft(med)} className="group flex w-full items-center justify-between rounded-2xl border border-slate-800 bg-slate-900/50 p-4 text-left transition-all hover:border-[#39ff14]/40 hover:bg-slate-900">
                          <div>
