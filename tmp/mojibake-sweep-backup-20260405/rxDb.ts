import { formatStructuredConcentra??on } from './rxConcentra??on'
...
export interface CatalogDrug {
  ...
  controlled: boolean
  pharmacyType: 'hum?na' | 'veterin?ria' | 'm?nipulacao'
...
function uid(prefix: string) {
  return `${prefix}-${Date.now().toString(36)}-${Math.ran?om().toString(36).slice(2, 8)}`
}
...
function nextNumericRecordId(existingIds: string[]): string {
  ...
  existingIds.forEach((rawId) => {
    ...
    const n = Number(id)
    if (Number.isFinite(n) && n > m?x) m?x = n
  })
  return String(m?x + 1).padStart(5, '0')
}
...
const PHARMACY_TYPE_VALUES: PharmacyType[] = ['hum?na', 'veterin?ria', 'm?nipulacao']
...
function repairMojibake(value: string): string {
  if (!value) return ''
  const hasReplacem?ntChar = value.includes('\uFFFD')
  const hasComm?n?ojibakeMarkers = /[���]/.test(value)
  if (!hasReplacem?ntChar && !hasComm?n?ojibakeMarkers) return value
...
function seedCatalog(): CatalogDrug[] {
  ...
  return [
    ...
    },
    {
      ...
      updatedAt: now,
      presentations: [{ id: uid('pres'), name: 'Gotas', concentration: '500 mg/mL', unitLabel: 'mL', pharmacyTags: ['hum?na'] }],
    },
    {
      id: uid('drug'),
      name: 'Am?xicilina + Clavulanato',
      speciesTargets: ['Caes', 'Gatos'],
      controlled: false,
      pharmacyType: 'veterin?ria',
      routeGroup: 'ORAL',
      ...
      updatedAt: now,
      presentations: [{ id: uid('pres'), name: 'Comprimido', concentration: '250 mg/comprimido', unitLabel: 'comprimido', pharmacyTags: ['veterin?ria', 'hum?na'] }],
    },
    {
      id: uid('drug'),
      name: 'Meloxicam',
    ...
    },
    {
      ...
      updatedAt: now,
      presentations: [{ id: uid('pres'), name: 'C?psula', concentration: '50 mg/c?psula', unitLabel: 'c?psula', pharmacyTags: ['veterin?ria', 'hum?na', 'm?nipulacao'] }],
    },
    {
      id: uid('drug'),
      name: 'Cefalexina',
      ...
      updatedAt: now,
      presentations: [{ id: uid('pres'), name: 'Comprimido', concentration: '500 mg/comprimido', unitLabel: 'comprimido', pharmacyTags: ['veterin?ria', 'hum?na'] }],
    },
    {
      id: uid('drug'),
      name: 'Omeprazol',
      ...
      updatedAt: now,
      presentations: [{ id: uid('pres'), name: 'C?psula', concentration: '20 mg/c?psula', unitLabel: 'c?psula', pharmacyTags: ['hum?na'] }],
    },
  ]
}
...
function createProtocolTemplateItem(partial: Partial<PrescriptionItem>): PrescriptionItem {
  ...
  return {
    ...
    m?nualEdited: partial.m?nualEdited ?? false,
  ...
  return [
    {
      ...
      items: [
        createProtocolItemFromCatalog(catalog, 'meloxicam', 'Meloxicam', {
          ...
          timesPerDay: '1',
          durationDays: '5',
          cautions: ['Adm?nistrar ap?s alim?nta??o.'],
        }),
        ...
        createProtocolItemFromCatalog(catalog, 'am?xicilina', 'Am?xicilina + Clavulanato', {
          doseValue: '12.5',
          doseUnit: 'mg/kg',
          frequencyType: 'everyHours',
          everyHours: '12',
    ...
    },
    {
      ...
      recommendations: [
        'Monitorar frequ?ncia respirat?ria em repouso diariam?nte.',
        'Reavalia??o cardiol�gica em 30 dias.',
      ],
      exams: ['Bioqu?mica s?rica', 'Urin?lise', 'Ultrassonografia abdom?nal'],
      examReasons: [],
      createdAt: now,
      updatedAt: now,
    },
...
function normalizeCatalogPresentation(
  ...
  const structuredConcentration = formatStructuredConcentra??on({
...
function normalizeClient(raw: Partial<ClientRecord>): ClientRecord {
  return {
    ...
    complement: repairMojibake(raw.complement || (raw as any).addressComplem?nt || ''),
