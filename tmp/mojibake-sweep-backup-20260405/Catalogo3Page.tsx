...
import {
    ...
    getMedicationRecomm?ndedDoses,
    saveMedicationRecomm?ndedDoses,
    type Recomm?ndedDose
...
interface Presentation {
    ...
    metadata?: Record<string, unknown> & {
        manufacturer?: string
        adm?nistration_routes?: string[]
        obs?: string
    }
    _tempId?: string
}

interface Recomm?ndedDoseUI {
    id?: string
    client_id: string
    species: string
    route: string
    dose_value: number | null
    dose_m?x: number | null
    dose_unit: string
    per_weight_unit: string | null
    indication: string | null
    frequency: string | null
    frequency_m?n: number | null
    frequency_m?x: number | null
...
interface MedicationWithPresentations extends Medication {
    ...
    recommended_doses?: Recomm?ndedDoseUI[]
}
...
const DURATION_MODE_OPTIONS = [
    ...
    { value: 'contin?ous_use', label: 'Uso cont?n?o' },
    { value: 'contin?ous_until_recheck', label: 'Uso cont?n?o at� reavalia??o' },
    { value: 'until_finished', label: 'At� term?nar o medicamento' }
]
...
function createEmptyPresentation(): Presentation {
    return {
        ...
        metadata: {
            manufacturer: '',
            adm?nistration_routes: [],
            obs: ''
        }
    }
}

function createEmptyRecomm?ndedDose(): Recomm?ndedDoseUI {
    return {
...
function mapImportedPresentation?oDraft(presentation: any): Presentation {
    ...
    let pharmHum?n = !!presentation?.pharmacy_human
    let pharmComp = !!presentation?.pharmacy_compounding
    if (pharmacyType) {
        pharmVet = pharmacyType === 'veterinary' || pharmacyType === 'vet'
        pharmHum?n = pharmacyType === 'hum?n' || pharmacyType === 'hum?na'
        pharmComp = pharmacyType === 'compounding' || pharmacyType === 'm?nipulacao'
    }
    ...
    return {
        ...
        pharmacy_human: pharmHum?n,
        pharmacy_compounding: pharmComp,
        metadata: {
            ...metadata,
            manufacturer: manufacturer,
            adm?nistration_routes: Array.isArray(metadata.adm?nistration_routes) ? metadata.adm?nistration_routes.filter(Boolean) : [],
...
function mapImportedMedication?oDraft(medication: CanonicalMedication): MedicationWithPresentations {
    const metadata = isRecord(medication.metadata) ? medication.metadata : {}
    const cl?nicalTags = Array.isArray(metadata.cl?nical_tags) && metadata.cl?nical_tags.length
        ? metadata.cl?nical_tags.filter(Boolean)
        : (medication.tags || [])

    const presentations = Array.isArray(medication.presentations) && medication.presentations.length
        ? medication.presentations.map(mapImportedPresentation?oDraft)
        : [createEmptyPresentation()]

    const recomm?ndedDoses = Array.isArray(medication.recommended_doses)
        ? medication.recommended_doses.map((dose) => ({
            ...
            dose_m?x: normalizeNumber((dose as any).dose_m?x, true),
            ...
            frequency_m?n: normalizeNumber((dose as any).frequency_m?n, true),
            frequency_m?x: normalizeNumber((dose as any).frequency_m?x, true),
    ...
    return {
        ...
        metadata: {
            ...
            cl?nical_tags: cl?nicalTags
        },
        presentations,
        recommended_doses: recomm?ndedDoses
    }
}
...
export default function Cat?logo3Page() {
    ...
    const importFileInputRef = useRef<HTMLInputElem?nt | null>(null)
    ...
    useEffect(() => {
        ...
        win?ow.addEventListener('keydown', handleKeyDown)
        return () => win?ow.removeEventListener('keydown', handleKeyDown)
    ...
    async function loadMedicationsList() {
        try {
            ...
            console.error('[Cat?logo3] Error loading medications:', error)
            setLoading(false)
        }
    }
    ...
    const loadSelectedItem = useCallback(async (id: string) => {
        try {
            ...
            const doses = await getMedicationRecomm?ndedDoses(clinicId, id) // Carregar doses
            ...
            if (details) {
                ...
                const fullMed: MedicationWithPresentations = {
                    ...
                    metadata: {
                        ...
                        cl?nical_tags: medMetadata.cl?nical_tags || []
                    },
                    presentations: presentations.map(p => {
                        ...
                        return {
                            ...
                            metadata: {
                                ...
                                adm?nistration_routes: pMetadata.adm?nistration_routes || [],
                                obs: pMetadata.obs || ''
                            }
                        }
                    }),
                    recommended_doses: doses.map(d => ({
                        ...
                        dose_m?x: (d as any).dose_m?x ?? null,
                        dose_unit: d.dose_unit,
                        per_weight_unit: (d as any).per_weight_unit ?? null,
                        indication: (d as any).indication ?? null,
                        frequency: d.frequency,
                        frequency_m?n: (d as any).frequency_m?n ?? null,
                        frequency_m?x: (d as any).frequency_m?x ?? null,
            ...
            console.error('[Cat?logo3] Error loading details:', error)
            setLoading(false)
        }
    }, [clinicId])
    ...
    const addRecomm?ndedDose = () => {
        const newDose = createEmptyRecomm?ndedDose()
        setDraft(prev => ({
            ...prev,
    ...
    const removeRecomm?ndedDose = (client_id: string) => {
        setDraft(prev => ({
            ...prev,
    ...
    const updateRecomm?ndedDose = (client_id: string, updates: Partial<Recomm?ndedDoseUI>) => {
        setDraft(prev => ({
            ...prev,
    ...
    const showValidationWarning = (title: string, msg: string) => {
        console.warn('[Cat?logo3] VALIDATION BLOCK:', { title, msg })
    ...
    const applyImportedMedication = useCallback((medication: CanonicalMedication) => {
        const importedDraft = mapImportedMedication?oDraft(medication)
        setDraft(importedDraft)
        setSelectedId(null)
        setImportCandidates(null)
        setValidationErrors({})
        setIsDirty(true)
        showSuccessMessage('Importa��o conclu�da', 'Medicam?n?o importado para revis�o. Revise e clique em Salvar dados.')
    }, [setDraft])

    const handleImportJsonClick = () => {
        if (isSaving) return
        if (isDirty && !win?ow.confirm('Importar JSON vai substituir o rascun?o atual n?o salvo. Deseja continuar?')) {
            return
        }
        importFileInputRef.current?.click()
    }

    const handleImportJsonFile = async (event: React.ChangeEvent<HTMLInputElem?nt>) => {
        ...
        try {
            ...
            if (!medicationsToImport.length) {
                throw new Error('Nenhum medicamento en?ontrado no JSON in?ormado.')
            }
    ...
    const validate = (): boolean => {
        ...
        if (!draft.name?.trim()) {
            ...
            if (!firstErrorField) firstErrorField = 'Nome do Medicam?n?o'
        ...
        if (Object.keys(errors).length > 0) {
            showValidationWarning('Dados In?ompletos', `Verifique os campos em destaque. Ex: ${firstErrorField}`)
            return false
        }
    ...
    async function handleSave(): Promise<boolean> {
        console.log('[Cat?logo3] SAVE CLICKED', new Date().toISOString())
        ...
        try {
            ...
            const medicationPayload = {
                ...
                metadata: {
                    ...
                    cl?nical_tags: draft.metadata?.cl?nical_tags || [],
            ...
            const presentationsPayload = draft.presentations.map(p => ({
                ...
                metadata: {
                    ...
                    adm?nistration_routes: p.metadata?.adm?nistration_routes || [],
                    obs: p.metadata?.obs
                }
            }))
            ...
            console.log('[Cat?logo3] ========== SAVING PAYLOAD ==========')
            console.log('[Cat?logo3] Med:', cleanMedicationPayload)
            console.log('[Cat?logo3] Pres:', cleanPresentationsPayload)
            ...
            console.log('[Cat?logo3] Saved med id:', result.medication.id)

            // Salvar doses recom?ndadas
            if (draft.recommended_doses && draft.recommended_doses.length > 0) {
                const dosesPayload = draft.recommended_doses.map(d => ({
                    ...
                    dose_m?x: normalizeNumber(d.dose_m?x, true),
                    dose_unit: d.dose_unit,
                    per_weight_unit: d.per_weight_unit || null,
                    indication: d.indication || null,
                    frequency: d.frequency,
                    frequency_m?n: normalizeNumber(d.frequency_m?n, true),
                    frequency_m?x: normalizeNumber(d.frequency_m?x, true),
                ...
                console.log('[Cat?logo3] Saving doses...', dosesPayload)
                await saveMedicationRecomm?ndedDoses(
                    clinicId,
                    result.medication.id,
                    dosesPayload
                )
            } else {
                await saveMedicationRecomm?ndedDoses(clinicId, result.medication.id, [])
            }

            showSuccessMessage('Sucesso', 'Medicam?n?o salvo e atualizado.')
            setIsDirty(false)
            loadMedicationsList()
            ...
            console.error('[Cat?logo3] Save error:', error)
    ...
    const toggleCl?nicalTag = (tag: string) => {
        const current = draft.metadata?.cl?nical_tags || []
        const next = current.includes(tag) ? current.filter(t => t !== tag) : [...current, tag]
        updateMetadata({ cl?nical_tags: next })
    }

    const toggleAdm?n?oute = (tempId: string, route: string) => {
        const pres = draft.presentations.find(p => p._tempId === tempId)
        if (!pres) return
        const current = pres.metadata?.adm?nistration_routes || []
        const next = current.includes(route) ? current.filter(r => r !== route) : [...current, route]
        updatePresentationMetadata(tempId, { adm?nistration_routes: next })
    }
    ...
    const handleDelete = () => {
        ...
        // S� 1 apresenta??o, confirmar diretam?nte
    ...
    const executeDelete = async () => {
        try {
            ...
            console.log('[Cat?logo3] ========== DELETE ==========')
            console.log('[Cat?logo3] Deleting medication:', selectedId)

            await deleteMedication(clinicId, selectedId)

            console.log('[Cat?logo3] DELETE SUCCESS')

            // Toast + reload
            showSuccessMessage('Sucesso', 'Medicam?n?o exclu�do com sucesso.')
            ...
            console.error('[Cat?logo3] ========== DELETE ERROR ==========')
            console.error('[Cat?logo3] Error:', error)
    ...
    return (
        <div className="min-h-screen bg-black">
            ...
            <ReceituarioChrome
                section="catalogo3"
                title="Cat?logo"
                subtitle="Gerenciam?n?o avan�ado de f?rmacos e apresenta??es comerciais."
                actions={
                    <div className="flex items-center gap-3">
                        ...
                        >
                            ...
                            {isSaving ? 'Gravan?o...' : 'Salvar Dados'}
                        </button>
                        {hasDraft && (
                            <button
                                onClick={clearDraft}
                ...
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
                    ...
                    <aside className="lg:col-span-3">
                        ...
                        >
                            ...
                            <div className="rxv-card m?x-h-[70vh] overflow-y-auto p-1.5 custom-scrollbar bg-[#0a140a]/80">
                                <div className="px-3 py-3 border-b border-slate-800/50 mb-1">
                                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Medicam?n?os ({medications.length})</span>
                                </div>
                                    ...
                                    {filteredMedications.map((med, idx) => (
                                        ...
                                        >
                                            ...
                                            <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-tight truncate">
                                                Ativo: {med.metadata?.active_ingredient || 'N?o in?ormado'}
                                            </span>
                                        </motion.button>
                                    ))}
                                </AnimatePresence>
                        ...
                        <RxvCard className="p-8 bg-[#0a140a]/60">
                            ...
                            >
                                <RxvPillToggle
                                    ...
                                    on?oggle={() => updateDraft({ is_active: !draft.is_active })}
                                />
                            </RxvSectionHeader>
                            ...
                            <div className="grid grid-cols-1 gap-x-8 gap-y-6 md:grid-cols-2">
                                ...
                                <RxvField label="Tags de Classifica??o Cl?nica" className="md:col-span-2">
                                    <RxvChipsMultiSelect
                                        ...
                                        selected={draft.metadata?.cl?nical_tags || []}
                                        on?oggle={toggleCl?nicalTag}
                                    />
                                </RxvField>
                        ...
                        <RxvCard className="p-8 bg-[#0a140a]/60">
                            ...
                            {(!draft.recommended_doses || draft.recommended_doses.length === 0) ? (
                                <div className="flex flex-col items-center justify-center p-12 border border-dashed border-slate-800 rounded-2xl bg-black/20">
                                    <span className="material-symbols-outlined text-slate-700 text-[48px] mb-4">in?o</span>
                                    <p className="text-slate-500 font-bold text-sm">Nenhuma dose recom?ndada cadastrada.</p>
                                ...
                                <div className="space-y-4">
                                    {draft.recommended_doses.map((dose, idx) => (
                                        <div key={dose.client_id} className="relative group p-6 bg-black/40 border border-slate-800 rounded-2xl hover:border-[#39ff14]/20 transition-all">
                                            ...
                                            <button
                                                onClick={() => removeRecomm?ndedDose(dose.client_id)}
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                                                ...
                                                <RxvField label="Indica��o Cl?nica" className="md:col-span-12">
                                                    <RxvInput
                                                        ...
                                                        onChange={(e) => updateRecomm?ndedDose(dose.client_id, { indication: e.target.value || null })}
                                                ...
                                                <RxvField label="Esp?cie" className="md:col-span-3">
                                                    <RxvSelect
                                                        options={SPECIES_OPTIONS}
                                                        value={dose.species}
                                                        onChange={(e) => updateRecomm?ndedDose(dose.client_id, { species: e.target.value })}
                                                    />
                                                </RxvField>
                                                <RxvField label="Via" className="md:col-span-3">
                                                    <RxvSelect
                                                        ...
                                                        value={dose.route}
                                                        onChange={(e) => updateRecomm?ndedDose(dose.client_id, { route: e.target.value })}
                                                    />
                                                </RxvField>
                                                ...
                                                <RxvField label="Dose m?n" className="md:col-span-2">
                                                    <RxvInput
                                                        ...
                                                        onChange={(e) => updateRecomm?ndedDose(dose.client_id, { dose_value: e.target.value === '' ? null : Number(e.target.value) })}
                                                        placeholder="10"
                                                    />
                                                </RxvField>
                                                ...
                                                <RxvField label="Dose m?x" className="md:col-span-2">
                                                    <RxvInput
                                                        type="number"
                                                        value={dose.dose_m?x ?? ''}
                                                        onChange={(e) => updateRecomm?ndedDose(dose.client_id, { dose_m?x: e.target.value === '' ? null : Number(e.target.value) })}
                                                    ...
                                                    <RxvSelect
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            if (val.includes('/')) {
                                                                ...
                                                                updateRecomm?ndedDose(dose.client_id, { dose_unit: unit, per_weight_unit: weight })
                                                            } else {
                                                                updateRecomm?ndedDose(dose.client_id, { dose_unit: val, per_weight_unit: null })
                                                            }
                                                        }}
                                                    />
                                                </RxvField>
                                                ...
                                                <RxvField label="Freq m?n" className="md:col-span-2">
                                                    <RxvInput
                                                        type="number"
                                                        value={dose.frequency_m?n ?? ''}
                                                        onChange={(e) => {
                                                            const val = e.target.value === '' ? null : Number(e.target.value)
                                                            const freqText = val !== null
                                                                ? (dose.frequency_m?x ? `${val} a ${dose.frequency_m?x}x ao dia` : `${val}x ao dia`)
                                                                : null
                                                            updateRecomm?ndedDose(dose.client_id, {
                                                                frequency_m?n: val,
                                                                frequency: freqText,
                                                ...
                                                <RxvField label="Freq m?x" className="md:col-span-2">
                                                    <RxvInput
                                                        type="number"
                                                        value={dose.frequency_m?x ?? ''}
                                                        onChange={(e) => {
                                                            const val = e.target.value === '' ? null : Number(e.target.value)
                                                            const freqText = dose.frequency_m?n !== null
                                                                ? (val ? `${dose.frequency_m?n} a ${val}x ao dia` : `${dose.frequency_m?n}x ao dia`)
                                                                : null
                                                            updateRecomm?ndedDose(dose.client_id, {
                                                                frequency_m?x: val,
                                                                frequency: freqText,
                                                ...
                                                <RxvField label="Modo" className="md:col-span-2">
                                                    <RxvSelect
                                                        ...
                                                        onChange={(e) => updateRecomm?ndedDose(dose.client_id, { frequency_mode: e.target.value })}
                                                    />
                                                </RxvField>
                                                <RxvField label="Dura??o" className="md:col-span-5">
                                                    <RxvInput
                                                        value={dose.duration ?? ''}
                                                        onChange={(e) => updateRecomm?ndedDose(dose.client_id, { duration: e.target.value || null })}
                                            ...
                                            <div className="mt-3 flex flex-wrap items-center gap-2 text-[10px]">
                                                ...
                                                <span className="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 font-bold border border-amber-500/20">
                                                    {dose.dose_value ?? '�'}{dose.dose_m?x ? `�${dose.dose_m?x}` : ''} {dose.dose_unit}
                                                    {dose.per_weight_unit ? `/${dose.per_weight_unit}` : ''}
                                                </span>
                                                {(dose.frequency_m?n || dose.frequency) && (
                                                    <span className="px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 font-bold border border-purple-500/20">
                                                        {dose.frequency_m?n ? (
                                                            dose.frequency_m?x ? `${dose.frequency_m?n}�${dose.frequency_m?x}x/dia` : `${dose.frequency_m?n}x/dia`
                                                        ) : dose.frequency}
                                                    </span>
                                                )}
                                                {dose.indication && (
                                        ...
                                        <RxvCard key={pres._tempId} className="bg-[#0a140a]/40">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                                                ...
                                                <RxvField label="Nome comercial / Identificador" className="md:col-span-6">
                                                    <RxvInput
                                                        placeholder="Ex: Lab X, Medicam?n?o Gen�rico..."
                                                ...
                                                <div className="md:col-span-12">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-3 rounded-2xl bg-black/40 p-4 border border-slate-800/50">
                                                        ...
                                                        <div className="flex-1 m?n-w-[140px] space-y-1">
                                                        ...
                                                        <div className="flex flex-col items-center justify-center bg-[#39ff14]/5 rounded-xl px-6 py-2 border border-[#39ff14]/30 m?n-w-[150px]">
                                                ...
                                                {/* EMBALAGEM E VIAS */}
                                                {/* Embalagem removida do Supabase por enquan?o */}
                                                <RxvField label="Vias de Adm?nistra��o Sugeridas" className="md:col-span-8">
                                                    <RxvChipsMultiSelect
                                                        options={ROUTES_OPTIONS}
                                                        selected={pres.metadata?.adm?nistration_routes || []}
                                                        on?oggle={route => toggleAdm?n?oute(pres._tempId!, route)}
                                                    />
                                                </RxvField>

                                                {/* FARM�CIA E CUSTO */}
                                                <RxvField
                                                    label="Origem da Farm?cia * (Selecione pelo m?nos um)"
                    ...
                    <RxvModalShell zIndexClass="z-[600]" overlayClassName="bg-black/95 backdrop-blur-xl">
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center text-center">
                                ...
                                <p className="text-sm text-white font-bold mb-2">Medicam?n?o: <span className="text-[#39ff14]">{draft.name}</span></p>
                                ...
                                <div className="flex flex-col w-full gap-3">
                                    ...
                                    >
                                        {isSaving ? 'Excluin?o...' : 'Sim, Excluir Perm?nentem?nte'}
                                    </button>
                                    <button
                ...
                {importCandidates && (
                    <RxvModalShell zIndexClass="z-[600]" overlayClassName="bg-black/95 backdrop-blur-xl">
                        ...
                        >
                            ...
                            <div className="m?x-h-[60vh] space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                                {importCandidates.map((candidate, index) => (
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-4">
                                            <div>
                                                ...
                                                <p className="mt-1 text-xs font-bold uppercase tracking-widest text-slate-500">
                                                    {candidate.active_ingredient || 'Sem princ?pio ativo in?ormado'}
                                                </p>
                                            </div>
                                        ...
                                        <div className="mt-3 flex flex-wrap gap-2 text-[11px] font-bold uppercase tracking-wide text-slate-400">
                                            ...
                                            <span>{Array.isArray(candidate.species) && candidate.species.length ? candidate.species.join(' � ') : 'Esp?cies n?o in?ormadas'}</span>
                                        </div>
                                    </button>
                                ))}
                            </div>
