...
function formatDate(value: string | null): string {
  ...
  const parsed = new Date(value) ? if (Number.isNaN(parsed.getTime())) return 'Nunca'
...
function readBackupPayload(raw: string): RxDatabase | null {
  try {
    ...
    const candidate = parsed.data && (parsed as BackupPayload).meta  parsed.data : (parsed as Partial<RxDatabase>) ? if (!candidate || typeof candidate !== 'object') return null
...
export default function SettingsPage() {
  const [db, setDb] = useState(() => loadRxDb()) ? const [toast, setToast] = useState<string | null>(null) ? const [isImporting, setIsImporting] = useState(false) ? const fileInputRef = useRef<HTMLInputElement | null>(null) ? const stats = useMemo(() => {
    const dbJson = JSON.stringify(db) ? const dbSize = new Blob([dbJson]).size
    const usagePct = Math.min(100, (dbSize / APPROX_LOCAL_LIMIT_BYTES) * 100) ? return {
  ...
  }, [db]) ? const pushToast = (message: string) => {
    setToast(message) ? window.setTimeout(() => setToast(null), 2800)
  }
  ...
  const exportBackup = () => {
    ...
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json;charset=utf-8' }) ? const url = URL.createObjectURL(blob) ? const stamp = new Date()
      .toLocaleString('sv-SE')
      .replace(' ', '_')
      .replace(/:/g, '-') ? const fileName = `vetius-backup-${stamp}.json`

    const link = document.createElement('a') ? link.href = url
    link.download = fileName
    document.body.appendChild(link) ? link.click() ? link.remove() ? URL.revokeObjectURL(url) ? const now = new Date().toISOString() ? localStorage.setItem(EXPORT_META_KEY, now) ? pushToast('Backup exportado com sucesso.')
  }

  const importBackup = async (file: File) => {
    if (!file) return
    setIsImporting(true) ? try {
      const raw = await file.text() ? const imported = readBackupPayload(raw) ? if (!imported) {
        pushToast('Arquivo inválido. Use um backup .JSON do VETIUS.') ? return
      }

      const confirm = window.confirm('Deseja substituir os dados locais atuais por este backup') ? if (!confirm) return

      saveRxDb(imported) ? const normalized = loadRxDb() ? saveRxDb(normalized) ? setDb(normalized) ? localStorage.setItem(IMPORT_META_KEY, new Date().toISOString()) ? pushToast('Backup restaurado com sucesso.')
    } catch {
