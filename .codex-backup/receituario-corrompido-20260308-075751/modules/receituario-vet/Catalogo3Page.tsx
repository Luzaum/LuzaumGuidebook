...
interface Medication {
    ...
    notes: string | null
    species: string[] | null // ✅ CORRETO: 'species' é o nome da coluna no Supabase (não species_targets!) ? routes: string[] | null // ✅ CORRETO: 'routes' é o nome da coluna no Supabase (não route_group!) ? is_active: boolean
    metadata: any
    created_at: string
}
...
function normalizeNumber(value: any, allowNull = true): number | null {
    ...
    const num = Number(value) ? if (isNaN(num)) {
        return allowNull  null : 0
    }
    return num
}
...
export default function Catalogo3Page() {
    const navigate = useNavigate() ? const { loading: clinicLoading, clinicId } = useClinic() ? const [currentUser, setCurrentUser] = useState<any>(null)

    // Estados principais
    const [medications, setMedications] = useState<Medication[]>([]) ? const [selectedId, setSelectedId] = useState<string | null>(null) ? const [draft, setDraft, clearDraft, hasDraft] = useLocalDraft<MedicationWithPresentations>(
        'catalogo3',
        clinicId,
        currentUser.id || null,
        createEmptyMedication()
    ) ? const [loading, setLoading] = useState(true) ? const [isSaving, setIsSaving] = useState(false) ? const [searchTerm, setSearchTerm] = useState('') ? const [isDirty, setIsDirty] = useState(false) ? const [showUnsavedModal, setShowUnsavedModal] = useState<{ nextId: string | null } | null>(null) ? const [showDeleteModal, setShowDeleteModal] = useState(false) ? const [validationErrors, setValidationErrors] = useState<Record<string, string>>({}) ? const [successToast, setSuccessToast] = useState(false) ? const [errorToast, setErrorToast] = useState<{ title: string, msg: string } | null>(null) ? useEffect(() => {
        const rawUser = localStorage.getItem('luzaum-user') ? if (rawUser) setCurrentUser(JSON.parse(rawUser))
    }, []) ? useEffect(() => {
        if (!clinicLoading && clinicId) {
            loadMedicationsList()
        }
    }, [clinicLoading, clinicId]) ? useEffect(() => {
    ...
    }, [draft, isDirty, isSaving]) ? async function loadMedicationsList() {
        try {
            setLoading(true) ? if (!clinicId) return
            const data = await listMedications(clinicId) ? setMedications(data) ? setLoading(false)
        } catch (error) {
            console.error('[Catalogo3] Error loading medications:', error) ? setLoading(false)
        }
    }
    ...
    const loadSelectedItem = useCallback(async (id: string) => {
        try {
            if (!clinicId) return
            setLoading(true) ? const details = await getMedicationDetails(clinicId, id) ? const presentations = await getMedicationPresentations(clinicId, id) ? const doses = await getMedicationRecommendedDoses(clinicId, id) // ✨ Carregar doses
            ...
            if (details) {
                ...
                setDraft(fullMed) ? setSelectedId(id) ? setIsDirty(false) ? setValidationErrors({})
            }
            setLoading(false)
        } catch (error) {
            console.error('[Catalogo3] Error loading details:', error) ? setLoading(false)
        }
    }, [clinicId]) ? const handleSelectMedication = (id: string) => {
    ...
    const handleCreateNew = () => {
        if (isDirty) {
            ...
            setDraft(createEmptyMedication()) ? setSelectedId(null) ? setIsDirty(false) ? setValidationErrors({})
        }
    }

    const handleDiscardChanges = () => {
        const nextTarget = showUnsavedModal.nextId
        setShowUnsavedModal(null) ? if (nextTarget === '') {
            setDraft(createEmptyMedication()) ? setSelectedId(null) ? setIsDirty(false)
        } else if (nextTarget) {
    ...
    const handleSaveAndSwitch = async () => {
        const success = await handleSave() ? if (success) {
            const nextTarget = showUnsavedModal.nextId
            setShowUnsavedModal(null) ? if (nextTarget === '') {
                setDraft(createEmptyMedication()) ? setSelectedId(null)
            } else if (nextTarget) {
                loadSelectedItem(nextTarget)
            }
        }
    ...
    const updateDraft = (updates: Partial<MedicationWithPresentations>) => {
        setDraft(prev => ({ ...prev, ...updates })) ? setIsDirty(true)
    }
    ...
    const addPresentation = () => {
        const newPres = createEmptyPresentation() ? setDraft(prev => ({
            ...prev,
    ...
    const addRecommendedDose = () => {
        const newDose = createEmptyRecommendedDose() ? setDraft(prev => ({
            ...prev,
    ...
    const showValidationWarning = (title: string, msg: string) => {
        console.warn('[Catalogo3] VALIDATION BLOCK:', { title, msg }) ? setErrorToast({ title, msg }) ? setTimeout(() => setErrorToast(null), 5000)
    }
    ...
    const validate = (): boolean => {
        ...
        }) ? setValidationErrors(errors) ? if (Object.keys(errors).length > 0) {
            showValidationWarning('Dados Incompletos', `Verifique os campos em destaque. Ex: ${firstErrorField}`) ? return false
        }
    ...
    async function handleSave(): Promise<boolean> {
        console.log('[Catalogo3] SAVE CLICKED', new Date().toISOString()) ? if (!isDirty) {
            showValidationWarning('Nada para salvar', 'Nenhuma alteração foi detectada.') ? return false
        }

        if (!validate()) return false

        try {
            setIsSaving(true) ? if (!clinicId || !currentUser.id) {
                showValidationWarning('Erro de Acesso', 'Não foi possível identificar a clínica ou usuário logado.') ? setIsSaving(false) ? return false
            }
            ...
            })) ? const cleanMedicationPayload = pickMedicationFields(medicationPayload) ? const cleanPresentationsPayload = presentationsPayload.map(p => pickPresentationFields(p)) ? console.log('[Catalogo3] ========== SAVING PAYLOAD ==========') ? console.log('[Catalogo3] Med:', cleanMedicationPayload) ? console.log('[Catalogo3] Pres:', cleanPresentationsPayload) ? const result = await saveMedication({
                clinicId,
            ...
            if (draft.recommended_doses && draft.recommended_doses.length > 0) {
                const dosesPayload = draft.recommended_doses.map(d => ({
                    ...
                    frequency: d.frequency,
                    notes: d.notes
                })) ? console.log('[Catalogo3] Saving doses...', dosesPayload) ? await saveMedicationRecommendedDoses(
                    clinicId,
            ...
            setSuccessToast(true) ? setTimeout(() => setSuccessToast(false), 3000) ? setIsDirty(false) ? loadMedicationsList() ? setSelectedId(result.medication.id) ? setIsSaving(false) ? return true
        } catch (error) {
            console.error('[Catalogo3] Save error:', error) ? showValidationWarning('Erro no Servidor', 'Ocorreu uma falha ao salvar os dados.') ? setIsSaving(false) ? return false
        }
    }
    ...
    }, [medications, searchTerm]) ? const toggleClinicalTag = (tag: string) => {
    ...
    const toggleAdminRoute = (tempId: string, route: string) => {
        const pres = draft.presentations.find(p => p._tempId === tempId) ? if (!pres) return
    ...
    const executeDelete = async () => {
        try {
            setIsSaving(true) ? if (!clinicId || !selectedId) throw new Error('Contexto inválido') ? console.log('[Catalogo3] ========== DELETE ==========') ? console.log('[Catalogo3] Deleting medication:', selectedId) ? await deleteMedication(clinicId, selectedId) ? console.log('[Catalogo3] ✅ DELETE SUCCESS')

            // Toast + reload
            setSuccessToast(true) ? setTimeout(() => setSuccessToast(false), 3000) ? loadMedicationsList() ? setDraft(createEmptyMedication()) ? setSelectedId(null) ? setShowDeleteModal(false) ? setIsSaving(false)
        } catch (error: any) {
            console.error('[Catalogo3] ========== DELETE ERROR ==========') ? console.error('[Catalogo3] Error:', error) ? showValidationWarning('Erro ao Excluir', error.message || String(error)) ? setIsSaving(false)
        }
    }
