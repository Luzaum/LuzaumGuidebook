...
function buildCatalogControlledTargetPharmacy(
  ...
  const tags = new Set<PharmacyType>() ? controlledCatalog.forEach((drug) => {
  ...
  }) ? const hasVeterinaria = tags.has('veterinária') ? const hasHumana = tags.has('humana') ? const hasManipulacao = tags.has('manipulacao') ? const selectedCount = Number(hasVeterinaria) + Number(hasHumana) + Number(hasManipulacao) ? if (selectedCount > 1) return 'mista'
...
function controlledItemFromDrug(drug: CatalogDrug, index: number) {
  const item = createDefaultItem('medication') ? const presentation = drug.presentations[0]
  item.name = drug.name
...
export default function ControleEspecialPage() {
  const navigate = useNavigate() ? const [db, setDb] = useState(() => loadRxDb()) ? const refreshDb = useCallback(() => {
    setDb(loadRxDb())
  }, []) ? useEffect(() => {
    refreshDb() ? const intervalId = window.setInterval(refreshDb, 1400) ? const onStorage = () => refreshDb() ? const onFocus = () => refreshDb() ? const onVisibility = () => {
      if (document.visibilityState === 'visible') refreshDb()
    }

    window.addEventListener('storage', onStorage) ? window.addEventListener('focus', onFocus) ? document.addEventListener('visibilitychange', onVisibility) ? return () => {
      window.clearInterval(intervalId) ? window.removeEventListener('storage', onStorage) ? window.removeEventListener('focus', onFocus) ? document.removeEventListener('visibilitychange', onVisibility)
    }
  }, [refreshDb]) ? const controlledCatalog = useMemo(
    ...
    [db.templates]
  ) ? const previewProfile = useMemo(() => db.prescriberProfiles[0] || db.profile, [db.prescriberProfiles, db.profile]) ? const controlledTargetPharmacy = useMemo(
    () => buildCatalogControlledTargetPharmacy(controlledCatalog),
    [controlledCatalog]
  ) ? const previewDoc = useMemo(() => {
    const base = createDefaultPrescriptionState() ? const items = controlledCatalog.slice(0, 3).map((drug, idx) => controlledItemFromDrug(drug, idx)) ? const previewItems = items.length
       items
      : [
          {
  ...
  }, [controlledCatalog, previewProfile]) ? const openTemplateEditor = () => {
    const templateId = specialTemplate.id || 'rx_br_control_special'
    navigate(`/receituario-vet/templatestemplate=${encodeURIComponent(templateId)}&kind=special-control`)
  }
                  ...
                  <article key={drug.id} className="rounded-xl border border-amber-500/25 bg-amber-500/5 p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <button
                        ...
                        onClick={() => navigate(`/receituario-vet/catalogodrug=${encodeURIComponent(drug.id)}`)}
                      >
                        Editar no catálogo
                      </button>
                    </div>
