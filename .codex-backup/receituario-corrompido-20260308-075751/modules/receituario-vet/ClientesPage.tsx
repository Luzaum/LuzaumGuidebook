...
function nextNumericRecordId(existingIds: string[]): string {
  ...
  existingIds.forEach((rawId) => {
    const id = String(rawId || '').trim() ? if (!isNumericRecordId(id)) return
    const numeric = Number(id) ? if (Number.isFinite(numeric) && numeric > max) max = numeric
  }) ? return String(max + 1).padStart(5, '0')
}

function parseWeight(value: string): number | null {
  const normalized = String(value || '').replace(',', '.') ? const numeric = Number(normalized) ? if (!Number.isFinite(numeric) || numeric <= 0) return null
  return numeric
}

function formatWeight(value: string): string {
  const numeric = parseFloat(String(value).replace(',', '.')) ? if (Number.isNaN(numeric)) return '-'
...
function sparklinePath(animal: ClientAnimalRecord, width = 360, height = 120): string {
  ...
  const minY = Math.min(...points.map((entry) => entry.value)) ? const maxY = Math.max(...points.map((entry) => entry.value)) ? const spanY = Math.max(maxY - minY, 0.01) ? const left = 20
...
function buildUpdatedAnimalForSave(animal: ClientAnimalRecord): ClientAnimalRecord {
  const now = new Date().toISOString() ? const history: Array<{ id: string; date: string; weightKg: string; source: 'manual' | 'prescription' }> = [...(animal.weightHistory || [])]
    .map((entry) => ({
    ...
    .filter((entry) => !!entry.weightKg) ? const isSupabasePatient = isUuid(animal.id) ? const currentWeight = String(animal.weightKg || '').trim() ? if (currentWeight && !isSupabasePatient) {
...
function historyForAnimal(records: HistoryRecord[], client: ClientRecord, animal: ClientAnimalRecord): HistoryRecord[] {
  ...
  const tutorName = normalizeLooseText(client.fullName) ? const patientName = normalizeLooseText(animal.name) ? return [...records]
    .filter((entry) => {
...
export default function ClientesPage() {
  const { clinicId } = useClinic() ? const rxDataSource = useMemo(
  ...
  // ----- SUPABASE mode state -----
  const [supabaseTutors, setSupabaseTutors] = useState<TutorInfo[]>([]) ? const [supabasePatientsMap, setSupabasePatientsMap] = useState<Record<string, PatientInfo[]>>({}) ? const [supabaseLoading, setSupabaseLoading] = useState(false) ? const [supabaseSaving, setSupabaseSaving] = useState(false)

  // Load tutors from Supabase when in Supabase mode
  const loadSupabaseTutors = useCallback(async () => {
    if (rxDataSource !== 'supabase') return;
    console.log('[ClientesPage] REFRESH list', { dataSource: rxDataSource, clinicId }) ? setSupabaseLoading(true) ? try {
      ...
      const { data: directData, error: directError } = await supabase
        ...
        .is('deleted_at', null)
        .order('full_name', { ascending: true }) ? console.log('[ClientesPage] LIST result (direct)', { count: directData.length, data: directData, error: directError }) ? if (directData) {
      ...
      console.error('[ClientesPage] Failed to load Supabase tutors', err) ? syncToast(`Falha ao carregar lista: ${err instanceof Error  err.message : String(err)}`)
    } finally {
      setSupabaseLoading(false)
    }
  }, [rxAdapter, rxDataSource, clinicId]) ? useEffect(() => {
    console.log('[ClientesPage] active clinic', clinicId) ? if (rxDataSource === 'supabase') {
      void loadSupabaseTutors()
    }
  }, [rxDataSource, loadSupabaseTutors, clinicId]) ? useEffect(() => {
  ...
  const loadSupabasePatientsForTutor = useCallback(async (tutorId: string) => {
    ...
    try {
      console.log('[ClientesPage] LIST patients (explicit)', { tutorId }) ? const { data, error } = await supabase
        .from('patients')
        .select(`
          id,
          clinic_id,
        ...
        .eq('tutor_id', tutorId)
        .is('deleted_at', null)
        .order('name', { ascending: true }) ? if (error) {
        console.error('[ClientesPage] Failed to load patients (explicit)', error) ? throw error
      }
      ...
      if (data) {
        ...
        })) ? setSupabasePatientsMap((prev) => ({ ...prev, [tutorId]: mapped })) ? return mapped
      }
      return []
    } catch (err) {
      if (import.meta.env.DEV) console.error('[ClientesPage] Failed to load patients for tutor', tutorId, err) ? return []
    }
  }, [rxAdapter, rxDataSource, clinicId])

  // ----- SHARED state -----
  const [currentUserId, setCurrentUserId] = useState<string | null>(null) ? const [selectedId, setSelectedId] = useState<string>('') ? const baseInitialDraft = useMemo<ClientRecord>(() => {
    const localDb = loadRxDb() ? if (localDb.clients[0]) return localDb.clients[0]
    const empty = createEmptyClient() ? const nextClientId = nextNumericRecordId(localDb.clients.map((entry) => entry.id)) ? const nextAnimalId = nextNumericRecordId(localDb.clients.flatMap((entry) => entry.animals.map((animal) => animal.id))) ? return {
      ...empty,
      id: nextClientId,
  ...
  ) ? const [toast, setToast] = useState<string | null>(null) ? const [cepLookupLoading, setCepLookupLoading] = useState(false) ? const [cepLookupMessage, setCepLookupMessage] = useState<string | null>(null)

  // Bulk mode state
  const [isBulkMode, setIsBulkMode] = useState(false) ? const [selectedTutorIds, setSelectedTutorIds] = useState<Set<string>>(new Set()) ? const [isDeleting, setIsDeleting] = useState(false)

  // Custom Confirm Modal State
  const [confirmOpen, setConfirmOpen] = useState(false) ? const [confirmTitle, setConfirmTitle] = useState("") ? const [confirmDesc, setConfirmDesc] = useState("") ? const [confirmBusy, setConfirmBusy] = useState(false) ? const [confirmAction, setConfirmAction] = useState<null | (() => Promise<void> | void)>(null) ? function openConfirm(opts: { title: string; desc: string; onConfirm: () => Promise<void> | void }) {
    setConfirmTitle(opts.title) ? setConfirmDesc(opts.desc) ? setConfirmAction(() => opts.onConfirm) ? setConfirmOpen(true)
  }

  // Tutor Delete State (à prova de burrice) ? const [pendingDeleteTutorId, setPendingDeleteTutorId] = useState<string | null>(null) ? const [pendingDeleteTutorName, setPendingDeleteTutorName] = useState<string>('') ? const [isDeleteTutorOpen, setIsDeleteTutorOpen] = useState(false) ? const [tutorHasPrescriptions, setTutorHasPrescriptions] = useState(false) ? async function openDeleteTutorModal(tutorId: string, tutorName: string) {
    console.log('[TutorDelete] open modal', { tutorId, tutorName }) ? setPendingDeleteTutorId(tutorId) ? setPendingDeleteTutorName(tutorName) ? setTutorHasPrescriptions(false) ? if (clinicId) {
      const { count, error } = await supabase
        .from('prescriptions')
        .select('id', { count: 'exact', head: true })
        .eq('clinic_id', clinicId)
        .eq('tutor_id', tutorId) ? if (!error && (count  0) > 0) {
        console.log('[TutorDelete] tutor has prescriptions', { tutorId, count }) ? setTutorHasPrescriptions(true)
      }
    }
  ...
  async function confirmDeleteTutor() {
    console.log('[TutorDelete] confirm clicked', {
      ...
      console.warn('[TutorDelete] no pendingDeleteTutorId, aborting') ? return
    }
    if (!clinicId) {
      console.warn('[TutorDelete] no clinicId, aborting') ? return
    }

    setIsDeleting(true) ? try {
      const { deleteTutorSoft } = await import('@/src/lib/clinicRecords') ? await deleteTutorSoft(pendingDeleteTutorId, clinicId) ? console.log('[TutorDelete] success', { pendingDeleteTutorId }) ? syncToast(tutorHasPrescriptions ? `Tutor ${pendingDeleteTutorName} arquivado.` : `Tutor ${pendingDeleteTutorName} removido.`) ? void loadSupabaseTutors() ? if (selectedId === pendingDeleteTutorId) {
        setSelectedId('') ? createNewClient()
      }
    } catch (err) {
      console.error('[TutorDelete] failed', { error: err, tutorId: pendingDeleteTutorId }) ? syncToast('Falha ao remover tutor.')
    } finally {
      setIsDeleting(false) ? setIsDeleteTutorOpen(false) ? setPendingDeleteTutorId(null) ? setPendingDeleteTutorName('')
    }
  }
  ...
  const selectedClient = useMemo(
    ...
    setToast(message) ? window.setTimeout(() => setToast(null), 2400)
  }

  const selectClient = (clientId: string) => {
    const found = db.clients.find((entry) => entry.id === clientId) ? if (!found) return
    setSelectedId(found.id) ? setDraft(JSON.parse(JSON.stringify(found)) as ClientRecord) ? setCepLookupMessage(null)
  }
  ...
  const createNewClient = () => {
    const next = createEmptyClient() ? if (rxDataSource === 'supabase') {
      // In Supabase mode, clear ID (UUID will be generated on save) ? setSelectedId('') ? setDraft({
        ...next,
        id: '',
  ...
  const appendManualWeight = async (index: number) => {
    ...
    if (!animal) return
    const currentWeight = String(animal.weightKg || '').trim() ? if (!currentWeight) {
      syncToast('Preencha o peso atual antes de registrar na evolução.') ? return
    }

    if (rxDataSource === 'supabase' && isUuid(animal.id)) {
      const weightNum = parseFloat(currentWeight.replace(',', '.')) ? if (isNaN(weightNum)) {
        syncToast('Peso inválido.') ? return
      }

      console.log('[WeightInsert] UI Action', { patientId: animal.id, weightNum, weightDate: animal.weightDate }) ? const result = await insertPatientWeight({
        patientId: animal.id,
        clinicId: clinicId!,
        weightKg: weightNum,
        ...
        const freshWeights = await loadPatientWeights(animal.id, clinicId!) ? patchAnimal(index, 'weightHistory', freshWeights.map(w => ({
          id: w.id,
          date: w.measured_at,
          weightKg: String(w.weight_kg),
          source: 'manual'
    ...
    if (last.weightKg === currentWeight) {
      syncToast('Peso atual já está registrado na evolução.') ? return
    }
    const now = new Date() ? const todayDate = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`
    ...
    history.push({
      ...
      weightKg: currentWeight,
      source: 'manual',
    }) ? patchAnimal(index, 'weightDate', entryDate) ? patchAnimal(index, 'weightHistory', history) ? syncToast('Peso adicionado ao gráfico de evolução.')
  }
  ...
  const toggleTutorSelection = (tutorId: string) => {
    setSelectedTutorIds((prev) => {
      const next = new Set(prev) ? if (next.has(tutorId)) next.delete(tutorId) ? else next.add(tutorId) ? return next
    })
  }
  ...
  const selectAllTutors = () => {
    const allIds = rxDataSource === 'supabase'
      ...
      : db.clients.map(c => c.id) ? setSelectedTutorIds(new Set(allIds))
  }
  ...
  const deleteSingleTutorSupabase = (tutorId: string, name: string) => {
    openConfirm({
      ...
      onConfirm: async () => {
        setIsDeleting(true) ? console.log('[TutorDelete] START', { tutorId, activeClinicId: clinicId }) ? try {
          const { data, error } = await supabase
            .from('tutors')
            .update({ deleted_at: new Date().toISOString() })
            .eq('id', tutorId)
            .eq('clinic_id', clinicId)
            .select('id, clinic_id') ? console.log('[TutorDelete] RESULT', { data, error }) ? if (error) {
            console.error('[TutorDelete] ERROR', error) ? syncToast(`Erro ao apagar: ${error.message}`) ? return
          }

          if (!data || data.length === 0) {
            console.warn('[TutorDelete] NOTHING DELETED (data empty). Check clinic_id mismatch or RLS.') ? syncToast('Nenhum registro apagado. Verifique permissões.') ? return
          }

          console.log('[TutorDelete] SUCCESS', { deleted: data }) ? syncToast(`Tutor ${name} removido.`) ? void loadSupabaseTutors() ? if (selectedId === tutorId) {
            setSelectedId('') ? createNewClient()
          }
        } catch (err: any) {
          console.error('[TutorDelete] UNEXPECTED ERROR', err) ? syncToast('Falha ao remover tutor.')
        } finally {
          setIsDeleting(false)
        }
      },
  ...
  const deletePatientSupabase = (patientId: string, name: string, index: number) => {
    openConfirm({
      ...
      onConfirm: async () => {
        setIsDeleting(true) ? console.log('[PatientDelete] START', { patientId, activeClinicId: clinicId }) ? try {
          const { data, error } = await supabase
            .from('patients')
            .update({ deleted_at: new Date().toISOString() })
            .eq('id', patientId)
            .eq('clinic_id', clinicId)
            .select('id, clinic_id') ? console.log('[PatientDelete] RESULT', { data, error }) ? if (error) {
            console.error('[PatientDelete] ERROR', error) ? syncToast(`Erro ao apagar paciente: ${error.message}`) ? return
          }

          if (!data || data.length === 0) {
            console.warn('[PatientDelete] NOTHING DELETED. Check clinic_id mismatch or RLS.') ? syncToast('Nenhum paciente apagado.') ? return
          }

          console.log('[PatientDelete] SUCCESS', { deleted: data }) ? syncToast(`Paciente ${name} removido.`)
          ...
          console.error('[PatientDelete] UNEXPECTED ERROR', err) ? syncToast('Falha ao remover paciente.')
        } finally {
          setIsDeleting(false)
        }
      },
  ...
  const deleteMultipleTutorsSupabase = () => {
    ...
    if (count === 0) return
    const tutorIds = Array.from(selectedTutorIds) ? openConfirm({
      title: "Apagar tutores selecionados",
      desc: `Você tem certeza que quer apagar ${count} tutores do sistema Isso é irreversível.`,
      onConfirm: async () => {
        setIsDeleting(true) ? console.log('[BulkDelete] START', { count, tutorIds, activeClinicId: clinicId }) ? try {
          const { data, error } = await supabase
            .from('tutors')
            .update({ deleted_at: new Date().toISOString() })
            .in('id', tutorIds)
            .eq('clinic_id', clinicId)
            .select('id, clinic_id') ? console.log('[BulkDelete] RESULT', { data, error }) ? if (error) {
            console.error('[BulkDelete] ERROR', error) ? syncToast(`Erro ao apagar vários: ${error.message}`) ? return
          }

          if (!data || data.length === 0) {
            console.warn('[BulkDelete] NOTHING DELETED. Check clinic_id mismatch or RLS.') ? syncToast('Nenhum registro apagado.') ? return
          }

          console.log('[BulkDelete] SUCCESS', { deletedCount: data.length }) ? syncToast(`${data.length} tutores removidos.`) ? setSelectedTutorIds(new Set()) ? setIsBulkMode(false) ? void loadSupabaseTutors() ? if (selectedTutorIds.has(selectedId)) {
            setSelectedId('') ? createNewClient()
          }
        } catch (err: any) {
          console.error('[BulkDelete] UNEXPECTED ERROR', err) ? syncToast('Falha ao remover tutores.')
        } finally {
          setIsDeleting(false)
        }
      },
  ...
  const saveClientDraft = async () => {
    const now = new Date().toISOString() ? const animals = (draft.animals.length  draft.animals : [createEmptyClientAnimal()]).map((animal) =>
    ...
    if (rxDataSource === 'supabase') {
      ...
      const fullName = nextClient.fullName.trim() ? console.log('[ClientesPage] SAVE clicked', {
        dataSource: rxDataSource,
        activeClinicId: clinicId,
        tutorId: draft.id,
        tutorName: fullName,
      }) ? if (!fullName) {
        syncToast('Preencha o nome do tutor antes de salvar.') ? return
      }
      setSupabaseSaving(true) ? try {
        // 1. Determine if this is a new tutor or existing (by UUID in selectedId) ? let tutorId: string
        const isExistingTutor = isUuid(selectedId) ? if (isExistingTutor) {
          ...
          console.log('[DEBUG] tutor payload', payloadTutor) ? try {
            const result = await insertTutor(payloadTutor, clinicId) ? console.log('[DEBUG] tutor result', { data: result, error: null }) ? const inserted = Array.isArray(result) ? result[0] : result
            tutorId = (inserted as { id: string }).id
            if (!tutorId) throw new Error('ID do tutor não retornado pelo Supabase.') ? if (import.meta.env.DEV) console.log('[ClientesPage] New tutor ID:', tutorId)
          } catch (tutorErr) {
            console.log('[DEBUG] tutor result', { data: null, error: tutorErr }) ? throw tutorErr
          }
        }

        // 2. Save each animal as a patient linked to this tutor
        const animalsToSave = animals.filter((a) => a.name.trim()) ? const newPatients: PatientInfo[] = []
        for (const animal of animalsToSave) {
          // Skip animals that already have a UUID (already saved) ? if (isUuid(animal.id)) {
            // Already a Supabase patient — skip re-insert
            continue
          }
          try {
            const isNeutered = normalizeNeutered(animal.reproductiveStatus) ? if (import.meta.env.DEV) {
            ...
            console.log('[DEBUG] patient payload', payloadPatient) ? const patientResult = await insertPatient(payloadPatient, clinicId) ? console.log('[DEBUG] patient result', { data: patientResult, error: null }) ? const insertedPatient = Array.isArray(patientResult) ? patientResult[0] : patientResult
            if (insertedPatient) {
              newPatients.push({
            ...
            syncToast(`Erro ao salvar paciente "${animal.name}": ${msg}`) ? if (import.meta.env.DEV) console.error('[ClientesPage] Patient save error:', patientErr)
          }
        }

        // Update local state
        setSelectedId(tutorId) ? setDraft((prev) => ({
          ...prev,
          id: tutorId,
          animals: prev.animals.map((a) => {
            if (isUuid(a.id)) return a // already persisted
            // Find by name in successfully saved patients
            const saved = newPatients.find(p => p.name === a.name) ? return saved  { ...a, id: saved.patientRecordId } : a
          }),
        }))

        // Refresh Supabase tutor list and patient cache
        await loadSupabaseTutors() ? await loadSupabasePatientsForTutor(tutorId) ? clearDraft() ? const savedCount = animalsToSave.filter((a) => !isUuid(a.id)).length
        ...
        console.error('[ClientesPage] SAVE error', error) ? const detailedMsg = error.message || error.details || String(error) ? const code = error.code ? ` [Code: ${error.code}]` : ''
      ...
      const nextDb = upsertClient(db, nextClient) ? saveRxDb(nextDb) ? setDb(nextDb) ? const persistedClient =
        ...
        nextDb.clients[0]
      if (persistedClient) {
        setSelectedId(persistedClient.id) ? setDraft(JSON.parse(JSON.stringify(persistedClient)) as ClientRecord)
      }
      clearDraft() ? syncToast('Tutor e pacientes salvos com sucesso.')
    }
  }
  ...
  const deleteSelectedClient = () => {
    ...
    openConfirm({
      ...
      onConfirm: () => {
        const nextDb = removeClient(db, selectedClient.id) ? saveRxDb(nextDb) ? setDb(nextDb) ? if (nextDb.clients[0]) {
          setSelectedId(nextDb.clients[0].id) ? setDraft(JSON.parse(JSON.stringify(nextDb.clients[0])) as ClientRecord)
        } else {
          setSelectedId('') ? const empty = createEmptyClient() ? const nextClientId = nextNumericRecordId(nextDb.clients.map((entry) => entry.id)) ? const nextAnimalId = nextNumericRecordId(nextDb.clients.flatMap((entry) => entry.animals.map((animal) => animal.id))) ? setDraft({
            ...empty,
            id: nextClientId,
            animals: empty.animals.map((animal, idx) => ({ ...animal, id: idx === 0  nextAnimalId : animal.id })),
          })
      ...
      actions={
        <>
          ...
          <button
            ...
            onClick={() => {
              clearDraft() ? createNewClient() ? syncToast('Rascunho limpo.')
            }}
            ...
            {rxDataSource === 'supabase' && (
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  onClick={() => {
                    setIsBulkMode(!isBulkMode) ? if (isBulkMode) clearTutorSelection()
                  }}
                >
                    ...
                    <div
                      ...
                      onKeyDown={(e) => {
                        if (!isBulkMode && !isDeleting && (e.key === 'Enter' || e.key === ' ')) {
                          e.preventDefault() ? selectSupabaseTutor(tutor)
                        }
                      }}
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {!isBulkMode && (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation() ? openDeleteTutorModal(tutor.tutorRecordId, tutor.full_name || tutor.name || '')
                            }}
          ...
          <section className="rxv-card p-5">
            ...
            <div className="space-y-6">
              {draft.animals.map((animal, index) => {
                const historyEntries = historyForAnimal(db.history, draft, animal) ? const weightLine = sparklinePath(animal) ? const sortedWeightHistory = [...(animal.weightHistory || [])].sort(
                  (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
                ) ? const breedOptions = breedOptionsForSpecies(animal.species === 'Felina' ? 'Felina' : 'Canina') ? const coatOptions = coatOptionsForSpecies(animal.species === 'Felina' ? 'Felina' : 'Canina') ? return (
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    setConfirmBusy(true) ? await confirmAction() ? setConfirmOpen(false)
                  } catch (err) {
