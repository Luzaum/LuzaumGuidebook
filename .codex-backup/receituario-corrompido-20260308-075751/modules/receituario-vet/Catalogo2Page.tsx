...
interface Presentation {
  ...
  // Front-end only fields (MVP - não enviados ao Supabase se não existirem na schema) ? manufacturer: string
...
export default function Catalogo2Page() {
  ...
  // ==================== STATE ====================
  const [medications, setMedications] = useState<MedicationWithPresentations[]>([]) ? const [selectedId, setSelectedId] = useState<string>('') ? const [draft, setDraft] = useState<MedicationWithPresentations>(createEmptyMedication()) ? const [originalDraft, setOriginalDraft] = useState<MedicationWithPresentations>(createEmptyMedication()) ? const [loading, setLoading] = useState(true) ? const [isSaving, setIsSaving] = useState(false) ? const [errors, setErrors] = useState<Record<string, string>>({}) ? const [searchQuery, setSearchQuery] = useState('') ? const [showUnsavedModal, setShowUnsavedModal] = useState(false) ? const [pendingMedication, setPendingMedication] = useState<MedicationWithPresentations | null>(null) ? const [saveSuccess, setSaveSuccess] = useState(false)

  // ==================== COMPUTED ====================

  const isDirty = useMemo(() => {
    return JSON.stringify(draft) !== JSON.stringify(originalDraft)
  }, [draft, originalDraft]) ? const filteredMedications = useMemo(() => {
    if (!searchQuery.trim()) return medications
    const query = searchQuery.toLowerCase() ? return medications.filter((med) => med.name.toLowerCase().includes(query))
  }, [medications, searchQuery])

  // ==================== MOUNT LOG ====================
  useEffect(() => {
    console.log('[Catalogo2Page] MOUNTED ✅', new Date().toISOString()) ? console.log('[Clinic] clinicId =', clinicId, ', clinicName =', clinicName)
  }, [clinicId, clinicName])

  // ==================== CARREGAR MEDICAMENTOS ====================
  useEffect(() => {
    if (clinicLoading) {
      console.log('[Catalogo2] Waiting for clinic context to load...') ? return
    }
    if (!clinicId) {
      console.error('[Catalogo2] ❌ No clinic ID after clinic loaded') ? setLoading(false) ? return
    }
    loadMedications()
  }, [clinicLoading, clinicId]) ? async function loadMedications() {
    try {
      console.log('[Catalogo2] Loading medications from Supabase for clinic:', clinicId) ? setLoading(true) ? if (!clinicId) {
        console.error('[Catalogo2] ❌ No clinic ID available') ? setLoading(false) ? return
      }
      ...
      // Buscar medications
      const meds = await listMedications(clinicId) ? console.log('[Catalogo2] Loaded', meds.length, 'medications')

      // Buscar presentations de cada medication
      const medsWithPres: MedicationWithPresentations[] = await Promise.all(
        meds.map(async (med) => {
          const pres = await getMedicationPresentations(clinicId, med.id) ? return {
            ...med,
      ...
      if (medsWithPres.length > 0 && !selectedId) {
        ...
        setSelectedId(firstMed.id) ? setDraft(JSON.parse(JSON.stringify(firstMed))) ? setOriginalDraft(JSON.parse(JSON.stringify(firstMed)))
      }

      setLoading(false)
    } catch (error) {
      console.error('[Catalogo2] Load error:', error) ? setErrors({ load: `Erro ao carregar: ${error instanceof Error  error.message : String(error)}` }) ? setLoading(false)
    }
  }
  ...
  function attemptSelectMedication(med: MedicationWithPresentations) {
    if (isDirty) {
      setPendingMedication(med) ? setShowUnsavedModal(true)
    } else {
  ...
  function selectMedication(med: MedicationWithPresentations) {
    setSelectedId(med.id) ? const copy = JSON.parse(JSON.stringify(med)) ? setDraft(copy) ? setOriginalDraft(copy) ? setErrors({}) ? setShowUnsavedModal(false) ? setPendingMedication(null)
  }
  ...
  function createNew() {
    if (isDirty) {
      setPendingMedication(createEmptyMedication()) ? setShowUnsavedModal(true)
    } else {
      const newMed = createEmptyMedication() ? setSelectedId('') ? setDraft(newMed) ? setOriginalDraft(newMed) ? setErrors({})
    }
  }
  ...
  async function handleSave() {
    console.log('[Catalogo2] ========== SAVE CLICKED ==========', new Date().toISOString()) ? console.log('[Catalogo2] draft:', draft)

    // Validar
    if (!validateForm()) {
      console.error('[Catalogo2] Validation failed:', errors) ? return
    }

    try {
      setIsSaving(true) ? setErrors({}) ? if (!clinicId) {
        throw new Error('Nenhuma clínica ativa. Faça login novamente.')
      }

      // Obter userId
      const rawUser = localStorage.getItem('luzaum-user') ? const user = rawUser  JSON.parse(rawUser) : null
      const userId = user.id

      console.log('[Catalogo2] STEP 1: Context:', { clinicId, userId }) ? if (!userId) {
        throw new Error('Usuário não identificado. Faça login novamente.')
      }

      // Preparar payloads (APENAS campos do Supabase, sem manufacturer/package_*) ? const medicationPayload = {
      ...
      })) ? console.log('[Catalogo2] STEP 2: Payloads') ? console.log('  medication:', medicationPayload) ? console.log('  presentations:', presentationsPayload)
      ...
      ) ? console.log('[Catalogo2] STEP 3: Calling saveMedication...') ? const result = await saveMedication({
        clinicId,
        userId,
        medication: medicationPayload,
        medicationId: draft.id || undefined,
        presentations: presentationsPayload
      }) ? console.log('[Catalogo2] STEP 4: Supabase results') ? console.log('  medication:', result.medication) ? console.log('  presentations:', result.presentations) ? console.log('  Saved medication ID:', result.medication.id)

      // Verificação post-save
      console.log('[Catalogo2] STEP 5: POST-SAVE VERIFY (SELECT)') ? const verifyMed = await getMedicationDetails(clinicId, result.medication.id) ? const verifyPres = await getMedicationPresentations(clinicId, result.medication.id) ? console.log('  Verify medication:', verifyMed) ? console.log('  Verify presentations:', verifyPres) ? if (!verifyMed) {
        console.error('[Catalogo2] ⚠️ POST-SAVE VERIFY FAILED: medication NOT FOUND!') ? throw new Error('Medicamento não foi encontrado no banco após salvar. Verifique RLS policies.')
      } else {
      ...
      // Selecionar o salvo
      const savedMed = await getMedicationDetails(clinicId, result.medication.id) ? const savedPres = await getMedicationPresentations(clinicId, result.medication.id) ? const fullSavedMed: MedicationWithPresentations = {
        ...savedMed!,
      ...
      setSelectedId(result.medication.id) ? setDraft(JSON.parse(JSON.stringify(fullSavedMed))) ? setOriginalDraft(JSON.parse(JSON.stringify(fullSavedMed))) ? setIsSaving(false) ? setSaveSuccess(true) ? setTimeout(() => setSaveSuccess(false), 3000) ? console.log('[Catalogo2] ========== SAVE SUCCESS ==========')
    } catch (error: any) {
      console.error('[Catalogo2] ========== SAVE ERROR ==========') ? console.error('  Error object:', error) ? console.error('  message:', error.message) ? console.error('  details:', error.details) ? console.error('  hint:', error.hint) ? console.error('  code:', error.code)
      ...
      const isRLS =
        ...
        error.message.toLowerCase().includes('permission denied') ? if (isRLS) {
        setErrors({
                ...
                <div>
                  <label className="text-sm">
                    ...
                    <input
                      ...
                      value={draft.name}
                      onChange={(e) => {
                        setDraft((prev) => ({ ...prev, name: e.target.value })) ? if (errors.name) setErrors((prev) => ({ ...prev, name: '' }))
                      }}
                    ...
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-12">
                      ...
                      <label className="text-xs md:col-span-3">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const val = e.target.value === ''  null : Number(e.target.value) ? updatePresentation(pres.client_id, { avg_price_brl: val })
                          }}
                          placeholder="79.90"
                        />
                      </label>
                      ...
                      <label className="text-xs md:col-span-2">
                        ...
                        <input
                          ...
                          value={pres.value  ''}
                          onChange={(e) => {
                            const val = e.target.value === ''  null : Number(e.target.value) ? updatePresentation(pres.client_id, { value: val })
                          }}
                          placeholder="250"
                        />
                      </label>
                      ...
                      <label className="text-xs md:col-span-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const val = e.target.value === ''  1 : Number(e.target.value) ? updatePresentation(pres.client_id, { per_value: val })
                          }}
                        />
                      </label>
                      ...
                      <label className="text-xs md:col-span-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const val = e.target.value === ''  undefined : Number(e.target.value) ? updatePresentation(pres.client_id, { package_quantity: val })
                          }}
                          placeholder="10"
                        />
                      </label>
