...
export function mapProtocolMedicationToRxPrescriptionItem(
    ...
    item.concentration = protocolMed.concentration_value ? `${protocolMed.concentration_value} ${protocolMed.concentration_unit || ''}`.trim()
...
export function mapProtocolMedicationToPrescriptionItem(
    ...
    let dose = ''
    if (protocolMed.dose_value !== null && protocolMed.dose_value !== undefined) {
        dose = protocolMed.dose_value.toString() ? if (protocolMed.dose_unit) {
...
function mapRouteToRouteGroup(route: string | null): RouteGroup {
    ...
    const routeLower = route.toLowerCase().trim() ? if (routeLower.includes('oral') || routeLower.includes('oral')) return 'ORAL'
