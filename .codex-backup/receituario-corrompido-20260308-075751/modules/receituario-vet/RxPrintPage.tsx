...
function buildZoneDefaultText(
  ...
  if (zone === 'recommendations') {
    ...
    if (doc.recommendations.length) {
      lines.push('RECOMENDAÇÕES:') ? doc.recommendations.forEach((line) => lines.push(`- ${line}`))
    }
    if (doc.exams.length) {
      if (lines.length) lines.push('') ? lines.push('EXAMES SUGERIDOS:') ? doc.exams.forEach((line) => lines.push(`- ${line}`))
    }
    if (inlineEditNote.trim()) {
      if (lines.length) lines.push('') ? lines.push('OBSERVAÇÃO ADICIONAL:') ? lines.push(inlineEditNote.trim())
    }
    return lines.join('\n')
  }
...
function normalizeWhatsAppPhone(value: string): string {
  const digits = onlyDigits(value) ? if (!digits) return ''
  if (digits.startsWith('00')) return digits.slice(2) ? if (digits.startsWith('55')) return digits
...
function buildPdfFileBaseName(patientName: string, tutorName: string): string {
  const date = new Date().toLocaleDateString('pt-BR').replace(/\//g, '-') ? return `${sanitizeFilePart(patientName)}_${sanitizeFilePart(tutorName)}_${date}`
}
...
async function buildPdfFileFromPreview(args: {
  ...
  const pdf = new jsPDF({
    ...
    const canvas = await html2canvas(element, {
      ...
      useCORS: true,
      logging: false,
    }) ? const img = canvas.toDataURL('image/png') ? if (idx > 0) pdf.addPage() ? const pageW = pdf.internal.pageSize.getWidth() ? const pageH = pdf.internal.pageSize.getHeight() ? pdf.addImage(img, 'PNG', 0, 0, pageW, pageH, undefined, 'FAST')
  }

  const fileName = `${buildPdfFileBaseName(patientName, tutorName)}.pdf`
  const blob = pdf.output('blob') ? return new File([blob], fileName, { type: 'application/pdf' })
}
...
function patchItemInState(prev: PrescriptionState, itemId: string, updater: (item: PrescriptionItem) => PrescriptionItem): PrescriptionState {
  const idx = prev.items.findIndex((entry) => entry.id === itemId) ? if (idx < 0) return prev

  const now = new Date().toISOString() ? let nextItem = { ...updater(prev.items[idx]), updatedAt: now }
  let nextItems = prev.items.map((entry) => (entry.id === itemId  nextItem : entry)) ? if (nextItem.autoInstruction && !nextItem.manualEdited) {
...
export default function RxPrintPage() {
  const location = useLocation() ? const { id } = useParams<{ id: string }>() ? const mode = useMemo(() => new URLSearchParams(location.search).get('mode') || 'final', [location.search]) ? const isReviewMode = mode === 'review'

  const db = useMemo(() => loadRxDb(), []) ? const [prescription, setPrescription] = useState<PrescriptionState>(() => {
    const draft = loadRxDraft() ? if (draft && (!id || draft.id === id)) return draft
    return createDefaultPrescriptionState()
  }) ? const [selection, setSelection] = useState<ReviewSelection>(null) ? const rawActiveTemplate = useMemo(() => {
    ...
    const found = preferredStandardId
      ...
      : db.templates.find((entry) => entry.id === db.activeTemplateId) ? return found || db.templates[0]
  }, [db.activeTemplateId, db.templates, prescription.recommendations.standardTemplateId]) ? const activeTemplate = useMemo(() => {
  ...
  }, [db.templates, rawActiveTemplate]) ? const selectedPrescriberProfile = useMemo(
  ...
  ) ? const [activeDocIndex, setActiveDocIndex] = useState(0) ? const [docViewMode, setDocViewMode] = useState<'single' | 'side-by-side'>('single') ? const [inlineEditNote, setInlineEditNote] = useState('') ? const [zoneTextOverrides, setZoneTextOverrides] = useState<ZoneTextOverrideState>({
    standard: {},
    'special-control': {},
  }) ? const [toast, setToast] = useState<string | null>(null) ? const [share, setShare] = useState<ShareState>({
    open: false,
    ...
    includeBcc: false,
  }) ? const previewSheetRefs = useRef<Array<HTMLDivElement | null>>([]) ? useEffect(() => {
    if (!isReviewMode) return
    const timeout = window.setTimeout(() => saveRxDraft(prescription), 350) ? return () => window.clearTimeout(timeout)
  }, [isReviewMode, prescription]) ? const splitDocs = useMemo(() => splitPrescriptionByControl(prescription), [prescription]) ? const hasSpecialControlDocs = useMemo(
    () => (splitDocs.specialControl.items.length || 0) > 0,
    [splitDocs.specialControl]
  ) ? const selectableTemplates = useMemo(() => {
    const list = db.templates.filter((entry) => entry.documentKindTarget !== 'special-control') ? return list.length  list : [activeTemplate]
  }, [activeTemplate, db.templates]) ? const specialSelectableTemplates = useMemo(() => {
    const list = db.templates.filter((entry) => entry.documentKindTarget === 'special-control') ? return list.length  list : db.templates
  }, [db.templates]) ? const printDocs = useMemo(() => {
    const renderMode = mode === 'template' ? 'template' : 'final'
    const docs: Array<ReturnType<typeof renderRxToPrintDoc>> = []
    if (splitDocs.standard) docs.push(renderRxToPrintDoc(splitDocs.standard, { renderMode, documentKind: 'standard' })) ? if (splitDocs.specialControl) docs.push(renderRxToPrintDoc(splitDocs.specialControl, { renderMode, documentKind: 'special-control' })) ? if (!docs.length) docs.push(renderRxToPrintDoc(prescription, { renderMode, documentKind: 'standard' })) ? return docs
  }, [mode, prescription, splitDocs.specialControl, splitDocs.standard]) ? const specialControlTemplate = useMemo(() => {
    ...
    const fixed =
      ...
      db.templates.find((entry) => entry.id === 'rx_br_control_special') ? return fixed || templateDraft
  }, [db.templates, prescription.recommendations.specialControlTemplateId, templateDraft]) ? const specialControlTargetPharmacy = useMemo<SpecialControlPharmacy>(() => {
    ...
    return 'veterinária'
  }, [prescription.recommendations.specialControlPharmacy]) ? const templateForDoc = (doc: ReturnType<typeof renderRxToPrintDoc>) => {
  ...
  const activeDocKind = resolveDocKindKey(activeDoc) ? const activeDocLabel = activeDoc.documentKind === 'special-control' ? 'Receita de controle especial' : 'Receita padrão'
  const specialDocIndex = useMemo(() => printDocs.findIndex((entry) => entry.documentKind === 'special-control'), [printDocs]) ? const activeDocItemIds = useMemo(() => {
  ...
  }, [activeDocIndex, printDocs.length]) ? useEffect(() => {
    if (printDocs.length > 1) return
    setDocViewMode('single')
  }, [printDocs.length]) ? useEffect(() => {
    if (hasSpecialControlDocs) return
    if (templateDraft.documentKindTarget !== 'special-control') return
    setTemplateDraft(activeTemplate)
  }, [activeTemplate, hasSpecialControlDocs, templateDraft.documentKindTarget]) ? useEffect(() => {
    if (!isReviewMode) return
    const validItemIds = activeDocItemIds.filter((entryId) => prescription.items.some((entry) => entry.id === entryId)) ? if (selection.kind === 'item' && validItemIds.includes(selection.itemId)) return
    if (validItemIds.length > 0) {
      setSelection({ kind: 'item', itemId: validItemIds[0] }) ? return
    }
    setSelection({ kind: 'zone', zone: 'body' })
  }, [activeDocItemIds, isReviewMode, prescription.items, selection]) ? const selectedItem = useMemo(() => {
    if (selection.kind !== 'item') return null
    return prescription.items.find((entry) => entry.id === selection.itemId) || null
  }, [prescription.items, selection]) ? const activeZoneOverrideText =
  ...
  }, [activeDoc, inlineEditNote, selection]) ? const pushToast = (message: string) => {
  ...
  const saveReviewDraft = () => {
    try {
      saveRxDraft(prescription) ? let nextDb = upsertPatientFromPrescription(loadRxDb(), prescription) ? nextDb = upsertClientFromPrescription(nextDb, prescription) ? nextDb = upsertHistoryFromPrescription(nextDb, prescription) ? saveRxDb(nextDb) ? pushToast('Rascunho salvo com sucesso.')
    } catch {
  ...
  const saveTemplateDraft = () => {
    const nextDb = upsertTemplate(loadRxDb(), { ...templateDraft, updatedAt: new Date().toISOString() }) ? const activated = setActiveTemplate(nextDb, templateDraft.id) ? saveRxDb(activated) ? pushToast('Template salvo com sucesso.')
  }
  ...
  const buildPdfFile = async () => {
    const elements = printDocs
      ...
      .filter((entry): entry is HTMLElement => !!entry) ? if (!elements.length) throw new Error('Preview não disponível para exportação.') ? const standard = templateDraft.documentKindTarget === 'special-control'  activeTemplate : templateDraft
  ...
  const downloadPdf = async () => {
    const file = await buildPdfFile() ? triggerFileDownload(file) ? pushToast(`PDF gerado: ${file.name}`)
  }

  const openWhatsApp = async () => {
    const phone = normalizeWhatsAppPhone(share.whatsappPhone) ? if (!phone) {
      pushToast('Informe o telefone para WhatsApp.') ? return
    }

    const message = (share.whatsappMessage || '').trim() ? const query = message ? `text=${encodeURIComponent(message)}` : ''
    window.open(`https://wa.me/${phone}${query}`, '_blank', 'noopener,noreferrer') ? pushToast('WhatsApp aberto direto na conversa para revisão da mensagem.') ? try {
      const file = await buildPdfFile() ? triggerFileDownload(file) ? pushToast('PDF baixado para anexar na conversa do WhatsApp.')
    } catch {
  ...
  const openEmail = async () => {
    const file = await buildPdfFile() ? const body = share.whatsappMessage
    const shared = await tryShareFile(file, share.emailSubject, body) ? if (shared) {
      pushToast('Arquivo compartilhado com anexo.') ? return
    }

    triggerFileDownload(file) ? const bcc = share.includeBcc ? '&bcc=clínica@vetius.local' : ''
  ...
  const openGmail = async () => {
    const file = await buildPdfFile() ? triggerFileDownload(file) ? const body = share.whatsappMessage
    const gmailUrl = `https://mail.google.com/mail/view=cm&fs=1&to=${encodeURIComponent(share.email || '')}&su=${encodeURIComponent(
      share.emailSubject
    )}&body=${encodeURIComponent(body)}`
    window.open(gmailUrl, '_blank', 'noopener,noreferrer') ? pushToast('Gmail aberto com mensagem pronta. PDF baixado para anexar.')
  }

  const openOutlook = async () => {
    const file = await buildPdfFile() ? triggerFileDownload(file) ? const body = share.whatsappMessage
    const outlookUrl = `https://outlook.office.com/mail/deeplink/composeto=${encodeURIComponent(share.email || '')}&subject=${encodeURIComponent(
      share.emailSubject
    )}&body=${encodeURIComponent(body)}`
    window.open(outlookUrl, '_blank', 'noopener,noreferrer') ? pushToast('Outlook aberto com mensagem pronta. PDF baixado para anexar.')
  }
                ...
                {printDocs.map((doc, idx) => (
                  ...
                  >
                    <RxPrintView
                      ...
                      onZoneSelect={(zone) => {
                        setActiveDocIndex(idx) ? setSelection({ kind: 'zone', zone })
                      }}
                      onItemSelect={(itemId) => {
                        setActiveDocIndex(idx) ? setSelection({ kind: 'item', itemId })
                      }}
          ...
          <div className="flex flex-col gap-4">
              ...
              <>
                ...
                {specialDocIndex >= 0 ? (
                  <button
                    ...
                    onClick={() => {
                      setActiveDocIndex(specialDocIndex) ? setSelection({ kind: 'zone', zone: 'body' })
                    }}
                  >
            ...
            <label className="block text-xs text-slate-400">Template
              <select
                ...
                onChange={(e) => {
                  const selected = selectableTemplates.find((entry) => entry.id === e.target.value) ? if (!selected) return
                  setTemplateDraft(selected) ? updatePrescription((prev) => ({
                    ...prev,
                    recommendations: {
                      ...prev.recommendations,
