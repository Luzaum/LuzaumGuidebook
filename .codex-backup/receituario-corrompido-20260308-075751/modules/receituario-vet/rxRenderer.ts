...
const DISCRETE_UNITS = new Set(['comprimido', 'capsula', 'gota', 'ampola', 'frasco', 'bisnaga', 'unidade', 'sachê', 'sache']) ? function normalizeText(value: string): string {
  return (value || '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
...
function normalizeUnit(unit: string): string {
  const key = normalizeText(unit).replace(/\./g, '') ? return UNIT_ALIASES[key] || key
}
...
function toNumber(raw: unknown): number | null {
  ...
  if (typeof raw === 'string') {
    ...
    const n = Number(normalized) ? return Number.isNaN(n) ? null : n
  }
  if (import.meta.env.DEV) {
    console.warn('[rxRenderer] toNumber unexpected type', typeof raw, raw)
  }
  const fallback = Number(String(raw)) ? return Number.isNaN(fallback) ? null : fallback
}
...
function inferPresentationUnit(presentation: string): string {
  const text = normalizeText(presentation) ? if (text.includes('comprim')) return 'comprimido'
  if (text.includes('capsul')) return 'capsula'
  if (text.includes('gota')) return 'gota'
  if ((text.includes('solu') && text.includes('oral')) || text.includes('suspens') || text.includes('xarope')) return 'ml'
...
function formatDisplayUnit(unit: string): string {
  const normalized = normalizeUnit(unit) ? const map: Record<string, string> = {
    capsula: 'cápsula',
    comprimido: 'comprimido',
    gota: 'gota',
    ampola: 'ampola',
...
function formatDoseWithUnit(value: number, unit: string): string {
  const normalized = normalizeUnit(unit) ? const singular = formatDisplayUnit(unit) ? const n = Math.abs(value) ? const isSingular = Math.abs(n - 1) < 0.001
  if (isSingular) return `${formatNumber(value)} ${singular}`
  ...
  const plural = pluralMap[normalized] || singular
  return `${formatNumber(value)} ${plural}`
}

function formatPresentationLabel(presentation: string): string {
  const raw = (presentation || '').trim() ? if (!raw) return ''
  const normalized = normalizeText(raw) ? const known: Record<string, string> = {
    capsula: 'Cápsula',
    comprimido: 'Comprimido',
    gotas: 'Gotas',
    gota: 'Gota',
...
function getDoseBaseUnit(unit: string) {
  const parts = (unit || '').split('/') ? return (parts[0] || '').trim()
}
...
function parseConcentration(concentration: string, presentation: string): ParsedConcentration | null {
  const text = (concentration || '').trim() ? if (!text) return null
  const withDivider = text.match(/(\d+(:[.,]\d+))\s*([a-zA-Z%µ]+)\s*\/\s*(\d+(:[.,]\d+))\s*([a-zA-Z%µ]+)/) ? if (withDivider) {
  ...
  const simple = text.match(/(\d+(:[.,]\d+))\s*([a-zA-Z%µ]+)/) ? if (!simple) return null
  const amount = toNumber(simple[1]) || 0
  const amountUnit = simple[2]
  const perValue = 1
  const perUnit = inferPresentationUnit(presentation) ? if (amount <= 0) return null
  return { amount, amountUnit, perValue, perUnit }
}
...
export function resolveFrequency(item: PrescriptionItem): {
  ...
  if (item.frequencyType === 'everyHours') {
    const hours = toNumber(item.everyHours) ? if (!hours || hours <= 0) return { normalizedTimesPerDay: null, label: 'frequência não informada' }
    return { normalizedTimesPerDay: 24 / hours, label: `a cada ${formatNumber(hours, 0)} horas` }
  }

  const times = toNumber(item.timesPerDay) ? if (!times || times <= 0) return { normalizedTimesPerDay: null, label: 'frequência não informada' }
  return {
    normalizedTimesPerDay: times,
...
function frequencyLabelForTutor(item: PrescriptionItem, fallback: string): string {
  if (item.frequencyType === 'everyHours') {
    const hours = toNumber(item.everyHours) ? if (hours && hours > 0) return `a cada ${formatNumber(hours, 0)} horas`
    return fallback
  }

  const tokenTimes = item.frequencyToken  FREQUENCY_TOKEN_TO_TIMES[item.frequencyToken] : null
  const numericTimes = tokenTimes || toNumber(item.timesPerDay) ? if (!numericTimes || numericTimes <= 0) return fallback
  ...
  if (Number.isFinite(interval) && Math.abs(Math.round(interval) - interval) < 0.001) {
    const roundedInterval = Math.round(interval) ? return roundedInterval === 1 ? 'a cada 1 hora' : `a cada ${formatNumber(roundedInterval, 0)} horas`
  }
...
function uniqueByNormalizedText(lines: string[]): string[] {
  ...
  for (const line of lines) {
    ...
    const key = normalizeText(trimmed) ? if (!key || seen.has(key)) continue
...
export function calculateMedicationQuantity(item: PrescriptionItem, state: PrescriptionState): QuantityResult {
  const steps: string[] = []
  const doseValue = toNumber(item.doseValue) ? if (!doseValue || doseValue <= 0) {
    return {
      perDose: null,
      total: null,
      unit: '',
  ...
  const weightKg = toNumber(state.patient.weightKg) ? const doseUnitRaw = (item.doseUnit || '').trim() ? const dosePerKg = isDosePerKg(doseUnitRaw) ? const baseUnitRaw = dosePerKg  getDoseBaseUnit(doseUnitRaw) : doseUnitRaw
  const baseUnitNorm = normalizeUnit(baseUnitRaw) ? let baseDosePerAdministration = doseValue
  if (dosePerKg) {
  ...
  const concentration = parseConcentration(item.concentration, item.presentation) ? let perDoseOutput = baseDosePerAdministration
  let perDoseOutputUnit = baseUnitRaw || inferPresentationUnit(item.presentation) ? if (concentration) {
    const concentrationAmountUnitNorm = normalizeUnit(concentration.amountUnit) ? if (concentrationAmountUnitNorm === baseUnitNorm && concentration.amount > 0 && concentration.perValue > 0) {
      const ratio = concentration.amount / concentration.perValue
      perDoseOutput = baseDosePerAdministration / ratio
      perDoseOutputUnit = concentration.perUnit || inferPresentationUnit(item.presentation) ? steps.push(
  ...
  const durationDays = toNumber(item.durationDays) ? let total: number | null = null
  ...
  const displayUnit = formatDisplayUnit(perDoseOutputUnit) ? const label = roundedTotal !== null ? `Total estimado: ${formatDoseWithUnit(roundedTotal, displayUnit)}`
    : `Por dose: ${formatDoseWithUnit(perDoseOutput, displayUnit)}`
...
export function buildCalculationMemory(item: PrescriptionItem, state: PrescriptionState): string[] {
  const result = calculateMedicationQuantity(item, state) ? return result.memory.steps
}

export function buildAutoInstruction(item: PrescriptionItem, state: PrescriptionState): string {
  const freq = resolveFrequency(item) ? const qty = calculateMedicationQuantity(item, state) ? const doseValue = toNumber(item.doseValue) ? const doseUnit = (item.doseUnit || '').trim() ? const weightKg = toNumber(state.patient.weightKg) ? const baseDoseUnit = doseUnit.includes('/kg') ? getDoseBaseUnit(doseUnit) : doseUnit

  let perDoseValueText = ''
  if (qty.perDose !== null && qty.unit) {
    perDoseValueText = formatDoseWithUnit(qty.perDose, qty.unit)
  } else if (doseValue && doseUnit) {
    if (isDosePerKg(doseUnit) && weightKg) perDoseValueText = formatDoseWithUnit(doseValue * weightKg, baseDoseUnit) ? else perDoseValueText = formatDoseWithUnit(doseValue, doseUnit)
  }
  ...
  const frequencySegment =
    freq.label === 'frequência não informada' ? 'conforme frequência clínica' : frequencyLabelForTutor(item, freq.label) ? let durationSegment = ''
  if (item.continuousUse) durationSegment = 'com uso contínuo até reavaliação do paciente'
    ...
    const days = toNumber(item.durationDays) ? if (days && days > 0) durationSegment = `durante ${formatNumber(days, 0)} dias`
  }
...
function resolveInstruction(item: PrescriptionItem, state: PrescriptionState): string {
  if (item.autoInstruction && !item.manualEdited && state) return buildAutoInstruction(item, state) ? return item.instruction
}
...
function buildItemTitle(item: PrescriptionItem): string {
  const base = [item.name, item.concentration].filter(Boolean).join(' ').trim() ? if (!base) return ''
...
export function splitPrescriptionByControl(state: PrescriptionState): {
  ...
  const catalogControlledById = new Map<string, boolean>() ? try {
    const db = loadRxDb() ? db.catalog.forEach((drug) => {
  ...
  const isControlledItem = (item: PrescriptionItem) =>
    !!item.controlled || (!!item.catalogDrugId && catalogControlledById.get(item.catalogDrugId) === true) ? const controlledItems = state.items.filter((item) => isControlledItem(item)) ? const standardItems = state.items.filter((item) => !isControlledItem(item)) ? if (!controlledItems.length) {
...
export function renderRxToPrintDoc(
  ...
  const grouped = new Map<RouteGroup, PrescriptionItem[]>() ? for (const key of SECTION_ORDER) grouped.set(key, []) ? for (const item of state.items) {
    const key = item.routeGroup || 'OUTROS'
    const current = grouped.get(key) || []
    current.push(item) ? grouped.set(key, current)
  }
  ...
  const sections = SECTION_ORDER
    .map((key) => {
      ...
      const items: PrintDocItem[] = source.map((item, idx) => {
        const qty = calculateMedicationQuantity(item, state) ? const instruction = resolveInstruction(item, state) ? const subtitleParts = [formatPresentationLabel(item.presentation)]
        ...
        if (qty.label !== 'Quantidade não calculada') {
          ...
          const missingWeight = (item.doseUnit || '').includes('/kg') && !toNumber(state.patient.weightKg) ? subtitleParts.push(missingWeight ? 'Peso não informado para cálculo' : 'Sem concentração para cálculo')
        }
        return {
          id: item.id,
          index: idx + 1,
  ...
  const recommendations =
    documentKind === 'special-control'
      ...
      : [
        ...
        ...(state.recommendations.waterMlPerDay.trim() ? [`Meta hídrica diária: ${state.recommendations.waterMlPerDay} mL/dia.`]
          : []),
      ]
  ...
  if (state.patient.breed.trim()) patientParts.push(state.patient.breed) ? if (state.patient.ageText.trim()) patientParts.push(state.patient.ageText) ? const tutorName = state.tutor.name || state.tutor.fullName || (state.tutor as any).full_name || '-'
  const tutorLineParts: string[] = [tutorName]
  if (state.tutor.cpf.trim()) tutorLineParts.push(`CPF: ${state.tutor.cpf.trim()}`) ? if (state.tutor.rg.trim()) tutorLineParts.push(`RG: ${state.tutor.rg.trim()}`) ? const tutorAddressLine = [
    state.tutor.street || '',
    state.tutor.number || '',
