...
function itemFromDrug(drug: CatalogDrug, presentationId: string) {
  const base = createDefaultItem('medication', drug.routeGroup) ? const presentation = drug.presentations.find((entry) => entry.id === presentationId) || drug.presentations[0]
  return {
    ...base,
...
function toNumber(raw: string): number | null {
  const normalized = raw.replace(',', '.').trim() ? if (!normalized) return null
  const parsed = Number(normalized) ? if (Number.isNaN(parsed)) return null
  return parsed
}

function simulationGaugePercent(value: number | null): number {
  if (value === null || !Number.isFinite(value) || value <= 0) return 10
  const normalized = Math.log10(value + 1) / Math.log10(101) ? return Math.max(10, Math.min(100, Math.round(normalized * 100)))
}

function joinConcentration(value: string, unit: string): string {
  const normalizedValue = value.trim() ? const normalizedUnit = unit.trim() ? if (!normalizedValue) return ''
...
function splitConcentration(raw: string): { value: string; unit: string } {
  const normalized = raw.trim() ? if (!normalized) return { value: '', unit: DEFAULT_CONCENTRATION_UNIT }
  ...
  const value = normalized.slice(0, normalized.length - foundUnit.length).trim() ? return {
    value,
    unit: foundUnit,
  }
}
...
function createManualProtocolItemDraft(): RxProtocol['items'][number] {
  const base = createDefaultItem('medication', 'ORAL') ? return {
    ...base,
    name: '',
    presentation: '',
    concentration: '',
...
function safeRouteGroup(value: unknown): RouteGroup {
  const raw = String(value || '').trim().toUpperCase() ? if (
    raw === 'ORAL' ||
    raw === 'OTOLOGICO' ||
    raw === 'OFTALMICO' ||
    raw === 'TOPICO' ||
...
function normalizeExamKey(label: string, idx: number) {
  const base = String(label || '')
    ...
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '') ? return (base || `exam_${idx + 1}`).slice(0, 64)
}
...
function mapBundleToRxProtocol(bundle: ProtocolBundle, fallbackFolderId: string): RxProtocol {
  ...
  const now = new Date().toISOString() ? const items = (bundle.medications || []).map((m, idx) => {
    const base = createDefaultItem('medication', safeRouteGroup(m.route)) ? const name = String(m.manual_medication_name || m.medication_name || '').trim() ? const presentation = String(m.manual_presentation_label || m.presentation_text || '').trim() ? const frequencyType: RxProtocol['items'][number]['frequencyType'] =
...
function mapRxItemToProtocolMedication(item: RxProtocol['items'][number], idx: number): ProtocolMedicationItem {
  const manualName = String(item.name || '').trim() ? const manualPresentation = String(item.presentation || '').trim() ? return {
    medication_id: null,
    medication_name: '',
...
export default function ProtocolosPage() {
  const navigate = useNavigate() ? const { clinicId } = useClinic() ? const rxDataSource = useMemo(() => resolveRxDataSource(import.meta.env.VITE_RX_DATA_SOURCE), []) ? const supabaseMode = rxDataSource === 'supabase'
  const [sbUserId, setSbUserId] = useState<string | null>(null) ? const [sbLoading, setSbLoading] = useState(false) ? const initialDb = useMemo(() => loadRxDb(), []) ? const [protocols, setProtocols] = useState<RxProtocol[]>(initialDb.protocols) ? const [folders, setFolders] = useState<RxProtocolFolder[]>(orderFolders(initialDb.protocolFolders)) ? const [selectedFolderId, setSelectedFolderId] = useState('all') ? const [selectedProtocolId, setSelectedProtocolId] = useState(initialDb.protocols[0].id || '') ? const [search, setSearch] = useState('') ? const [saved, setSaved] = useState(false) ? const [folderModalOpen, setFolderModalOpen] = useState(false) ? const [newFolderName, setNewFolderName] = useState('') ? const [newFolderColor, setNewFolderColor] = useState('#39ff14') ? const [customExamDraft, setCustomExamDraft] = useState('') ? const [examReasonDraft, setExamReasonDraft] = useState('') ? const [selectedDrugId, setSelectedDrugId] = useState(initialDb.catalog[0].id || '') ? const [selectedDrugPresentationId, setSelectedDrugPresentationId] = useState(initialDb.catalog[0].presentations[0].id || '') ? const [addItemModalOpen, setAddItemModalOpen] = useState(false) ? const [addItemMode, setAddItemMode] = useState<'catalogo' | 'manual'>('catalogo') ? const [manualItemDraft, setManualItemDraft] = useState(() => createManualProtocolItemDraft()) ? const [manualConcentrationValue, setManualConcentrationValue] = useState('') ? const [manualConcentrationUnit, setManualConcentrationUnit] = useState(DEFAULT_CONCENTRATION_UNIT) ? const [simulationWeightKg, setSimulationWeightKg] = useState(10) ? const [simulationWeightInput, setSimulationWeightInput] = useState('10') ? const [rxTheme, setRxTheme] = useState<'dark' | 'light'>(() => {
    try {
      return localStorage.getItem('receituario-vet:theme:v1') === 'light' ? 'light' : 'dark'
    } catch { return 'dark' }
  }) ? const isDark = rxTheme === 'dark'
  const [draft, setDraft] = useState<RxProtocol>(() => initialDb.protocols[0]  cloneProtocol(initialDb.protocols[0]) : createEmptyProtocol(initialDb.protocolFolders[0].id)) ? const folderMap = useMemo(() => {
    const map = new Map<string, RxProtocolFolder>() ? folders.forEach((folder) => map.set(folder.id, folder)) ? return map
  }, [folders]) ? const visibleProtocols = useMemo(() => {
    return [...protocols]
  ...
  }, [protocols, selectedFolderId, search]) ? const selectedDrug = useMemo(
    ...
    const exists = selectedDrug.presentations.some((entry) => entry.id === selectedDrugPresentationId) ? if (!exists) {
      setSelectedDrugPresentationId(selectedDrug.presentations[0].id || '')
    }
  }, [selectedDrug, selectedDrugPresentationId]) ? useEffect(() => {
    if (!supabaseMode) return
    const targetClinicId = String(clinicId || '').trim() ? if (!targetClinicId) return

    let cancelled = false
    setSbLoading(true) ? void (async () => {
      const { data, error } = await supabase.auth.getUser() ? if (cancelled) return
      if (error || !data.user.id) {
        setSbUserId(null) ? return
      }
      const userId = data.user.id
      setSbUserId(userId) ? let sbFolders = await sbListFolders(targetClinicId, userId) ? if (sbFolders.length === 0) {
        await sbCreateFolder(targetClinicId, userId, { name: 'Geral', icon_key: 'folder', sort_order: 0 }) ? sbFolders = await sbListFolders(targetClinicId, userId)
      }
      ...
      })) ? const orderedFolders = orderFolders(mappedFolders) ? const fallbackFolderId = orderedFolders[0].id || ''
      setFolders(orderedFolders) ? const sbProtocols = await sbListProtocols(targetClinicId, userId) ? const mappedProtocols: RxProtocol[] = sbProtocols.map((p) => ({
        id: p.id,
        name: p.name,
      ...
      })) ? setProtocols(mappedProtocols) ? const initialId = mappedProtocols[0].id || ''
      setSelectedProtocolId(initialId) ? if (initialId && isUuid(initialId)) {
        const bundle = await sbLoadProtocolBundle(targetClinicId, userId, initialId) ? if (bundle) {
          const full = mapBundleToRxProtocol(bundle, fallbackFolderId) ? setDraft(full) ? setProtocols((prev) => prev.map((entry) => (entry.id === full.id  full : entry))) ? return
        }
      }

      if (fallbackFolderId) {
        setDraft(emptyProtocolDraft(fallbackFolderId)) ? return
      }
    })()
      .catch((err) => {
        if (cancelled) return
  ...
  }, [supabaseMode, clinicId]) ? const writeDb = (nextDb: RxDatabase) => {
    if (supabaseMode) return
    saveRxDb(nextDb) ? setProtocols(nextDb.protocols) ? setFolders(orderFolders(nextDb.protocolFolders))
  }
  ...
  const selectProtocol = async (protocol: RxProtocol) => {
    setSelectedProtocolId(protocol.id) ? if (!supabaseMode) {
      setDraft(cloneProtocol(protocol)) ? return
    }

    const targetClinicId = String(clinicId || '').trim() ? const userId = String(sbUserId || '').trim() ? if (!targetClinicId || !userId || !isUuid(protocol.id)) {
      setDraft(cloneProtocol(protocol)) ? return
    }

    setSbLoading(true) ? try {
      const bundle = await sbLoadProtocolBundle(targetClinicId, userId, protocol.id) ? if (!bundle) {
        setDraft(cloneProtocol(protocol)) ? return
      }
      const fallbackFolderId = folders[0].id || protocol.folderId
      const full = mapBundleToRxProtocol(bundle, fallbackFolderId) ? setDraft(full) ? setProtocols((prev) => prev.map((entry) => (entry.id === full.id  full : entry)))
    } catch (err) {
      const message = err instanceof Error  err.message : 'Falha ao carregar protocolo.'
      alert(message) ? setDraft(cloneProtocol(protocol))
    } finally {
      setSbLoading(false)
    }
  }
  ...
  const saveDraft = async () => {
    const normalizedFolderId = folderMap.has(draft.folderId) ? draft.folderId
      : folders[0].id || draft.folderId

    if (!supabaseMode) {
      const base = loadRxDb() ? const nextDb = upsertProtocol(base, {
        ...draft,
      ...
      }) ? writeDb(nextDb) ? setSelectedProtocolId(draft.id) ? pushSaved() ? return
    }

    const targetClinicId = String(clinicId || '').trim() ? const userId = String(sbUserId || '').trim() ? if (!targetClinicId || !userId) {
      alert('Clínica ativa ou usuário não encontrado para salvar protocolo no Supabase.') ? return
    }

    setSbLoading(true) ? try {
      const meds = draft.items.map((item, idx) => mapRxItemToProtocolMedication(item, idx)) ? const recs = draft.recommendations
        .map((line, idx) => ({ text: String(line || '').trim(), sort_order: idx }))
        .filter((r) => !!r.text) ? const savedProtocol = await sbSaveProtocolBundle(targetClinicId, userId, {
        protocol: {
      ...
      }) ? pushSaved() ? const refreshed = await sbListProtocols(targetClinicId, userId) ? const fallbackFolderId = folders[0].id || normalizedFolderId
      const mappedProtocols: RxProtocol[] = refreshed.map((p) => ({
        ...
        createdAt: p.created_at,
        updatedAt: p.updated_at || p.created_at,
      })) ? setProtocols(mappedProtocols) ? setSelectedProtocolId(savedProtocol.id) ? const bundle = await sbLoadProtocolBundle(targetClinicId, userId, savedProtocol.id) ? if (bundle) {
        const full = mapBundleToRxProtocol(bundle, fallbackFolderId) ? setDraft(full) ? setProtocols((prev) => prev.map((entry) => (entry.id === full.id  full : entry)))
      } else {
  ...
  const createProtocol = () => {
    ...
    if (!supabaseMode) {
      const next = createEmptyProtocol(folderId) ? setSelectedProtocolId(next.id) ? setDraft(next) ? return
    }

    setSelectedProtocolId('') ? setDraft(emptyProtocolDraft(folderId))
  }

  const deleteProtocolDraft = async () => {
    if (!supabaseMode) {
      const nextDb = removeProtocol(loadRxDb(), draft.id) ? writeDb(nextDb) ? if (nextDb.protocols[0]) {
        setSelectedProtocolId(nextDb.protocols[0].id) ? setDraft(cloneProtocol(nextDb.protocols[0])) ? return
      }
      const empty = createEmptyProtocol(folders[0].id) ? setSelectedProtocolId(empty.id) ? setDraft(empty) ? return
    }

    const targetClinicId = String(clinicId || '').trim() ? const userId = String(sbUserId || '').trim() ? if (!targetClinicId || !userId) {
      alert('Clínica ativa ou usuário não encontrado.') ? return
    }

    if (!isUuid(draft.id)) {
      setSelectedProtocolId('') ? setDraft(emptyProtocolDraft(folders[0].id || draft.folderId)) ? return
    }

    setSbLoading(true) ? try {
      await sbDeleteProtocol(targetClinicId, userId, draft.id) ? const refreshed = await sbListProtocols(targetClinicId, userId) ? const fallbackFolderId = folders[0].id || draft.folderId
      const mappedProtocols: RxProtocol[] = refreshed.map((p) => ({
        id: p.id,
        name: p.name,
        summary: p.description || '',
        ...
        createdAt: p.created_at,
        updatedAt: p.updated_at || p.created_at,
      })) ? setProtocols(mappedProtocols) ? setSelectedProtocolId('') ? setDraft(emptyProtocolDraft(fallbackFolderId))
    } catch (err) {
  ...
  const addItemFromCatalog = () => {
    const drug = initialDb.catalog.find((entry) => entry.id === selectedDrugId) ? if (!drug) return
    setDraft((prev) => ({
      ...prev,
  ...
  const addManualItem = () => {
    ...
    const fresh = createDefaultItem('medication') ? const concentration = joinConcentration(manualConcentrationValue, manualConcentrationUnit) ? setDraft((prev) => ({
      ...prev,
    ...
    })) ? setManualItemDraft(createManualProtocolItemDraft()) ? setManualConcentrationValue('') ? setManualConcentrationUnit(DEFAULT_CONCENTRATION_UNIT) ? setAddItemModalOpen(false)
  }

  const openAddItemModal = () => {
    setAddItemMode('catalogo') ? setManualItemDraft(createManualProtocolItemDraft()) ? setManualConcentrationValue('') ? setManualConcentrationUnit(DEFAULT_CONCENTRATION_UNIT) ? setAddItemModalOpen(true)
  }
  ...
  const addFolder = async () => {
    ...
    if (!supabaseMode) {
      const nextDb = upsertProtocolFolder(loadRxDb(), createProtocolFolder(name, newFolderColor)) ? writeDb(nextDb) ? setNewFolderName('') ? setNewFolderColor('#39ff14') ? return
    }
    ...
    setSbLoading(true) ? try {
      await sbCreateFolder(targetClinicId, userId, { name, icon_key: 'folder', sort_order: 0 }) ? const sbFolders = await sbListFolders(targetClinicId, userId) ? const mappedFolders: RxProtocolFolder[] = sbFolders.map((folder) => ({
        id: folder.id,
        name: folder.name,
        color: '#39ff14',
        icon: folder.icon_key || 'folder',
        sortOrder: folder.sort_order || 0,
      })) ? setFolders(orderFolders(mappedFolders)) ? setNewFolderName('') ? setNewFolderColor('#39ff14')
    } catch (err) {
  ...
  const dropFolder = (folderId: string) => {
    if (supabaseMode) {
      alert('Exclusão de pasta ainda não está habilitada no modo Supabase.') ? return
    }
    const nextDb = removeProtocolFolder(loadRxDb(), folderId) ? writeDb(nextDb) ? if (selectedFolderId === folderId) setSelectedFolderId('all') ? if (draft.folderId === folderId && nextDb.protocolFolders[0]) {
      setDraft((prev) => ({ ...prev, folderId: nextDb.protocolFolders[0].id }))
    }
  }

  const onSimulationWeightSlider = (value: number) => {
    const safe = Math.min(120, Math.max(1, value)) ? setSimulationWeightKg(safe) ? setSimulationWeightInput(String(safe))
  }

  const onSimulationWeightInputChange = (raw: string) => {
    setSimulationWeightInput(raw) ? const parsed = toNumber(raw) ? if (parsed && parsed > 0) {
      setSimulationWeightKg(Math.min(120, Math.max(1, Math.round(parsed))))
    }
  }

  const onSimulationWeightInputBlur = () => {
    const parsed = toNumber(simulationWeightInput) ? if (!parsed || parsed <= 0) {
      setSimulationWeightInput(String(simulationWeightKg)) ? return
    }
    const safe = Math.min(120, Math.max(1, parsed)) ? setSimulationWeightKg(Math.round(safe)) ? setSimulationWeightInput(String(Number(safe.toFixed(2))))
  }

  const simulationState = useMemo(() => {
    const base = createDefaultPrescriptionState() ? return {
      ...base,
      patient: {
        ...base.patient,
  ...
  }, [simulationWeightInput, simulationWeightKg]) ? const applyInNewRecipe = () => {
    // Snapshot (sem link vivo) ? try {
          ...
          <section className="rxv-card rxv-protocol-glass p-5">
            ...
            <div className="space-y-2 rxv-protocol-drug-fields">
              {draft.items.map((item, idx) => {
                const concentrationParts = splitConcentration(item.concentration) ? return (
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setDraft((prev) => ({ ...prev, examReasons: [...prev.examReasons, examReasonDraft.trim()] })) ? setExamReasonDraft('')
                  }}
                >
                  Adicionar
                </button>
          ...
          <section className="rxv-card rxv-protocol-glass p-5">
            ...
            <div className="space-y-2">
              {draft.items.slice(0, 4).map((item) => {
                const quantity = calculateMedicationQuantity(item, simulationState) ? const gauge = simulationGaugePercent(quantity.perDose) ? const gaugeStyle = { ['--rxv-dose-gauge' as '--rxv-dose-gauge']: `${gauge}%` } as React.CSSProperties
                return (
                  <div key={`sim-${item.id}`} className="rxv-protocol-sim-card rounded-lg border border-[color:var(--rxv-border)] bg-[color:var(--rxv-surface-2)]/60 p-3" style={gaugeStyle}>
                    ...
                    <p className="mt-1 text-sm font-semibold text-[#7bf069]">
                      {quantity.perDose !== null ? `${quantity.perDose.toFixed(2)} ${quantity.unit || 'unidade'} por dose`
                        : 'Dose não definida'}
                    </p>
                  ...
                  <label className="rxv-protocol-field text-xs text-[color:var(--rxv-muted)]">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setSelectedDrugId(nextDrugId) ? const firstPresentation = initialDb.catalog.find((entry) => entry.id === nextDrugId).presentations[0].id || ''
