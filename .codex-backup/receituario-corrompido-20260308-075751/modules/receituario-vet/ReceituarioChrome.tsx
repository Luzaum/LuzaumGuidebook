...
function readTheme(): RxTheme {
  try {
    const raw = localStorage.getItem(THEME_KEY) ? return raw === 'light' ? 'light' : 'dark'
  } catch {
    return 'dark'
  }
}
...
function readSidebarUserIdentity(): SidebarUserIdentity {
  try {
    const raw = localStorage.getItem('luzaum-user') ? if (raw) {
      ...
      const nameSource = String(parsed.name || parsed.fullName || parsed.email || '').trim() ? const idSource = String(parsed.id || '').trim() ? return {
...
export default function ReceituarioChrome({
  ...
  const navigate = useNavigate() ? const location = useLocation() ? const [theme, setTheme] = useState<RxTheme>(() => readTheme()) ? const [mobileMenuOpen, setMobileMenuOpen] = useState(false) ? const [sidebarPinned, setSidebarPinned] = useState(true) ? const [sidebarHovered, setSidebarHovered] = useState(false) ? const [sidebarUser] = useState<SidebarUserIdentity>(() => readSidebarUserIdentity()) ? useEffect(() => {
    saveTheme(theme) ? if (onThemeChange) onThemeChange(theme)
  }, [theme, onThemeChange]) ? useEffect(() => {
    if (forcedTheme && forcedTheme !== theme) {
      setTheme(forcedTheme)
    }
  }, [forcedTheme]) ? useEffect(() => {
    setMobileMenuOpen(false)
  }, [location.pathname]) ? useEffect(() => {
    if (!sidebarPinned) setSidebarHovered(false)
  }, [sidebarPinned]) ? const dark = theme === 'dark'
  ...
  }, [dark]) ? const pageClass = dark ? 'rxv-dark' : 'rxv-light'
  ...
  const handleTopbarMenuClick = () => {
    if (typeof window !== 'undefined' && window.matchMedia('(min-width: 1024px)').matches) {
      setSidebarPinned((prev) => !prev) ? setSidebarHovered(false) ? return
    }
