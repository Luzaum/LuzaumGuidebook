...
function readOwnerKey(): string {
  try {
    const raw = (localStorage.getItem(ACCOUNT_KEY) || '').trim() ? if (raw) return raw
  } catch {
    // no-op
  }
...
function normalizeState(raw: Partial<PrescriptionState> | null | undefined): PrescriptionState {
  ...
  const normalizeLoose = (value: string) =>
    (value || '')
      ...
      .replace(/[\u0300-\u036f]/g, '')
      .toLowerCase()
      .trim() ? const parsedSex = normalizeLoose(String(raw.patient.sex || '')) ? const normalizedSex =
  ...
  const parsedReproductive = normalizeLoose(String(raw.patient.reproductiveStatus || '')) ? const normalizedReproductive =
  ...
  return {
    ...
    recommendations: {
      ...
      customExams: Array.isArray(raw.recommendations.customExams) ? raw.recommendations.customExams
        : base.recommendations.customExams,
      examReasons: Array.isArray(raw.recommendations.examReasons) ? raw.recommendations.examReasons
...
function loadSavedDraftEntries(): StoredDraftEntry[] {
  try {
    const raw = localStorage.getItem(SAVED_DRAFTS_KEY) ? if (!raw) return []
    ...
    return parsed
      .map((entry) => {
        ...
        const normalizedState = normalizeState(candidate.state as Partial<PrescriptionState>) ? return {
          id: candidate.id,
...
function upsertSavedDraft(state: PrescriptionState, ownerKey = readOwnerKey()) {
  const safeState = normalizeState(state) ? const all = loadSavedDraftEntries() ? const existing = all.find((entry) => entry.ownerKey === ownerKey && entry.id === safeState.id) ? const updatedAt = nowIso() ? const nextEntry: StoredDraftEntry = {
    id: safeState.id,
    ownerKey,
  ...
  const withoutCurrent = all.filter((entry) => !(entry.ownerKey === ownerKey && entry.id === safeState.id)) ? const merged = [nextEntry, ...withoutCurrent]

  const ownerDrafts = merged
    .filter((entry) => entry.ownerKey === ownerKey)
    .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
    .slice(0, MAX_SAVED_RX_DRAFTS) ? const keepOwnerIdSet = new Set(ownerDrafts.map((entry) => `${entry.ownerKey}:${entry.id}`)) ? const finalList = [...merged.filter((entry) => entry.ownerKey !== ownerKey), ...ownerDrafts].filter((entry) => {
    if (entry.ownerKey !== ownerKey) return true
    return keepOwnerIdSet.has(`${entry.ownerKey}:${entry.id}`)
  }) ? finalList.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()) ? saveSavedDraftEntries(finalList)
}
...
export function loadRxDraftById(draftId: string, ownerKey = readOwnerKey()): PrescriptionState | null {
  const id = (draftId || '').trim() ? if (!id) return null
  const all = loadSavedDraftEntries() ? const scoped = all.find((entry) => entry.ownerKey === ownerKey && entry.id === id) ? if (scoped) return normalizeState(scoped.state) ? const fallback = all.find((entry) => entry.id === id) ? return fallback  normalizeState(fallback.state) : null
}
...
export function removeSavedRxDraft(draftId: string, ownerKey = readOwnerKey()) {
  ...
  const all = loadSavedDraftEntries() ? const next = all.filter((entry) => !(entry.ownerKey === ownerKey && entry.id === id)) ? saveSavedDraftEntries(next)
}
...
export function saveRxDraft(
  ...
  const safeState = normalizeState(state) ? localStorage.setItem(DRAFT_KEY, JSON.stringify(safeState)) ? if (options.persistInSavedList === false) return
