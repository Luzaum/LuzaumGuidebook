...
function readFileAsDataUrl(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    const reader = new FileReader() ? reader.onload = () => resolve(String(reader.result || '')) ? reader.onerror = () => reject(new Error('Não foi possível ler o arquivo selecionado.')) ? reader.readAsDataURL(file)
  })
}

function loadImage(file: File): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    const objectUrl = URL.createObjectURL(file) ? const image = new Image() ? image.onload = () => {
      URL.revokeObjectURL(objectUrl) ? resolve(image)
    }
    image.onerror = () => {
      URL.revokeObjectURL(objectUrl) ? reject(new Error('Não foi possível processar a imagem selecionada.'))
    }
    image.src = objectUrl
  })
}
...
async function optimizeImageDataUrl(file: File): Promise<string> {
  const image = await loadImage(file) ? const maxSide = Math.max(image.width, image.height) ? const scale = maxSide > MAX_UPLOAD_DIMENSION  MAX_UPLOAD_DIMENSION / maxSide : 1
  const width = Math.max(1, Math.round(image.width * scale)) ? const height = Math.max(1, Math.round(image.height * scale)) ? const canvas = document.createElement('canvas') ? canvas.width = width
  canvas.height = height
  const context = canvas.getContext('2d') ? if (!context) {
    throw new Error('Não foi possível preparar a imagem para upload.')
  }

  context.drawImage(image, 0, 0, width, height) ? for (const quality of IMAGE_QUALITY_STEPS) {
    const dataUrl = canvas.toDataURL('image/webp', quality) ? if (dataUrl.length <= MAX_DATA_URL_LENGTH) {
      return dataUrl
    }
  }
...
export default function ProfilePage() {
  ...
  const { clinicId } = useClinic() ? const initialDb = useMemo(() => loadRxDb(), []) ? const initialProfiles = initialDb.prescriberProfiles.length > 0
     initialDb.prescriberProfiles
    : [createPrescriberProfileFromSettings(initialDb.profile, 'Perfil padrão', 'default')]

  const [db, setDb] = useState(initialDb) ? const [profiles, setProfiles] = useState(initialProfiles) ? const [selectedProfileId, setSelectedProfileId] = useState(initialProfiles[0].id || 'default') ? const [profileName, setProfileName] = useState(initialProfiles[0].profileName || 'Perfil padrão') ? const [profile, setProfile] = useState<ProfileSettings>({ ...initialProfiles[0], adminId: 'ADMIN' }) ? const [saved, setSaved] = useState(false) ? const [saveModalOpen, setSaveModalOpen] = useState(false) ? const [saveError, setSaveError] = useState('') ? const [saving, setSaving] = useState(false) ? const currentProfileExists = profiles.some((entry) => entry.id === selectedProfileId) ? const onUpload = (field: 'clinicLogoDataUrl' | 'signatureDataUrl' | 'mapaSignatureDataUrl') => async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files.[0]
    if (!file) return

    setSaveError('') ? try {
      const dataUrl = await toStorableImageDataUrl(file) ? setProfile((prev) => ({ ...prev, [field]: dataUrl }))
    } catch (error) {
  ...
  const loadProfile = (profileId: string) => {
    const selected = profiles.find((entry) => entry.id === profileId) ? if (!selected) return
    setSelectedProfileId(selected.id) ? setProfileName(selected.profileName) ? setProfile({
      adminId: 'ADMIN',
  ...
  const newProfile = () => {
    setSelectedProfileId('new') ? setProfileName('') ? setProfile(emptyProfile()) ? setSaveError('')
  }

  const saveProfile = async () => {
    setSaving(true) ? setSaveError('') ? try {
      const normalizedName = profileName.trim() || 'Perfil sem nome'
      const baseDb = loadRxDb() ? const activeStoredProfile = currentProfileExists
      ...
      for (const field of PROFILE_IMAGE_FIELDS) {
        ...
        // E2: Passar clinicId para que o path comece com UUID (evita erro 22P02) ? const uploaded = await uploadProfileImageDataUrl({
          dataUrl: nextValue,
          field,
          profileId: currentProfileExists  selectedProfileId : normalizedName,
          clinicId: clinicId || undefined,
        }) ? const previousValue = String(activeStoredProfile.[field] || '').trim() ? profileToPersist[field] = uploaded.publicUrl
      ...
      ) ? let nextDb = updateProfile(baseDb, profileToPersist) ? nextDb = upsertPrescriberProfile(nextDb, nextProfile) ? saveRxDb(nextDb) ? const hydratedProfiles = nextDb.prescriberProfiles.length > 0
         nextDb.prescriberProfiles
        : [createPrescriberProfileFromSettings(nextDb.profile, 'Perfil padrão', 'default')]

      setDb(nextDb) ? setProfiles(hydratedProfiles) ? setSelectedProfileId(nextProfile.id) ? setProfileName(nextProfile.profileName) ? setProfile({
        adminId: 'ADMIN',
      ...
      }) ? setSaveModalOpen(false) ? setSaved(true) ? window.setTimeout(() => setSaved(false), 2200)
    } catch (error) {
      ...
      actions={
        <>
          ...
          <button
            ...
            onClick={() => {
              setSaveError('') ? setSaveModalOpen(true)
            }}
          >
