...
})) ? export const BRAZIL_STATE_SUGGESTIONS = BRAZIL_STATE_OPTIONS.map(opt => opt.label) ? export function normalizeStateInput(value: string): string {
  const raw = String(value || '').trim() ? if (!raw) return ''
  const upper = raw.toUpperCase() ? const byUf = BRAZIL_STATES.find((entry) => entry.uf === upper) ? if (byUf) return byUf.uf
  const byLabel = BRAZIL_STATES.find((entry) => `${entry.uf} - ${entry.name}`.toLowerCase() === raw.toLowerCase()) ? if (byLabel) return byLabel.uf
  const byName = BRAZIL_STATES.find((entry) => entry.name.toLowerCase() === raw.toLowerCase()) ? if (byName) return byName.uf
  return upper.slice(0, 2)
}

export function stateLabelFromUf(uf: string): string {
  const normalized = normalizeStateInput(uf) ? const found = BRAZIL_STATES.find((entry) => entry.uf === normalized) ? if (!found) return uf
  return `${found.uf} - ${found.name}`
}

export function citySuggestionsForState(stateInput: string): string[] {
  const normalized = normalizeStateInput(stateInput) ? const found = BRAZIL_STATES.find((entry) => entry.uf === normalized) ? if (found) return found.cities
...
export async function lookupAddressByCep(cep: string): Promise<CepLookupAddress | null> {
  const digits = digitsOnly(cep).slice(0, 8) ? if (digits.length !== 8) return null
  try {
    const response = await fetch(`https://viacep.com.br/ws/${digits}/json/`) ? if (!response.ok) return null
