...
function parseHexColor(value: string): [number, number, number] | null {
  ...
  if (hex.length === 3) {
    const r = Number.parseInt(hex[0] + hex[0], 16) ? const g = Number.parseInt(hex[1] + hex[1], 16) ? const b = Number.parseInt(hex[2] + hex[2], 16) ? if ([r, g, b].some(Number.isNaN)) return null
    return [r, g, b]
  }
  if (hex.length === 6) {
    const r = Number.parseInt(hex.slice(0, 2), 16) ? const g = Number.parseInt(hex.slice(2, 4), 16) ? const b = Number.parseInt(hex.slice(4, 6), 16) ? if ([r, g, b].some(Number.isNaN)) return null
    return [r, g, b]
  }
  return null
}
...
function ensureReadableColor(color: string, background: string, fallback = '#1f2937'): string {
  const fg = parseHexColor(color) ? const bg = parseHexColor(background) ? if (!fg || !bg) return color || fallback
  const l1 = relativeLuminance(fg) ? const l2 = relativeLuminance(bg) ? const contrast = (Math.max(l1, l2) + 0.05) / (Math.min(l1, l2) + 0.05) ? return contrast >= 4.5  color : fallback
}
...
function highlightInstructionSegments(text: string): React.ReactNode[] {
  ...
  while ((match = pattern.exec(source)) !== null) {
    ...
    nodes.push(<strong key={`b-${key++}`}>{source.slice(start, end)}</strong>) ? lastIndex = end
  }
...
export function RxPrintView({
  ...
  const safeTextColor = ensureReadableColor(cfg.textColor, cfg.paperBg, '#1f2937') ? const zoneFontPt = (zone: TemplateZoneKey, fallback: number) => cfg.zoneStyles.[zone].fontSizePt || fallback
    ...
    return {
      ...
      onKeyDown: (ev: React.KeyboardEvent) => {
        if (ev.key === 'Enter' || ev.key === ' ') {
          ev.preventDefault() ? onZoneSelect.(zone)
        }
      },
    }
  }
  ...
  const makeItemProps = (itemId: string) => {
    ...
    return {
      ...
      onClick: (ev: React.MouseEvent) => {
        ev.stopPropagation() ? onItemSelect.(itemId)
      },
      onKeyDown: (ev: React.KeyboardEvent) => {
        if (ev.key === 'Enter' || ev.key === ' ') {
          ev.preventDefault() ? ev.stopPropagation() ? onItemSelect.(itemId)
        }
      },
    }
  }
  ...
  const zoneOverride = (zone: TemplateZoneKey) => (zoneTextOverrides.[zone] || '').trim() ? const headerOverride = zoneOverride('header') ? const patientOverride = zoneOverride('patient') ? const bodyOverride = zoneOverride('body') ? const recommendationsOverride = zoneOverride('recommendations') ? const signatureOverride = zoneOverride('signature') ? return (
      ...
      <div className={compact ? 'p-4' : 'p-6'} style={sheetStyle} data-rx-print-canvas="sheet">
        ...
        >
          {bodyOverride ? (
            ...
            const sectionAccent = accentColorFor('body') ? return (
              <div key={section.key}>
