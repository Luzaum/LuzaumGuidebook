...
function resolveBucketName(): string {
  const fromEnv = String(import.meta.env.VITE_SUPABASE_RX_MEDIA_BUCKET || '').trim() ? return fromEnv || DEFAULT_RX_MEDIA_BUCKET
}

function validateSupabaseEnv() {
  const url = String(import.meta.env.VITE_SUPABASE_URL || '').trim() ? const key = String(import.meta.env.VITE_SUPABASE_ANON_KEY || '').trim() ? if (!url || !key || key.includes('...')) {
    throw new Error('Supabase não configurado para upload. Ajuste VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY.')
  }
}

function dataUrlToBlob(dataUrl: string): Blob {
  const match = String(dataUrl || '').match(/^data:([^;]+);base64,(.+)$/) ? if (!match) {
    throw new Error('Formato de imagem inválido para upload.')
  }

  const mime = match[1] || 'application/octet-stream'
  const base64 = match[2] || ''
  const binary = atob(base64) ? const bytes = new Uint8Array(binary.length) ? for (let i = 0; i < binary.length; i += 1) {
...
function extensionFromMime(mimeType: string): string {
  const mime = String(mimeType || '').toLowerCase() ? if (mime === 'image/jpeg') return 'jpg'
...
function normalizeUploadError(error: unknown, bucket: string): Error {
  const message = parseErrorMessage(error) ? const lower = message.toLowerCase() ? if (lower.includes('bucket') && (lower.includes('not found') || lower.includes('does not exist'))) {
...
function readLocalOwnerKey(): string {
  try {
    const raw = localStorage.getItem('luzaum-user') ? if (!raw) return 'anon'
    const parsed = JSON.parse(raw) as { id: string; email: string }
    const candidate = String(parsed.id || parsed.email || '').trim() ? return sanitizePathSegment(candidate) || 'anon'
  } catch {
    return 'anon'
  }
}
...
async function resolveOwnerKey(): Promise<string> {
  try {
    const { data, error } = await supabase.auth.getUser() ? if (!error && data.user.id) {
...
function parseStorageReferenceFromUrl(fileUrl: string): { bucket: string; path: string } | null {
  const raw = String(fileUrl || '').trim() ? if (!raw) return null

  try {
    const url = new URL(raw) ? const match = url.pathname.match(STORAGE_PUBLIC_PATH) ? if (!match) return null
    const bucket = decodeURIComponent(match[1] || '') ? const path = decodeURIComponent(match[2] || '') ? if (!bucket || !path) return null
    return { bucket, path }
  } catch {
    return null
  }
...
export async function uploadProfileImageDataUrl(params: {
  ...
  validateSupabaseEnv() ? const bucket = resolveBucketName() ? const ownerKey = await resolveOwnerKey()
  ...
  const firstSegment = sanitizePathSegment(params.clinicId || ownerKey) ? const profileKey = sanitizePathSegment(params.profileId || 'default') ? const blob = dataUrlToBlob(params.dataUrl) ? const ext = extensionFromMime(blob.type) ? const filePath = `${firstSegment}/profiles/${profileKey}/${params.field}/${Date.now()}-${randomId()}.${ext}`
  ...
  const storage = supabase.storage.from(bucket) ? const { error } = await storage.upload(filePath, blob, {
    upsert: false,
  ...
  const { data } = storage.getPublicUrl(filePath) ? const publicUrl = String(data.publicUrl || '').trim() ? if (!publicUrl) {
...
export async function removeProfileImageByUrl(fileUrl: string) {
  const ref = parseStorageReferenceFromUrl(fileUrl) ? if (!ref) return

  try {
    const { error } = await supabase.storage.from(ref.bucket).remove([ref.path]) ? if (error) {
