...
export function buildPrescriptionStateFromNovaReceita2(state: NovaReceita2State): PrescriptionState {
    ...
    return {
        ...
        tutor: {
            ...
            addressStreet: state.tutor?.street || '',
            addressNumber: state.tutor?.number || '',
            addressComplement: state.tutor?.complement || '',
            addressDistrict: state.tutor?.neighborhood || '',
            addressCity: state.tutor?.city || '',
            addressState: state.tutor?.state || '',
            addressZip: state.tutor?.zipcode || '',
        },
        patient: {
...
export function buildPrintDocFromNovaReceita2(
    ...
    // Backward-compatible: returns just the first doc (standard or all-in-one)
    const docs = buildPrintDocsFromNovaReceita2(state, _template)
    return docs[0]
}

/**
 * Build PrintDocs for Nova Receita 2.0, splitting controlled medications
 * into a separate "special-control" document when present.
 * Returns [standardDoc] or [standardDoc, specialControlDoc].
 */
export function buildPrintDocsFromNovaReceita2(
    ...
    const { standard, specialControl } = splitPrescriptionByControl(rxState)
    ...
    // Standard doc (normal medications)
    if (standard) {
        const stdDoc = renderRxToPrintDoc(standard, { renderMode: 'final', documentKind: 'standard' })
        stdDoc.addressLine = tutorAddressLine || ''
        docs.push(stdDoc)
    }

    // Special-control doc (controlled medications)
    if (specialControl) {
        const ctrlDoc = renderRxToPrintDoc(specialControl, { renderMode: 'final', documentKind: 'special-control' })
        ctrlDoc.addressLine = tutorAddressLine || ''
        docs.push(ctrlDoc)
    }

    // Fallback: if no items at all, produce an empty standard doc
    if (!docs.length) {
        const fallbackDoc = renderRxToPrintDoc(rxState, { renderMode: 'final', documentKind: 'standard' })
        fallbackDoc.addressLine = tutorAddressLine || ''
