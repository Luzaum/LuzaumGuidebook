...
function writeHeaderSelectionSession(snapshot: RxHeaderSelectionSession | null) {
  try {
    if (!snapshot) {
      sessionStorage.removeItem(RX_HEADER_SELECTION_SESSION_KEY) ? return
    }
...
function readHeaderSelectionSession(): RxHeaderSelectionSession | null {
  try {
    const raw = sessionStorage.getItem(RX_HEADER_SELECTION_SESSION_KEY) ? if (!raw) return null
...
function replaceItem(items: PrescriptionItem[], item: PrescriptionItem) {
  const exists = items.some((entry) => entry.id === item.id) ? if (!exists) return [...items, item]
...
function canonicalExamName(exam: string): string {
  const normalized = normalizeLooseText(exam) ? const found = COMMON_EXAMS.find((common) => normalizeLooseText(common) === normalized) ? return found || exam.trim()
}
...
function uniqueNormalizedLines(lines: string[]): string[] {
  ...
  for (const line of lines) {
    ...
    const key = normalizeLooseText(trimmed) ? if (!key || seen.has(key)) continue
    seen.add(key) ? result.push(trimmed)
  }
  return result
}
...
function normalizeExamState(
  ...
  const merged = uniqueNormalizedLines([...(commonRaw || []), ...(customRaw || [])].map(canonicalExamName)) ? const common = merged.filter((exam) =>
...
function calculateDefaultWaterMlPerDay(weightKgText: string): string {
  const numericWeight = Number((weightKgText || '').replace(',', '.')) ? if (!Number.isFinite(numericWeight) || numericWeight <= 0) return ''
...
function findProfileSettings(dbState: ReturnType<typeof loadRxDb>, profileId: string): { id: string; profile: ProfileSettings } {
  const selected = dbState.prescriberProfiles.find((entry) => entry.id === profileId) ? if (selected) return { id: selected.id, profile: selected }
...
function hydratePrescription(raw: PrescriptionState | null | undefined): PrescriptionState {
  const base = createDefaultPrescriptionState() ? if (!raw) return base
  const normalizedExams = normalizeExamState(raw.recommendations.exams, raw.recommendations.customExams) ? return {
    ...base,
    ...raw,
...
export default function NovaReceitaPage() {
  const navigate = useNavigate() ? const location = useLocation() ? const { clinicId } = useClinic() ? const rxDataSource = useMemo(
  ...
  useEffect(() => {
    if (import.meta.env.DEV) {
      console.log('[NovaReceitaPage] DATA SOURCE:', rxDataSource) ? console.log('[NovaReceitaPage] Using adapter:', rxAdapter.constructor.name)
    }
  }, [rxDataSource, rxAdapter]) ? const supabaseModeWithoutClinic = rxDataSource === 'supabase' && !clinicId
  const requestedDraftId = useMemo(() => new URLSearchParams(location.search).get('draft').trim() || '', [location.search]) ? const [db, setDb] = useState(() => loadRxDb()) ? const [prescription, setPrescription] = useState<PrescriptionState>(() => {
    const localDb = loadRxDb() ? const base = hydratePrescription(loadRxDraft()) ? const selectedProfile = findProfileSettings(localDb, base.prescriber.profileId) ? return applyProfileToPrescription(base, selectedProfile.profile, selectedProfile.id)
  }) ? const [saveStatus, setSaveStatus] = useState<SaveStatus>('saved') ? const [autosaveEnabled, setAutosaveEnabled] = useState(true) ? const [toast, setToast] = useState<ToastState | null>(null) ? const [mobileTab, setMobileTab] = useState<EditorTab>('editor') ? const [customExamDraft, setCustomExamDraft] = useState('') ? const [examReasonDraft, setExamReasonDraft] = useState('') ? const [protocolModalOpen, setProtocolModalOpen] = useState(false) ? const [patientCreateModalOpen, setPatientCreateModalOpen] = useState(false) ? const [protocolSearch, setProtocolSearch] = useState('') ? const [cepLookupLoading, setCepLookupLoading] = useState(false) ? const [cepLookupMessage, setCepLookupMessage] = useState<string | null>(null) ? const [modalState, setModalState] = useState<ModalState>(EMPTY_MODAL_STATE) ? const [supabaseLoading, setSupabaseLoading] = useState(false) ? const [supabasePrescriptionId, setSupabasePrescriptionId] = useState<string | null>(null)
  // Supabase tutor+patient import state
  const [selectedSupabaseTutor, setSelectedSupabaseTutor] = useState<TutorInfo | null>(null) ? const [supabasePatientsForTutor, setSupabasePatientsForTutor] = useState<PatientInfo[]>([]) ? const [supabasePatientsLoading, setSupabasePatientsLoading] = useState(false) ? const [patientWeights, setPatientWeights] = useState<any[]>([]) ? const [loadingWeights, setLoadingWeights] = useState(false) ? const saveTimeoutRef = useRef<number | null>(null) ? const pendingProtocolImportRef = useRef(false) ? const lastAutoWaterValueRef = useRef('') ? const loadedRequestedDraftIdRef = useRef('') ? const headerSelectionHydratedRef = useRef(false)
  ...
  */
  ...
  }, [db.protocolFolders]) ? const filteredProtocols = useMemo(() => {
    return [...db.protocols]
  ...
  }, [db.protocols, protocolSearch]) ? const prescriberProfiles = useMemo<PrescriberProfile[]>(() => {
  ...
  }, [db.profile, db.prescriberProfiles]) ? const breedOptions = useMemo(() => breedOptionsForSpecies(prescription.patient.species), [prescription.patient.species]) ? const coatOptions = useMemo(() => coatOptionsForSpecies(prescription.patient.species), [prescription.patient.species]) ? const pushToast = (type: ToastType, message: string) => {
    setToast({ type, message }) ? window.setTimeout(() => setToast(null), 2600)
  }

  const updatePrescription = (updater: (prev: PrescriptionState) => PrescriptionState) => {
    setPrescription((prev) => withTimestamp(updater(prev))) ? setSaveStatus('editing')
  }
  ...
  useEffect(() => {
  ...
  }, [rxDataSource]) ? useEffect(() => {
    if (!headerSelectionHydratedRef.current && !requestedDraftId) return

    const patientRecordId = String(prescription.patient.patientRecordId || '').trim() ? const tutorRecordId = String(prescription.tutor.tutorRecordId || '').trim() ? if (!patientRecordId && !tutorRecordId) {
      writeHeaderSelectionSession(null) ? return
    }
    ...
    const sessionSelection = readHeaderSelectionSession() ? if (!sessionSelection) return
    ...
    if (sessionSelection.source === 'supabase') {
      const hasInvalidId =
        ...
        (sessionSelection.tutorRecordId && !isUuid(sessionSelection.tutorRecordId)) ? if (hasInvalidId) {
        writeHeaderSelectionSession(null) ? return
      }
    }
      ...
      ; (async () => {
        try {
          ...
          if (
            ...
            const patients = await rxAdapter.listPatientsByTutorId(sessionSelection.tutorRecordId) ? resolvedPatient =
    ...
    const loaded = loadRxDraftById(requestedDraftId) ? if (!loaded) {
      pushToast('error', 'Rascunho não encontrado.') ? loadedRequestedDraftIdRef.current = requestedDraftId
      return
    }

    const hydrated = hydratePrescription(loaded) ? const selectedProfile = findProfileSettings(loadRxDb(), hydrated.prescriber.profileId) ? setPrescription(applyProfileToPrescription(hydrated, selectedProfile.profile, selectedProfile.id)) ? setSaveStatus('saved') ? loadedRequestedDraftIdRef.current = requestedDraftId
    pushToast('success', 'Rascunho carregado com sucesso.')
  }, [requestedDraftId]) ? useEffect(() => {
    if (!autosaveEnabled) return
    if (saveTimeoutRef.current) window.clearTimeout(saveTimeoutRef.current) ? setSaveStatus('saving') ? saveTimeoutRef.current = window.setTimeout(() => {
      try {
        saveRxDraft(withTimestamp(prescription)) ? setSaveStatus('saved')
      } catch {
        setSaveStatus('error')
      }
    }, 700) ? return () => {
  ...
  }, [prescription, autosaveEnabled]) ? useEffect(() => {
    const autoWater = calculateDefaultWaterMlPerDay(prescription.patient.weightKg) ? if (!autoWater) return
    const currentWater = (prescription.recommendations.waterMlPerDay || '').trim() ? const shouldApply = !currentWater || currentWater === lastAutoWaterValueRef.current
  ...
  }, [prescription.patient.weightKg, prescription.recommendations.waterMlPerDay]) ? const saveNow = () => {
    try {
      const nextRx = withTimestamp({
      ...
      }) ? saveRxDraft(nextRx) ? const currentDb = loadRxDb() ? let nextDb = upsertPatientFromPrescription(currentDb, nextRx) ? nextDb = upsertClientFromPrescription(nextDb, nextRx) ? nextDb = upsertHistoryFromPrescription(nextDb, nextRx) ? saveRxDb(nextDb) ? setDb(nextDb) ? const tutorCpfDigits = digitsOnly(nextRx.tutor.cpf || '') ? const matchedClient =
        ...
        (tutorCpfDigits
          ...
          : null) ||
        nextDb.clients.find((entry) => normalizeLooseText(entry.fullName) === normalizeLooseText(nextRx.tutor.name)) ? const matchedAnimal =
        (matchedClient &&
      ...
      setSaveStatus('saved') ? pushToast('success', 'Rascunho salvo com sucesso.')
    } catch {
      setSaveStatus('error') ? pushToast('error', 'Falha ao salvar o rascunho.')
    }
  }

  // DEV: Save prescription to Supabase (draft mode) ? const saveToSupabase = async () => {
    if (rxDataSource !== 'supabase') {
      pushToast('error', 'Modo Supabase não ativado. Configure via VITE_RX_DATA_SOURCE.') ? return
    }
    ...
    if (!isUuid(patientId) || !isUuid(tutorId)) {
      pushToast('error', 'Para salvar no Supabase, selecione um paciente/tutor do Supabase (não do legado/local).') ? console.warn('[saveToSupabase] IDs inválidos para Supabase', { patientId, tutorId }) ? return
    }

    setSupabaseLoading(true) ? try {
      if (supabasePrescriptionId) {
        // Update existing (savePrescription faz upsert quando id é fornecido) ? const result = await savePrescription({
        ...
        const result = await savePrescription({
          ...
          content: prescription as any,
          clinic_id: clinicId || undefined,
        }) ? setSupabasePrescriptionId(result.id) ? pushToast('success', `Receita criada no Supabase (ID: ${result.id})`)
      }
    } catch (error) {
      const msg = error instanceof Error  error.message : 'Erro ao salvar'
      pushToast('error', `Falha: ${msg}`) ? if (import.meta.env.DEV) console.error(error)
    } finally {
  ...
  const listSupabasePrescriptions = async () => {
    if (rxDataSource !== 'supabase') {
      pushToast('error', 'Modo Supabase não ativado.') ? return
    }

    if (!prescription.patient.patientRecordId) {
      pushToast('error', 'Paciente obrigatório para listar receitas.') ? return
    }

    setSupabaseLoading(true) ? try {
      const results = await listPrescriptionsByPatient(prescription.patient.patientRecordId, clinicId || undefined) ? pushToast('info', `${results.length} receita(s) encontrada(s) no Supabase`) ? if (import.meta.env.DEV) {
  ...
  const openCreateModal = (category: ItemCategory = 'medication') => {
    const draft = createDefaultItem(category) ? setModalState({
      open: true,
      mode: 'create',
      autosave: true,
      useCatalog: true,
  ...
  const saveModal = () => {
    ...
    commitModalDraft(modalState.draft) ? setModalState(EMPTY_MODAL_STATE)
  }

  const updateModalDraft = (updater: (prev: PrescriptionItem) => PrescriptionItem) => {
    if (!modalState.draft) return
    const next = withTimestamp(applyAutoInstructionIfNeeded(updater(modalState.draft), prescription)) ? setModalState((prev) => ({ ...prev, draft: next })) ? if (modalState.autosave) commitModalDraft(next)
  }
  ...
  const previewState = useMemo(() => {
  ...
  }, [modalState, prescription]) ? const previewSplitDocs = useMemo(() => splitPrescriptionByControl(previewState), [previewState]) ? const previewPrintDocs = useMemo(() => {
    const docs: Array<ReturnType<typeof renderRxToPrintDoc>> = []
    if (previewSplitDocs.standard) docs.push(renderRxToPrintDoc(previewSplitDocs.standard, { documentKind: 'standard' })) ? if (previewSplitDocs.specialControl) docs.push(renderRxToPrintDoc(previewSplitDocs.specialControl, { documentKind: 'special-control' })) ? if (!docs.length) docs.push(renderRxToPrintDoc(previewState, { documentKind: 'standard' })) ? return docs
  }, [previewSplitDocs.specialControl, previewSplitDocs.standard, previewState]) ? const selectedPreviewProfile = useMemo(
  ...
  }, [db.activeTemplateId, db.templates, previewState.recommendations.standardTemplateId]) ? const standardTemplate = useMemo(() => {
  ...
  }, [activeTemplate, db.templates]) ? const specialControlTemplate = useMemo(() => {
  ...
  }, [activeTemplate, db.templates, previewState.recommendations.specialControlTemplateId]) ? const previewSpecialControlTargetPharmacy = useMemo<SpecialControlPharmacy>(() => {
    ...
    return 'veterinária'
  }, [previewState.recommendations.specialControlPharmacy]) ? const removePrescriptionItem = (itemId: string) => {
    updatePrescription((prev) => ({ ...prev, items: removeItem(prev.items, itemId) }))
  }

  const duplicatePrescriptionItem = (item: PrescriptionItem) => {
    updatePrescription((prev) => ({ ...prev, items: [...prev.items, cloneItem(item)] })) ? pushToast('success', 'Item duplicado.')
  }
  ...
  const toggleExam = (exam: string, checked: boolean) => {
    const canonical = canonicalExamName(exam) ? const key = normalizeLooseText(canonical) ? updatePrescription((prev) => ({
      ...prev,
      recommendations: {
        ...prev.recommendations,
        exams: checked
  ...
  const importProtocol = (protocol: RxProtocol) => {
    const now = new Date().toISOString() ? updatePrescription((prev) => {
      const importedItems = protocol.items.map((item) => ({
        ...
        createdAt: now,
        updatedAt: now,
      })) ? const protocolExams = protocol.exams.filter(Boolean) ? const commonExamImports = protocolExams.filter((exam) =>
    ...
    }) ? setProtocolModalOpen(false) ? setProtocolSearch('') ? pushToast('success', `Protocolo \"${protocol.name}\" importado.`)
  }
  ...
  useEffect(() => {
    ...
    const sessionSnapshot = sessionStorage.getItem(snapshotKey) ? if (sessionSnapshot) {
      sessionStorage.removeItem(snapshotKey) ? try {
        const parsed = JSON.parse(sessionSnapshot) as RxProtocol
        pendingProtocolImportRef.current = true
        importProtocol(parsed) ? return
      } catch {
        // fallback below
      }
    }

    const snapshot = localStorage.getItem('receituario-vet:protocol-snapshot') ? if (snapshot) {
      localStorage.removeItem('receituario-vet:protocol-snapshot') ? try {
    ...
    const protocolId = localStorage.getItem('receituario-vet:protocol-to-import') ? if (!protocolId) return
    const protocol = db.protocols.find((entry) => entry.id === protocolId) ? localStorage.removeItem('receituario-vet:protocol-to-import') ? if (!protocol) return
    pendingProtocolImportRef.current = true
    importProtocol(protocol)
  }, [db.protocols]) ? const hasSpecialControlItems = (previewSplitDocs.specialControl.items.length || 0) > 0
  ...
  const tutorCitySuggestions = useMemo(
    ...
    const found = patientSuggestions.find((entry) => entry.name.toLowerCase() === name.trim().toLowerCase()) ? if (!found) return
    updatePrescription((prev) => ({
      ...prev,
      patient: {
        ...prev.patient,
  ...
  const applyClientFromBank = (clientId: string, animalId: string) => {
    const client = clientSuggestions.find((entry) => entry.id === clientId) ? if (!client) return
  ...
  const fetchCepForTutor = async (rawCep: string) => {
    const cepDigits = digitsOnly(rawCep).slice(0, 8) ? if (cepDigits.length !== 8) {
      setCepLookupMessage(null) ? return
    }

    setCepLookupLoading(true) ? setCepLookupMessage(null) ? const address = await lookupAddressByCep(cepDigits) ? setCepLookupLoading(false) ? if (!address) {
      setCepLookupMessage('CEP não encontrado.') ? return
    }
  ...
  const loadWeightsForPatient = useCallback(async (patientId: string) => {
    ...
    try {
      setLoadingWeights(true) ? const freshWeights = await loadPatientWeights(patientId, clinicId) ? setPatientWeights(freshWeights)
    } catch (err) {
  ...
  }, [prescription.patient.patientRecordId, loadWeightsForPatient]) ? function sparklinePath(pointsArr: any[], width = 240, height = 40): string {
    ...
    const minY = Math.min(...points.map((entry) => entry.value)) ? const maxY = Math.max(...points.map((entry) => entry.value)) ? const spanY = Math.max(maxY - minY, 0.01) ? const left = 5
    const right = width - 5
    const top = 5
  ...
  const startNewTutor = () => {
    const base = createDefaultPrescriptionState() ? setPrescription((prev) => ({
      ...prev,
      tutor: base.tutor,
      patient: base.patient,
    })) ? setSelectedSupabaseTutor(null) ? setSupabasePatientsForTutor([]) ? setPatientWeights([]) ? setCustomExamDraft('') ? setExamReasonDraft('') ? pushToast('info', 'Dados de tutor e paciente resetados.')
  }
  ...
  const startNewPatient = () => {
    const base = createDefaultPrescriptionState() ? setPrescription((prev) => ({
      ...prev,
      patient: base.patient,
    })) ? setPatientWeights([]) ? setCustomExamDraft('') ? setExamReasonDraft('') ? pushToast('info', 'Dados do paciente resetados.')
  }

  const applyPrescriberProfile = (profileId: string) => {
    const selected = prescriberProfiles.find((entry) => entry.id === profileId) ? if (!selected) return
    updatePrescription((prev) => ({
      ...prev,
    ...
    <ReceituarioChrome
      ...
      actions={
        <>
          ...
          <RxvButton variant="secondary" className="h-[38px] px-3 py-2 text-sm" onClick={() => { saveNow(); navigate(`/receituario-vet/rx/${prescription.id}/printmode=review`) }}>
        ...
        <div className={`space-y-6 lg:col-span-5 ${mobileTab === 'preview' ? 'hidden lg:block' : ''}`}>
              ...
              {rxDataSource === 'local' ? (
                <>
                  <PatientQuickSelect
                    ...
                    onPick={(payload) => {
                      applyPatientTutorToPrescription(payload) ? pushToast('success', `Paciente "${payload.patient.name}" aplicado na receita.`)
                    }}
                  ...
                  <TutorQuickSelect
                    ...
                    onPick={(tutor) => {
                      ...
                      if (rxAdapter.listPatientsByTutorId) {
                        setSupabasePatientsLoading(true) ? setSupabasePatientsForTutor([]) ? void rxAdapter.listPatientsByTutorId(tutor.tutorRecordId)
                          .then((patients) => {
                      ...
                      {supabasePatientsForTutor.length > 0 && (
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            const p = supabasePatientsForTutor.find((entry) => entry.patientRecordId === pid) ? if (!p) return
                            updatePrescription((prev) => ({
                              ...prev,
                              patient: {
                                ...prev.patient,
          ...
          <section className="rxv-card p-5">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto]">
              ...
              <div className="flex items-end">
                <Link
                  to={`/receituario-vet/templatestemplate=${encodeURIComponent(prescription.recommendations.standardTemplateId || activeTemplate.id || '')}`}
            ...
            <div className="space-y-3">
              {prescription.items.length === 0 ? (
                ...
                prescription.items.map((item, index) => {
                  const status = itemStatus(item, prescription) ? const qty = calculateMedicationQuantity(item, prescription) ? return (
          ...
          <section className="rxv-card p-5">
            ...
            <div className="mt-5">
              ...
              <RxvChipsMultiSelect
                ...
                onToggle={(exam) => {
                  const isSelected = prescription.recommendations.exams.some((entry) => normalizeLooseText(entry) === normalizeLooseText(exam)) ? toggleExam(exam, !isSelected)
                }}
              />
              <div className="mt-3 flex gap-2">
                ...
                <RxvButton
                  ...
                  onClick={() => {
                    ...
                    const canonical = canonicalExamName(customExamDraft.trim()) ? const isCommon = COMMON_EXAMS.some((entry) => normalizeLooseText(entry) === normalizeLooseText(canonical)) ? updatePrescription((prev) => ({
                      ...prev,
                      recommendations: {
                        ...prev.recommendations,
          ...
          {hasSpecialControlItems ? (
            <section className="rxv-card p-5">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div className="md:col-span-2">
                  <Link
                    to={`/receituario-vet/templatestemplate=${encodeURIComponent(
          ...
          <section className="grid grid-cols-1 gap-2 md:hidden">
            ...
            <button type="button" className="rxv-btn-secondary px-3 py-2 text-sm" onClick={() => { saveNow(); navigate(`/receituario-vet/rx/${prescription.id}/printmode=review`) }}>
              Revisar
            </button>
            ...
            <div className="max-h-[calc(92vh-72px)] overflow-y-auto p-5">
              ...
              <div className="space-y-3">
                {filteredProtocols.map((protocol) => {
                  const folder = protocolFolderMap.get(protocol.folderId) ? return (
      ...
      {rxDataSource === 'local' && (
        <PatientCreateModal
          ...
          onCreatedAndPicked={(payload) => {
            applyPatientTutorToPrescription(payload) ? setDb(loadRxDb()) ? pushToast('success', `Paciente "${payload.patient.name}" criado e aplicado.`)
          }}
