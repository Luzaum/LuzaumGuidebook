...
export default function HistoricoReceitasPage() {
    const [searchParams, setSearchParams] = useSearchParams() ? const navigate = useNavigate() ? const { clinicId, loading: clinicLoading } = useClinic() ? const patientId = searchParams.get('patientId') ? const patientName = searchParams.get('patientName') || 'Paciente'

    const [prescriptions, setPrescriptions] = useState<PrescriptionRecord[]>([]) ? const [isLoading, setIsLoading] = useState(true) ? const [previewDoc, setPreviewDoc] = useState<PrescriptionRecord | null>(null) ? const loadHistory = useCallback(async () => {
        ...
        setIsLoading(true) ? try {
            const data = await listPrescriptionsByPatient(patientId, clinicId) ? setPrescriptions(data)
        } catch (err: any) {
    ...
    }, [patientId, clinicId, clinicLoading]) ? useEffect(() => {
        loadHistory()
    }, [loadHistory]) ? const handleOpen = (record: PrescriptionRecord) => {
        // Direciona para o editor 2.0 com o ID da prescrição
        navigate(`/receituario-vet/nova-receita-2prescriptionId=${record.id}`)
    }
    ...
    const handleVoid = async (record: PrescriptionRecord) => {
        ...
        try {
            await voidPrescription(record.id, clinicId || undefined) ? loadHistory()
        } catch (err) {
    ...
    const handleDownloadPdf = async (record: PrescriptionRecord) => {
        ...
        try {
            const url = await getPdfSignedUrl(record.pdf_path, 60) ? window.open(url, '_blank', 'noopener,noreferrer')
        } catch (err) {
            console.error('[Historico] signed url error', err) ? alert('Erro ao obter link do PDF.')
        }
    }
    ...
    const handleDelete = async (record: PrescriptionRecord) => {
        ...
        try {
            ...
            console.error('[Historico] delete error', err) ? alert('Erro ao excluir receita.')
        }
    }
    ...
    const handleExport = (record: PrescriptionRecord) => {
        ...
        if (!record.content.stateSnapshot) {
            alert('Não é possível exportar: Snapshot de estado ausente.') ? return
        }
        sessionStorage.setItem('vetius:rx2:review-draft', JSON.stringify(record.content.stateSnapshot)) ? navigate('/receituario-vet/nova-receita-2-print')
    }
