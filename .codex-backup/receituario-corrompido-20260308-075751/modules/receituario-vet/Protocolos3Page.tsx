...
export default function Protocolos3Page() {
  const navigate = useNavigate() ? const { clinicId } = useClinic() ? const [userId, setUserId] = useState<string | null>(null)

  // ✅ Estado: lista de pastas e protocolos
  const [folders, setFolders] = useState<ProtocolFolderRecord[]>([]) ? const [protocols, setProtocols] = useState<ProtocolRecord[]>([]) ? const [isLoadingFolders, setIsLoadingFolders] = useState(false) ? const [isLoadingProtocols, setIsLoadingProtocols] = useState(false)

  // ✅ Estado: seleção STITCH layout
  const [selectedFolderId, setSelectedFolderId] = useState<string | null>(null) ? const [selectedProtocolId, setSelectedProtocolId] = useState<string | null>(null) ? const [selectedProtocolBundle, setSelectedProtocolBundle] = useState<ProtocolBundle | null>(null)

  // ✅ Estado: modal criar/editar protocolo
  const [modalOpen, setModalOpen] = useState(false) ? const [editingProtocol, setEditingProtocol] = useState<ProtocolBundle | null>(null) ? const [protocolDraft, setProtocolDraft, clearProtocolDraft, hasProtocolDraft] = useLocalDraft<ProtocolBundle | null>(
    'protocolos3-editor',
    clinicId || null,
    userId,
    null,
  ...
  // ✅ Estado: criar pasta
  const [createFolderOpen, setCreateFolderOpen] = useState(false) ? const [createFolderName, setCreateFolderName] = useState('') ? const [isCreatingFolder, setIsCreatingFolder] = useState(false)

  // ✅ Estado: busca de medicamentos
  const [medicationSearchOpen, setMedicationSearchOpen] = useState(false) ? const [medicationSearchQuery, setMedicationSearchQuery] = useState('') ? const [medications, setMedications] = useState<MedicationSearchResult[]>([]) ? const [isSearchingMedications, setIsSearchingMedications] = useState(false)
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      if (data.user) {
        setUserId(data.user.id) ? console.log('[Protocolos3] userId', data.user.id)
      }
    })
  }, [])

  // ✅ Carregar pastas (apenas após clinicId e userId estarem definidos) ? useEffect(() => {
    if (!clinicId || !userId) {
      console.log('[Protocolos3] Aguardando clinicId e userId', { clinicId, userId }) ? return
    }

    console.log('[Protocolos3] Carregando folders', { clinicId, userId }) ? setIsLoadingFolders(true) ? listFolders(clinicId, userId)
      .then(async (data) => {
        console.log('[Protocolos3] Folders carregados', data) ? await ensureDefaultSpecialtyProtocolSeed(clinicId, userId) ? const [refreshedFolders, refreshedProtocols] = await Promise.all([
          listFolders(clinicId, userId),
          listProtocols(clinicId, userId),
        ]) ? setFolders(refreshedFolders) ? setProtocols(refreshedProtocols)
      })
      .catch((err) => {
        console.error('[Protocolos3] Erro ao carregar folders', err) ? setFolders([])
      })
      .finally(() => setIsLoadingFolders(false))
  }, [clinicId, userId])

  // ✅ Carregar protocolos (apenas após clinicId e userId estarem definidos) ? useEffect(() => {
    if (!clinicId || !userId) {
      console.log('[Protocolos3] Aguardando clinicId e userId para protocolos', { clinicId, userId }) ? return
    }

    console.log('[Protocolos3] Carregando protocols', { clinicId, userId }) ? setIsLoadingProtocols(true) ? listProtocols(clinicId, userId)
      .then((data) => {
        console.log('[Protocolos3] Protocols carregados', data) ? setProtocols(data)
      })
      .catch((err) => {
        console.error('[Protocolos3] Erro ao carregar protocols', err) ? setProtocols([])
      })
  ...
  useEffect(() => {
    if (!clinicId || !medicationSearchOpen) {
      setMedications([]) ? return
    }

    const q = medicationSearchQuery.trim() ? const timer = setTimeout(async () => {
      try {
        setIsSearchingMedications(true) ? const results = await searchMedications(clinicId, q || '', q  50 : 20) ? console.log('[Protocolos3] Medicamentos encontrados', results.length) ? setMedications(results)
      } catch (err) {
        console.error('[Protocolos3] Erro ao buscar medicamentos', err) ? setMedications([])
      } finally {
        setIsSearchingMedications(false)
      }
    }, q  400 : 0) ? return () => clearTimeout(timer)
  }, [medicationSearchQuery, clinicId, medicationSearchOpen]) ? useEffect(() => {
    if (!modalOpen) return
    if (!hasProtocolDraft || !protocolDraft) return
    if (!editingProtocol) return
    if (editingProtocol.protocol.id) return
    setEditingProtocol(protocolDraft)
  }, [modalOpen, hasProtocolDraft, protocolDraft, editingProtocol]) ? useEffect(() => {
  ...
  const handleCreateProtocol = useCallback(() => {
  ...
  }, [clinicId, userId, selectedFolderId]) ? const handleEditProtocol = useCallback(
    async (protocolId: string) => {
      if (!clinicId || !userId) return

      try {
        console.log('[Protocolos3] Carregando protocolo para edição', protocolId) ? const bundle = await loadProtocolBundle(clinicId, userId, protocolId) ? if (bundle) {
          setEditingProtocol(bundle) ? setModalOpen(true)
        }
      } catch (err) {
    ...
    try {
      console.log('[Protocolos3] Salvando protocolo', editingProtocol.protocol.name) ? await saveProtocolBundle(clinicId, userId, editingProtocol)

      // Recarregar lista
      const updatedProtocols = await listProtocols(clinicId, userId) ? console.log('[Protocolos3] Protocols recarregados após salvar', updatedProtocols) ? setProtocols(updatedProtocols) ? setModalOpen(false) ? setEditingProtocol(null) ? clearProtocolDraft()
    } catch (err) {
      console.error('[Protocolos3] Erro ao salvar protocolo', err) ? const errorDetails = safeStringify(err) ? console.error('[Protocolos3] Detalhes do erro:', errorDetails) ? alert(`Falha ao salvar protocolo\n\nDetalhes:\n${errorDetails}`)
    }
  }, [clinicId, userId, editingProtocol]) ? const handleDeleteProtocol = useCallback(
    async (protocolId: string) => {
      if (!clinicId || !userId) return
      if (!confirm('Deseja realmente excluir este protocolo')) return

      try {
        console.log('[Protocolos3] Excluindo protocolo', protocolId) ? await deleteProtocol(clinicId, userId, protocolId)

        // Recarregar lista
        const updatedProtocols = await listProtocols(clinicId, userId) ? console.log('[Protocolos3] Protocols recarregados após exclusão', updatedProtocols) ? setProtocols(updatedProtocols)
      } catch (err) {
        console.error('[Protocolos3] Erro ao excluir protocolo', err) ? const errorDetails = safeStringify(err) ? console.error('[Protocolos3] Detalhes do erro de exclusão:', errorDetails) ? alert(`Falha ao excluir protocolo\n\nDetalhes:\n${errorDetails}`)
      }
    },
    [clinicId, userId]
    ...
    async (folderId: string, folderName: string) => {
      ...
      try {
        await deleteFolder(clinicId, userId, folderId) ? const [refreshedFolders, refreshedProtocols] = await Promise.all([
          listFolders(clinicId, userId),
          listProtocols(clinicId, userId),
        ]) ? setFolders(refreshedFolders) ? setProtocols(refreshedProtocols) ? if (selectedFolderId === folderId) setSelectedFolderId(null)
      } catch (err) {
        console.error('[Protocolos3] Erro ao excluir pasta', err) ? alert(`Falha ao excluir pasta\n\n${safeStringify(err)}`)
      }
    },
    ...
    async (protocolId: string) => {
      if (!clinicId || !userId) {
        alert('Clínica ou usuário não identificado.') ? return
      }
      ...
      try {
        ...
        const bundle = await loadProtocolBundle(clinicId, userId, protocolId) ? if (!bundle) {
          alert('Protocolo não encontrado.') ? return
        }
        ...
        console.error('[Protocolos3] Erro ao aplicar protocolo em Nova Receita', err) ? alert(`Erro ao aplicar protocolo: ${err instanceof Error  err.message : String(err)}`)
      }
    },
    ...
    async (protocolId: string) => {
      ...
      setSelectedProtocolId(protocolId) ? setSelectedProtocolBundle(null) // Clear previous bundle while loading

      try {
        console.log('[Protocolos3] Carregando detalhes do protocolo', protocolId) ? const bundle = await loadProtocolBundle(clinicId, userId, protocolId) ? if (bundle) {
          setSelectedProtocolBundle(bundle) ? console.log('[Protocolos3] Protocolo carregado com sucesso', bundle.protocol.name)
        } else {
          console.error('[Protocolos3] Protocolo não encontrado', protocolId) ? setSelectedProtocolId(null)
        }
      } catch (err) {
        console.error('[Protocolos3] Erro ao carregar detalhes do protocolo', err) ? const errorDetails = safeStringify(err) ? alert(`Falha ao carregar protocolo\n\nDetalhes:\n${errorDetails}`) ? setSelectedProtocolId(null)
      }
    },
    [clinicId, userId]
    ...
    async (medication: MedicationSearchResult) => {
      ...
      try {
        ...
        const presentations = await getMedicationPresentations(clinicId, medication.id) ? const defaultPresentation = presentations[0] // Use first available presentation

        if (!defaultPresentation) {
          alert('Medicamento não possui apresentações cadastradas.') ? return
        }
        ...
        }) ? setMedicationSearchOpen(false) ? setMedicationSearchQuery('')
      } catch (err) {
        console.error('[Protocolos3] Erro ao adicionar medicamento', err) ? alert(`Erro ao adicionar medicamento: ${err instanceof Error  err.message : String(err)}`)
      }
    },
    ...
    (index: number) => {
      ...
      const updated = editingProtocol.medications.filter((_, i) => i !== index) ? setEditingProtocol({
        ...editingProtocol,
        medications: updated,
      })
    },
    ...
    <ReceituarioChrome
      ...
      actions={
        <button
          ...
          onClick={() => {
            clearProtocolDraft() ? if (modalOpen && editingProtocol && !editingProtocol.protocol.id) {
                  ...
                  {editingProtocol.recommendations.map((rec, idx) => (
                    <div key={idx} className="relative group">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const updated = editingProtocol.recommendations.filter((_, i) => i !== idx) ? setEditingProtocol({ ...editingProtocol, recommendations: updated })
                        }}
