...
export interface CatalogPresentation {
  id?: string
  client_id?: string
  // New schema fields (Supabase) ? pharmaceutical_form?: string
  ...
  // Legacy fields (localStorage compatibility) ? name?: string
  presentation?: string
...
function getCurrentUserDbKey(): string {
  let userId = 'anon'
  try {
    const rawUser = localStorage.getItem('luzaum-user') ? if (rawUser) {
      const parsed = JSON.parse(rawUser) as { id?: string }
      const id = String(parsed?.id || '').trim() ? if (id) userId = id
    }
  } catch {
    // noop
  }
...
function nextNumericRecordId(existingIds: string[]): string {
  ...
  existingIds.forEach((rawId) => {
    ...
    const n = Number(id) ? if (Number.isFinite(n) && n > max) max = n
  }) ? return String(max + 1).padStart(5, '0')
}

function allocateClientId(db: RxDatabase, currentId?: string): string {
  const id = String(currentId || '').trim() ? if (isNumericRecordId(id)) return id
...
function allocateAnimalId(db: RxDatabase, currentId?: string, reserved: Set<string> = new Set()): string {
  const id = String(currentId || '').trim() ? if (isNumericRecordId(id) && !reserved.has(id)) {
    reserved.add(id) ? return id
  }
  const pool = db.clients.flatMap((entry) => entry.animals.map((animal) => animal.id)) ? let next = nextNumericRecordId(pool) ? while (reserved.has(next)) {
    const seed = [...pool, ...Array.from(reserved)]
    next = nextNumericRecordId(seed)
  }
  reserved.add(next) ? return next
}
...
function normalizePharmacyTags(
  ...
  const normalized = tags
    ...
    .filter((value): value is PharmacyType => PHARMACY_TYPE_VALUES.includes(value as PharmacyType)) ? return normalized.length ? Array.from(new Set(normalized)) : [fallback]
}

function repairMojibake(value: string): string {
  if (!value) return ''
  const hasReplacementChar = value.includes('\uFFFD') ? const hasCommonMojibakeMarkers = /[ÃÂâ]/.test(value) ? if (!hasReplacementChar && !hasCommonMojibakeMarkers) return value
  ...
  try {
    let decoded = decodeURIComponent(escape(value)) ? for (const [from, to] of Object.entries(replacements)) {
...
function firstCatalogHit(catalog: CatalogDrug[], query: string): CatalogDrug | undefined {
  const needle = query.toLowerCase() ? return catalog.find((drug) => drug.name.toLowerCase().includes(needle))
}
...
function createProtocolItemFromCatalog(
  ...
  const drug = firstCatalogHit(catalog, query) ? const presentation = drug?.presentations?.[0]
...
function defaultDb(): RxDatabase {
  const catalog = seedCatalog() ? const now = nowIso() ? const defaultPrescriberProfile: PrescriberProfile = {
    ...defaultProfile,
    id: 'default',
...
function normalizeCatalogPresentation(
  ...
  const concentrationValue = repairMojibake(raw.concentrationValue || '') ? const concentrationUnit = repairMojibake(raw.concentrationUnit || '') ? const concentrationPerValue = repairMojibake(raw.concentrationPerValue || '1') ? const concentrationPerUnit = repairMojibake(raw.concentrationPerUnit || '') ? const structuredConcentration = formatStructuredConcentration({
    value: concentrationValue,
    unit: concentrationUnit,
    perValue: concentrationPerValue,
    perUnit: concentrationPerUnit,
  }) ? const rawConcentration = repairMojibake(raw.concentration || '') ? return {
    id: raw.id || uid('pres'),
...
function normalizeProtocol(raw: Partial<RxProtocol>, fallbackFolderId: string): RxProtocol {
  const now = nowIso() ? const items = Array.isArray(raw.items) ? raw.items.map((item) => createProtocolTemplateItem(item)) : []
  const hasControlledItems = items.some((item) => !!item.controlled) ? return {
...
function normalizeTemplate(raw: Partial<RxTemplateStyle>): RxTemplateStyle {
  const base = createDefaultTemplateStyle(raw.id || uid('template'), raw.name || 'Template') ? const sourceZoneStyles = typeof raw.zoneStyles === 'object' && raw.zoneStyles ? raw.zoneStyles : {}
...
function normalizeClientAnimal(raw: Partial<ClientAnimalRecord>): ClientAnimalRecord {
  ...
  const fallbackWeight = repairMojibake(raw.weightKg || '') ? if (!weightHistory.length && fallbackWeight) {
    weightHistory.push({
      id: uid('w'),
...
function buildLegacyClientsFromPatients(patients: PatientRecord[]): ClientRecord[] {
  const map = new Map<string, ClientRecord>() ? patients.forEach((patient) => {
    const tutorName = repairMojibake(patient.tutorName || '').trim() || 'Tutor sem nome'
    const key = tutorName.toLowerCase() ? const existing = map.get(key) ? const animal = normalizeClientAnimal({
    ...
    const hasAnimal = existing.animals.some((entry) => entry.id === animal.id) ? existing.animals = hasAnimal
      ? existing.animals.map((entry) => (entry.id === animal.id ? animal : entry))
      : [...existing.animals, animal]
    if (!existing.phone && patient.tutorPhone) existing.phone = repairMojibake(patient.tutorPhone) ? if (!existing.street && patient.tutorAddress) existing.street = repairMojibake(patient.tutorAddress) ? existing.updatedAt = nowIso()
  }) ? return Array.from(map.values())
}

export function loadRxDb(): RxDatabase {
  try {
    const scopedKey = getCurrentUserDbKey() ? const rawScoped = localStorage.getItem(scopedKey) ? const rawLegacy = localStorage.getItem(LEGACY_DB_KEY) ? const raw = rawScoped || rawLegacy
    if (!raw) return defaultDb() ? const parsed = JSON.parse(raw) as Partial<RxDatabase>
    const base = defaultDb() ? const protocolFolders =
...
export function upsertPrescriberProfile(db: RxDatabase, profile: PrescriberProfile): RxDatabase {
  ...
  const exists = db.prescriberProfiles.some((entry) => entry.id === nextProfile.id) ? const prescriberProfiles = exists
...
export function removePrescriberProfile(db: RxDatabase, profileId: string): RxDatabase {
  const prescriberProfiles = db.prescriberProfiles.filter(p => p.id !== profileId) ? if (prescriberProfiles.length === 0) return db // do not allow removing the last profile
...
export function upsertDrug(db: RxDatabase, drug: CatalogDrug): RxDatabase {
  const normalizedDrug = normalizeCatalogDrug(drug) ? const normalizedPresentations = normalizedDrug.presentations.length
  ...
  const exists = db.catalog.some((entry) => entry.id === nextDrug.id) ? const catalog = exists
...
export function upsertProtocol(db: RxDatabase, protocol: RxProtocol): RxDatabase {
  const normalizedItems = protocol.items.map((item) => createProtocolTemplateItem(item)) ? const nextProtocol: RxProtocol = {
    ...protocol,
  ...
  const exists = db.protocols.some((entry) => entry.id === nextProtocol.id) ? return {
    ...db,
    protocols: exists
...
export function upsertProtocolFolder(db: RxDatabase, folder: RxProtocolFolder): RxDatabase {
  const exists = db.protocolFolders.some((entry) => entry.id === folder.id) ? const protocolFolders = exists
...
export function removeProtocolFolder(db: RxDatabase, folderId: string): RxDatabase {
  const protocolFolders = db.protocolFolders.filter((entry) => entry.id !== folderId) ? if (protocolFolders.length === 0) return db
...
export function upsertTemplate(db: RxDatabase, template: RxTemplateStyle): RxDatabase {
  const nextTemplate = normalizeTemplate({ ...template, updatedAt: nowIso() }) ? const exists = db.templates.some((entry) => entry.id === nextTemplate.id) ? const templates = exists
...
export function removeTemplate(db: RxDatabase, templateId: string): RxDatabase {
  const templates = db.templates.filter((entry) => entry.id !== templateId) ? if (templates.length === 0) {
    const fallback = createDefaultTemplateStyle() ? return {
      ...db,
      templates: [fallback],
...
export function upsertClient(db: RxDatabase, client: ClientRecord): RxDatabase {
  const now = nowIso() ? const normalizedInput = normalizeClient({ ...client, updatedAt: now }) ? const sourceId = normalizedInput.id
  const nextClientId = allocateClientId(db, normalizedInput.id) ? const reservedAnimalIds = new Set<string>() ? const nextAnimals = normalizedInput.animals.map((animal) =>
    ...
    animals: nextAnimals,
    updatedAt: now,
  }) ? const filtered = db.clients.filter((entry) => entry.id !== nextClientId && entry.id !== sourceId) ? return {
    ...db,
...
function isPlaceholderRecordId(value: string | null | undefined): boolean {
  const id = String(value || '').trim().toUpperCase() ? return !id || id === 'ADMIN' || id === 'DEFAULT' || id.startsWith('TMP-')
}

function findClientByTutorData(db: RxDatabase, rx: PrescriptionState): ClientRecord | null {
  const cpf = digitsOnlyText(rx.tutor.cpf || '') ? const phone = digitsOnlyText(rx.tutor.phone || '') ? const tutorName = normalizeLookupText(rx.tutor.name || '') ? if (cpf) {
    const byCpf = db.clients.find((entry) => digitsOnlyText(entry.cpf) === cpf) ? if (byCpf) return byCpf
  }
  ...
  if (tutorName) {
    const byName = db.clients.find((entry) => normalizeLookupText(entry.fullName || '') === tutorName) ? if (byName) return byName
  }
...
function findAnimalInClient(client: ClientRecord | null, rx: PrescriptionState): ClientAnimalRecord | null {
  ...
  const patientName = normalizeLookupText(rx.patient.name || '') ? const species = normalizeLookupText(rx.patient.species || '') ? if (!patientName) return null
  ...
  const byName = client.animals.find((entry) => normalizeLookupText(entry.name || '') === patientName) ? return byName || null
}

export function upsertPatientFromPrescription(db: RxDatabase, rx: PrescriptionState): RxDatabase {
  const patientName = rx.patient.name.trim() ? if (!patientName) return db
  const explicitPatientId = (rx.patient.patientRecordId || '').trim() ? const normalizedPatientName = normalizeLookupText(patientName) ? const normalizedTutorName = normalizeLookupText(rx.tutor.name || '') ? const matchedByName = db.patients.find(
    (entry) =>
    ...
    : matchedByName && isNumericRecordId(matchedByName.id)
      ? matchedByName.id
      : nextNumericRecordId(db.patients.map((entry) => entry.id)) ? const nextRecord: PatientRecord = {
    id: patientId,
    name: patientName,
...
export function upsertClientFromPrescription(db: RxDatabase, rx: PrescriptionState): RxDatabase {
  const tutorName = (rx.tutor.name || '').trim() ? if (!tutorName) return db
  const explicitTutorId = (rx.tutor.tutorRecordId || '').trim() ? const linkedClientFromId = !isPlaceholderRecordId(explicitTutorId)
    ? db.clients.find((entry) => entry.id === explicitTutorId) || null
    : null
  const inferredClient = linkedClientFromId || findClientByTutorData(db, rx) ? const clientId = linkedClientFromId?.id || inferredClient?.id || ''

  const explicitPatientId = (rx.patient.patientRecordId || '').trim() ? const linkedAnimalFromId =
    !isPlaceholderRecordId(explicitPatientId) && inferredClient
      ? inferredClient.animals.find((entry) => entry.id === explicitPatientId) || null
      : null
  const inferredAnimal = linkedAnimalFromId || findAnimalInClient(inferredClient, rx) ? const animalId = linkedAnimalFromId?.id || inferredAnimal?.id || ''
  const now = nowIso() ? const existingClient = db.clients.find((entry) => entry.id === clientId) ? const existingAnimal = existingClient?.animals.find((entry) => entry.id === animalId) ? const currentWeight = (rx.patient.weightKg || '').trim() ? const providedWeightDate = (rx.patient.weightDate || '').trim() ? const currentWeightDate = providedWeightDate || now
...
export function upsertHistoryFromPrescription(db: RxDatabase, rx: PrescriptionState): RxDatabase {
  ...
  const resolvedClient =
    ...
    findClientByTutorData(db, rx) ? const tutorId = resolvedClient?.id || explicitTutorId
  ...
  const resolvedAnimal =
    (explicitPatientId && resolvedClient
      ...
      : null) || findAnimalInClient(resolvedClient || null, rx) ? const patientId = resolvedAnimal?.id || explicitPatientId
  ...
  const exists = db.history.some((entry) => entry.id === rx.id) ? const history = exists
