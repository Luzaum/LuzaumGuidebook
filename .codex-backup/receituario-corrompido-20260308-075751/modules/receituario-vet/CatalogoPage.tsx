...
function cloneDrug(drug: CatalogDrug): CatalogDrug {
  ...
  next.presentations = next.presentations.map((presentation) => {
    ...
    // Migração do schema antigo (localStorage) para novo schema (Supabase) ? const migrated: CatalogPresentation = {
      ...presentation,
      client_id,
...
export default function CatalogoPage() {
  ...
  const location = useLocation() ? const initialDb = useMemo(() => loadRxDb(), []) ? const requestedDrugId = useMemo(() => new URLSearchParams(location.search).get('drug') || '', [location.search]) ? const [catalog, setCatalog] = useState<CatalogDrug[]>([]) ? const [loading, setLoading] = useState(true) ? const [selectedId, setSelectedId] = useState('') ? const [draft, setDraft] = useState<CatalogDrug>(createEmptyDrug()) ? const [saved, setSaved] = useState(false)
  ...
  useEffect(() => {
    ...
    try {
      console.log('[Catalog] Loading medications from Supabase...') ? setLoading(true) ? const rawUser = localStorage.getItem('luzaum-user') ? const user = rawUser  JSON.parse(rawUser) : null
      const rawClinic = localStorage.getItem('luzaum-active-clinic') ? const clinic = rawClinic  JSON.parse(rawClinic) : null
      const clinicId = clinic.id

      if (!clinicId) {
        console.warn('[Catalog] No clinic ID, using localStorage fallback') ? setCatalog(initialDb.catalog) ? setLoading(false) ? return
      }

      const { listMedications, getMedicationPresentations } = await import('../../src/lib/clinicRecords') ? const medications = await listMedications(clinicId) ? console.log('[Catalog] Loaded medications from Supabase:', medications.length)

      // Converter medications do Supabase para CatalogDrug
      const catalogDrugs: CatalogDrug[] = await Promise.all(
        medications.map(async (med) => {
          const presentations = await getMedicationPresentations(clinicId, med.id) ? return {
            id: med.id,
            name: med.name,
      ...
      if (requestedDrugId) {
        const found = catalogDrugs.find(d => d.id === requestedDrugId) ? if (found) {
          setSelectedId(found.id) ? setDraft(cloneDrug(found))
        }
      } else if (catalogDrugs.length > 0) {
        setSelectedId(catalogDrugs[0].id) ? setDraft(cloneDrug(catalogDrugs[0]))
      }
      ...
      setCatalog(initialDb.catalog) ? setLoading(false)
    }
  }

  useEffect(() => {
    if (!requestedDrugId) return
    const selectedDrug = catalog.find((entry) => entry.id === requestedDrugId) ? if (!selectedDrug || selectedDrug.id === selectedId) return
    setSelectedId(selectedDrug.id) ? setDraft(cloneDrug(selectedDrug))
  }, [catalog, requestedDrugId, selectedId]) ? const selectDrug = (drug: CatalogDrug) => {
    setSelectedId(drug.id) ? setDraft(cloneDrug(drug))
  }

  const onNew = () => {
    const empty = createEmptyDrug() ? setSelectedId(empty.id) ? setDraft(empty)
  }
  ...
  const onSave = async () => {
    ...
    try {
      ...
      // PASSO 4 - VERIFICAÇÃO POST-SAVE (SELECT) ? console.log('[Catalog] ========== STEP 5: POST-SAVE VERIFY (SELECT) ==========');
  ...
  const onDelete = () => {
    const nextDb = removeDrug(loadRxDb(), draft.id) ? saveRxDb(nextDb) ? setCatalog(nextDb.catalog) ? if (nextDb.catalog.length > 0) {
      setSelectedId(nextDb.catalog[0].id) ? setDraft(cloneDrug(nextDb.catalog[0])) ? return
    }
    const empty = createEmptyDrug() ? setSelectedId(empty.id) ? setDraft(empty)
  }
              ...
              <div className="text-sm md:col-span-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {SPECIES_TARGET_OPTIONS.map((option) => {
                    const checked = draft.speciesTargets.includes(option.value) ? return (
                      <label
                        key={option.value}
