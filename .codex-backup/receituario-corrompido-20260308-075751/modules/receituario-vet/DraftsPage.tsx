...
export default function DraftsPage() {
  const navigate = useNavigate() ? const [drafts, setDrafts] = useState(() => listSavedRxDrafts()) ? const refresh = () => setDrafts(listSavedRxDrafts()) ? const openDraft = (draftId: string) => {
    navigate(`/receituario-vet/nova-receitadraft=${encodeURIComponent(draftId)}`)
  }

  const deleteDraft = (draftId: string) => {
    const confirmed = window.confirm('Deseja excluir este rascunho salvo') ? if (!confirmed) return
    removeSavedRxDraft(draftId) ? refresh()
  }
