...
export function formatStructuredConcentration(parts: Partial<StructuredConcentration>): string {
  const value = (parts.value || '').trim() ? const unit = (parts.unit || '').trim() ? const perUnit = (parts.perUnit || '').trim() ? const perValue = (parts.perValue || '').trim() || '1'
...
export function parseStructuredConcentration(text: string): StructuredConcentration | null {
  const normalized = (text || '').trim() ? if (!normalized) return null
  ...
  const simple = normalized.match(/(\d+(:[.,]\d+))\s*([a-zA-Z%µ]+)/i) ? if (!simple) return null
