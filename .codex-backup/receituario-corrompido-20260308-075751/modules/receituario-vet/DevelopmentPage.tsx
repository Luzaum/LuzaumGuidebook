...
export default function DevelopmentPage() {
  const navigate = useNavigate() ? const [selectedId, setSelectedId] = useState(() => readHomeIconSelection().id) ? const [savedAt, setSavedAt] = useState<string>('') ? const selected = useMemo(
    () => HOME_ICON_OPTIONS.find((item) => item.id === selectedId) || HOME_ICON_OPTIONS[0],
    [selectedId]
  ) ? const applyIcon = (iconId: string) => {
    const applied = saveHomeIconSelection(iconId) ? setSelectedId(applied.id) ? setSavedAt(new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }))
  }
