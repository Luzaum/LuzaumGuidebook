# -*- coding: utf-8 -*-
"""Generate diseases.arritmias-cardiacas-caes-gatos.seed.ts"""
from pathlib import Path

OUT = Path(__file__).resolve().parents[1] / "modules/consulta-vet/data/seed/diseases.arritmias-cardiacas-caes-gatos.seed.ts"

OUT.write_text("""import type { DiseaseRecord } from '../../types/disease';
...
part3 = r"""
  ...
  diagnosis: {
    ...
    figuraNaoTrateEcg: {
      ...
      src: '/assets/consulta-vet/diseases/arritmias-cardiacas-caes-gatos/nao-trate-apenas-ecg.svg',
      alt: 'Fluxograma editorial: antes de antiarrítmico, avaliar estabilidade hemodinâmica, substrato cardíaco, causa reversível e objetivo terapêutico.',
      caption:
        'Não tratar apenas o traçado do ECG — integrar perfusão, pulso, ecocardiografia e monitorização prolongada quando indicado. (1)(4)(5)',
      display: 'wide',
    },
    holterIndicacoes:
...
print("part2+3 appended")

part4 = r"""
  treatment: {
    figuraEstratificacaoUrgencia: {
      kind: 'clinicalFigure' as const,
      src: '/assets/consulta-vet/diseases/arritmias-cardiacas-caes-gatos/estratificacao-urgencia.svg',
      alt: 'Estratificação de urgência ConsultaVET para arritmias: monitorar, prioritário, urgente e emergência de ressuscitação.',
      caption:
        'Estratificação de urgência — organização clínica ConsultaVET para priorizar conduta. NÃO é sistema de estadiamento consensual comparável ao ACVIM B1–D.',
      display: 'wide',
    },
    ...
    figuraFluxogramaPrincipal: {
      kind: 'clinicalFigure' as const,
      src: '/assets/consulta-vet/diseases/arritmias-cardiacas-caes-gatos/fluxograma-principal.svg',
      alt: 'Fluxograma principal: ritmo anormal → pulso? → estabilidade → taqui ou bradi → conduta específica.',
      caption: 'Fluxograma principal de abordagem — pulso ausente direciona para RECOVER 2024. (4)(25)',
      display: 'wide',
    },
    figuraFluxogramaFAOrca: {
      kind: 'clinicalFigure' as const,
      src: '/assets/consulta-vet/diseases/arritmias-cardiacas-caes-gatos/fluxograma-fa-orca.svg',
      alt: 'Fluxograma de controle de frequência na fibrilação atrial canina baseado nos critérios ORCA.',
      caption:
        'Controle de frequência na FA canina — conceito ORCA: ECG >150 bpm sugere controle inadequado; ECG <150 bpm exige Holter; meta FC média 24 h ≤125 bpm. Algoritmo original ORCA (CC BY-NC-ND): https://pmc.ncbi.nlm.nih.gov/articles/PMC10229328/#F1 (10)',
      display: 'wide',
    },
    figuraVtComSemPulso: {
      kind: 'clinicalFigure' as const,
      src: '/assets/consulta-vet/diseases/arritmias-cardiacas-caes-gatos/vt-com-pulso-vs-sem-pulso.svg',
      alt: 'Comparativo: taquicardia ventricular com pulso versus VT/VF sem pulso — condutas distintas.',
      caption:
        'VT com pulso ≠ VT/VF sem pulso — algoritmos completamente diferentes. Sem pulso: RECOVER 2024. (25)',
      display: 'wide',
    },
    arrFaCaninaOrca:
...
print("complete:", OUT, "lines:", sum(1 for _ in open(OUT, encoding="utf-8")))
