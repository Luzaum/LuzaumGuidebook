/**
 * Cria ou atualiza os usuários autorizados do Vetius no Supabase Auth.
 * Requer SUPABASE_SERVICE_ROLE_KEY e as quatro senhas no ambiente.
 *
 * Uso: npm run seed:app-users
 */
...
const INITIAL_APP_LOGINS = ['luis', 'pimenta', 'peduzzi', 'lobo'] as const
...
function loadEnvLocal(): Record<string, string> {
  ...
  try {
    ...
    // O arquivo é opcional quando as variáveis já estão no processo.
  }
  return values
}

function requiredValue(name: string, fileValues: Record<string, string>): string {
...
async function main() {
  ...
  const url =
    String(process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || fileValues.VITE_SUPABASE_URL || '').trim()
  ...
  const users = INITIAL_APP_LOGINS.map((login) => ({
    login,
