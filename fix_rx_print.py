...
filepath = r"c:\PROJETOS VET\Vetius\modules\receituario-vet\RxPrintPage.tsx"

with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# The return JSX block needs to be inserted before the closing of openEmail (line 393 is the closing })
# and after line 393, the file currently has the <aside> block orphaned.
# We need to wrap the existing lines 394-595 in a proper return ( <div> ... </div> )

# Split by the marker - end of openEmail function  
MARKER = "    pushToast('Anexo automático indisponível neste navegador. PDF baixado para envio manual.')\n  }\n\n"

idx = content.rfind(MARKER)
if idx == -1:
    print("Marker not found. Trying CRLF version...")
    MARKER = "    pushToast('Anexo autom\u00e1tico indispon\u00edvel neste navegador. PDF baixado para envio manual.')\r\n  }\r\n\r\n"
    idx = content.rfind(MARKER)
...
before_return = content[:idx + len(MARKER)]
            ...
            <div className="space-y-4">
              {printDocs.map((doc, idx) => (
                <RxPrintView
                  ...
                  onZoneSelect={(zone) => setSelection({ kind: 'zone', zone })}
                  onItemSelect={(itemId) => setSelection({ kind: 'item', itemId })}
                />
              ))}
            </div>
          </div>
...
print(f"Done. Total lines: {len((before_return + return_block).splitlines())}")
